...
Page 75045 "IFP Card"
{
        ...
        area(content)
        {
            group(General)
            {
                field("Code"; Rec.Code)
                {
                    ...
                    Editable = false;
                    ToolTip = 'Specifies the value of the IFP Notice No. field.';
                }
                ...
                field("Period Start Date"; Rec."Period Start Date")
                {
                    ...
                    ToolTip = 'Specifies the value of the Prequalification Period Start Date field.';
                }
                field("Period End Date"; Rec."Period End Date")
                {
                    ApplicationArea = Basic;
                    Caption = 'Prequalification Period End Date';
                    ToolTip = 'Specifies the value of the Prequalification Period End Date field.';
                }
                ...
                field("Responsibility Center descr"; Rec."Responsibility Center descr")
                {
                    ApplicationArea = Basic;
                    ToolTip = 'Specifies the value of the Responsibility Center descr field.';
                }
                ...
                field("Constituency description"; Rec."Constituency description")
                {
                    ApplicationArea = Basic;
                    Visible = false;
                    ToolTip = 'Specifies the value of the Constituency description field.';
                }
            ...
            {
                SubPageLink = "Document No" = field(Code),
                              "Document Type" = const("Invitation For Prequalification");
            }
        }
        area(factboxes)
        {
        ...
        area(processing)
        {
            group(Approval)
            {
                ...
                action(Approve)
                {
                    ...
                    var
                        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
                    begin
                        //ApprovalsMgmt.ApproveRecordApprovalRequest();
                    end;
                }
                action(Reject)
                {
            ...
            group("F&unctions")
            {
                ...
                action("Print IFP")
                {
                    ...
                    ToolTip = 'Executes the Print IFP action.';

                    trigger OnAction()
                    begin
                        Rec.Reset;
                        Rec.SetRange(Code, Rec.Code);
                        Report.Run(70063, true, true, Rec);
                    end;
                }
                action("Prequalification Responses")
                {
                    ApplicationArea = Basic;
                    Caption = 'Prequalification Responses Report';
                    Image = Print;
                    Promoted = true;
                    visible = false;
                    PromotedCategory = "Report";
                    ToolTip = 'Executes the Prequalification Responses Report action.';

                    trigger OnAction()
                    begin
                        Rec.Reset;
                        Rec.SetRange(Code, Rec.Code);
                        Report.Run(70097, true, true, Rec);
                    end;
                }
    ...
    begin
        ...
        // WorkflowWebhookMgt.GetCanRequestAndCanCancel(Record CanRequestApprovalForFlow, ow,CanCancelApprovalForFlow);
    end;
}
