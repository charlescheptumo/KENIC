...
Table 69701 "PreScreen Header"
{
    ...
    begin
        ...
        if ("Document Type" = "document type"::"Phone Interview Report") then begin
            ...
            if "Document No." = '' then
                "Document No."  := NoSeriesMgt.GetNextNo(HRSetup."Phone Interview Report Nos.", WorkDate(), true);
        end;
        if ("Document Type" = "document type"::"Ability Test Invitation") then begin
            HRSetup.TestField("Ability Test Invitation Nos.");
            if "Document No." = '' then
                 "Document No."  := NoSeriesMgt.GetNextNo(HRSetup."Ability Test Invitation Nos.", WorkDate(), true);
        end;
        "Created By" := UserId;
