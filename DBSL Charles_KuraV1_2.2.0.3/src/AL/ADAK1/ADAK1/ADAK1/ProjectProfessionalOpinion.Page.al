...
Page 72669 "Project Professional Opinion"
{
    DeleteAllowed = false;
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("DG Comments"; Rec."DG Comments")
                {
                    ApplicationArea = Basic;
                    MultiLine = true;
                    ToolTip = 'Specifies the value of the DG Comments field.';
                }
            ...
            part(Control46; "Project POP Evaluation Lines")
            {
                SubPageLink = "Document Type" = field("Document Type"),
                              "Document No." = field("Document No.");
            }
            part(Control49; "Project Eval Time Subpage")
            {
                Caption = 'Revised Activity Schedule';
                SubPageLink = "Document Type" = field("Document Type"),
                              "Document No." = field("Document No.");
                Visible = false;
            }
    ...
    actions
    {
        area(navigation)
        {
            action("Extension Justification")
            {
                ...
                ToolTip = 'Executes the Extension Justification action.';
            }
            ...
            action("Review Committee")
            {
                ...
                RunPageLink = "Document Type" = field("Document Type"),
                              "Document No." = field("Document No.");
                ToolTip = 'Executes the Review Committee action.';
            }
            action(Print)
            {
                ApplicationArea = Basic;
            ...
            action(Post)
            {
                ...
                begin
                    ...
                    // RMSManagement.FNPostPOP(JobPlanningLine,ProjectEvalTimeLine,Rec);
            ...
            action("Reject Professional Opinion ")
            {
                ApplicationArea = Basic;
                Image = Reject;
                Promoted = true;
                ...
                PromotedIsBig = true;
                PromotedOnly = true;
                ToolTip = 'Executes the Reject Professional Opinion  action.';

                trigger OnAction()
                begin
                    Rec.TestField("DG Comments");
            ...
            action(Acknolwledge)
            {
                ...
                PromotedCategory = Process;
                PromotedIsBig = true;
                ToolTip = 'Executes the Acknolwledge action.';

                trigger OnAction()
                begin

                    Message('Acnowledged Successfully');
                end;
            }
    ...
    var
        ProjectEvalTimeLine: Record "Project Eval Time Line";
        Job: Record Job;
