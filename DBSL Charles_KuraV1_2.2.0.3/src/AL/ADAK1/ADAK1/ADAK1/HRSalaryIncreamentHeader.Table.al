...
Table 69058 "HR Salary Increament Header"
{
    ...
    fields
    {
        ...
        {
            DataClassification = ToBeClassified;
            OptionCaption = 'Open,Pending Approval,Approved,Pending Prepayment,Released,Rejected,Closed';
            OptionMembers = Open,"Pending Approval",Approved,"Pending Prepayment",Released,Rejected,Closed;
        }
    ...
    begin
        if "Batch No" = '' then begin
            ...
            HumanResSetup.TestField("Salary Icreament Nos");
                  rec. "Batch No" := NoSeriesMgt.GetNextNo(HumanResSetup."Salary Icreament Nos", WorkDate(), true);
        end;
        "Run By" := UserId;
        Date := Today;
    end;
