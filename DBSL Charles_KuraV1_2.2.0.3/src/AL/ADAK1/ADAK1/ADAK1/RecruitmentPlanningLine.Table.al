...
Table 69616 "Recruitment Planning Line"
{
    ...
    fields
    {
        ...
        // {
        //     DataClassification = ToBeClassified;
        //     TableRelation = "Establishment Planning Line"."Position ID" where("Department Code" = field(Department));// where("Department Code" = field(Department),
        //                                                                                                              // Blcoked = const(false));
        ...
        field(2; "Position ID"; Code[30])
        {
            ...
            trigger OnValidate()
            var
                EPL: Record "Establishment Plan Position";
                EstablishmentPlan: Record "Staff Establishment Plan";
            begin
                RecruitmentPlan.Reset;
                RecruitmentPlan.SetRange("Document No.", "Recruitment Plan ID");
                /* EPL.Reset();
                EPL.SetRange("Job ID","Position ID");
                if EPL.FindFirst() then begin
                    EPL.CalcFields()
                end; */
                if RecruitmentPlan.FindSet then begin
                    if RecruitmentPlan."Recruitment Plan Type" = RecruitmentPlan."recruitment plan type"::"Special Recruitment"
                ...
                EstablishmentPlan.Reset();
                EstablishmentPlan.FindFirst();

                EPL.Reset();
                EPL.SetRange("Staff Establishment Code", EstablishmentPlan.Code);
                EPL.SetRange("Job ID", Rec."Position ID");
                if EPL.FindFirst() then begin
                    EPL.CalcFields("Approved No.", "Actual Active");
                    "Staff Establishment" := EPL."Approved No.";
                    "Current Headcount" := EPL."Actual Active";
                end;
            end;
        }
