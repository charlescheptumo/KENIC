...
Page 75237 "Standard Purchase Requisition"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                {
                    ApplicationArea = Basic;
                    ToolTip = 'Specifies the value of the Total PRN Amount field.';
                }
                field("Total PRN Amount (LCY)"; Rec."Total PRN Amount (LCY)")
                {
                    ApplicationArea = Basic;
                    ToolTip = 'Specifies the value of the Total PRN Amount (LCY) field.';
                }
                ...
                {
                    ApplicationArea = Basic;
                    ToolTip = 'Specifies the value of the Linked LPO No. field.';
                }
                ...
                field("Reasons for Termination"; Rec."Reasons for Termination")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Reasons for Termination field.', Comment = '%';
                    Editable = true;
                }
            }
    ...
    actions
    {
        area(navigation)
        {
            group("Purchase Requisition")
            {
                ...
                action("Terminate Requisition")
                {
                    Promoted = true;
                    PromotedCategory = Category8;
                    ApplicationArea = All;
                    Caption = 'Terminate Requisition';
                    Image = RemoveContacts;
                    ToolTip = 'Executes the Terminate Requisition action.';

                    trigger OnAction()
                    begin
                        if not Confirm('Do you want to terminate this requisiton %1', true, Rec."No.") then
                            error('Requisition not terminated');

                        Rec.TestField("Reasons for Termination");

                        Rec."Is Terminated" := true;
        ...
        area(processing)
        {
            ...
            group("F&unctions")
            {
                ...
                action("Create LPO/LSO")
                {
                    ...
                    //  Rec.status = Rec.status::Released;                    ToolTip = 'Executes the Create LPO/LSO action.';
                ...
                action("Create Request For Invitation")
                {
                    ...
                    ToolTip = 'Executes the Create Request For Invitation action.';
            ...
            group("Request Approval")
            {
                ...
                action(SendApprovalRequest)
                {
                    ...
                    begin
                        ...
                        if confirm('Kindly Ensure you have Attached Required documentation to proceed. Continue?', false) = true then begin
    ...
    begin
        ...
        if (UserSetup."Procurement Manager" = true) and (Rec.Status = Rec.Status::Released) then begin
            Edit := true;
        end;
