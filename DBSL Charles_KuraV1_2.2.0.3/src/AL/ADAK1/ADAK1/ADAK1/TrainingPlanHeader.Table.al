...
Table 69137 "Training Plan Header"
{
    ...
    fields
    {
        ...
        {
            CalcFormula = sum("Planning Line Entry"."Available Budget" where("Training Plan Id" = field(No)));
            FieldClass = FlowField;
            Editable = false;
        }
        field(17; "Budget Allocation"; Decimal)
        {
            CalcFormula = sum("Planning Line Entry"."Total Budget" where("Training Plan Id" = field(No)));
            FieldClass = FlowField;
            Editable = false;
        }
        field(18; "Budget Utilized"; Decimal)
        {
            CalcFormula = sum("Planning Line Entry"."Budget Utilized" where("Training Plan Id" = field(No)));
            FieldClass = FlowField;
            Editable = false;
        }
    ...
    begin
        ...
        if No = '' then begin
            ...
            No := NoSeriesMgt.GetNextNo(HRSetup."Traning Planning Nos", WorkDate(), true);
        end;
