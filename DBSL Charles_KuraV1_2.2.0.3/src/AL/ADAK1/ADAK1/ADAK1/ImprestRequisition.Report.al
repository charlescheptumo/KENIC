...
Report 57004 "Imprest Requisition"
{
    ...
    dataset
    {
        dataitem(payments; payments)
        {
            ...
            column(CreatedBySignature; ApplicantUserSetup.Picture)  // CHANGED: Now uses Applicant's signature
            {

            }
            column(CreatorName; ApplicantName)  // CHANGED: Now uses Applicant's name (Account Name)
            {
            ...
            begin
                ...
                // CHANGED: Get Applicant details (Account holder) instead of Creator
                // The applicant is the person requesting the imprest (Account Name/Payee)
                ApplicantName := payments."Account Name";  // Use Account Name as Applicant
                
                // Try to get the applicant's signature from User Setup based on Account No
                if ApplicantUserSetup.Get(payments."Account No.") then begin
                    ApplicantUserSetup.CalcFields(Picture);
                end else begin
                    // If Account No is not a User ID, try to find by Employee Name
                    ApplicantUserSetup.Reset();
                    ApplicantUserSetup.SetRange("Employee Name", payments."Account Name");
                    if ApplicantUserSetup.FindFirst() then
                        ApplicantUserSetup.CalcFields(Picture);
                end;
    ...
    var
        ...
        ApplicantUserSetup: Record "User Setup";  // CHANGED: Renamed from CreatorUserSetup
