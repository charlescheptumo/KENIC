...
Table 69011 "Loan Application"
{
    ...
    fields
    {
            ...
            begin
                ...
                if LoanType.Get("Loan Product Type") then begin
                    if "Interest Calculation Method" = "interest calculation method"::"Reducing Balance" then begin
                        ...
                        Repayment := ROUND("Approved Amount" / Installments, LoanType."Rounding Precision", '>');
                ...
                if "Interest Calculation Method" = "interest calculation method"::"Flat Rate" then begin
                    ...
                    "Flat Rate Interest" := ROUND(FlatRateCalc("Approved Amount", "Interest Rate"), LoanType."Rounding Precision", '>');
                    // "Flat Rate Principal" := Repayment - "Flat Rate Interest";
                    "Flat Rate Principal" := ROUND((Repayment - "Flat Rate Interest"), LoanType."Rounding Precision", '>');
                end;
        ...
        field(8; Instalment; Integer)
        {
            ...
            begin
                if (("Start Date" <> 0D) and ("End Date" <> 0D)) then
                    Error('Cannot manually change Instalment when Start Date and End Date are specified. Clear the dates first.');
        ...
        field(9; Repayment; Decimal)
        {
            ...
            begin
                if LoanProduct.Get(LoanProduct."Code") then
                    Repayment := Round(Repayment, LoanProduct."Rounding Precision", '>');
            end;
        }
        field(10; "Flat Rate Principal"; Decimal)
        {
            ...
            begin
                if LoanProduct.Get(LoanProduct."Code") then
                    "Flat Rate Principal" := Round("Flat Rate Principal", LoanProduct."Rounding Precision", '>');
            end;
        }
    ...
    begin
        ...
        if "Loan No" = '' then begin
            if LoanType.Get("Loan Product Type") then begin
                ...
                "Loan No":= NoSeriesMgt.GetNextNo(LoanType."Loan No Series", WorkDate(), true);
            end;
        end
    end;
    ...
    local procedure CalculateInstallments()
    var
    ...
    begin
        ...
        MonthsBetween := CalculateMonthDifference("Start Date", "End Date");

        if MonthsBetween <= 0 then
            Error('The loan period must be at least 1 month');

        Instalment := MonthsBetween;

        if "Approved Amount" <> 0 then
            Validate("Approved Amount");
    end;

    local procedure CalculateMonthDifference(StartDate: Date; EndDate: Date): Integer
    var
        StartYear: Integer;
        StartMonth: Integer;
        EndYear: Integer;
    ...
    begin
        ...
        // Calculate total months: (year difference * 12) + month difference
        TotalMonths := ((EndYear - StartYear) * 12) + (EndMonth - StartMonth);

        // Add 1 to include both start and end months
        // For example: Jan 2024 to Mar 2024 = 3 months (Jan, Feb, Mar)
        exit(TotalMonths + 1);
    end;
