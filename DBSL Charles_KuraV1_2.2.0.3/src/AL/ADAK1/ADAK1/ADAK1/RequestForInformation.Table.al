...
Table 70084 "Request For Information"
{
    ...
    begin
        if Code = '' then begin
           ...
           Code:= NoSeriesMgt.GetNextNo(PurchSetup."IFP Nos.", WorkDate(), true);
            end;
            if "Document Type" = "document type"::"Invitation for Registation" then begin
                PurchSetup.Get;
                PurchSetup.TestField("Vendor Registration Invite Nos");
                  Code:= NoSeriesMgt.GetNextNo(PurchSetup."Vendor Registration Invite Nos", WorkDate(), true);
            end else begin
                PurchSetup.Get;
                PurchSetup.TestField("EOI No. Series");
                      Code:= NoSeriesMgt.GetNextNo(PurchSetup."EOI No. Series", WorkDate(), true);
            end;
        end;
