                        ...
                        if Answer = true then begin
                            ...
                            repeat     //*********start HRLeaveTypes loop
                                ...
                                begin
                                    ...
                                    HRLeaveLedger."Leave Posting Description" := 'Reimbursed Leave Days';
                                    ...
                                    if HRLeaveLedger."No. of days" > 0 then
                            ...
                            HRLeavePeriods.Description := Format(intNewYear) + '-' + 'Leave Period';
                            HRLeavePeriods.Closed := false;
                            HRLeavePeriods.Insert;

                            //Mark HRLeaveLedgerEntries closed for the closed periods
                            HRLeavePeriods.Reset();
                            HRLeavePeriods.SetRange(Closed, true);
                            if HRLeavePeriods.FindSet() then begin
                                repeat
                                    HRLeaveLedger.Reset();
                                    // HRLeaveLedger.SetRange("Staff No.", Employee."No.");
                                    HRLeaveLedger.SetRange("Leave Period", Format(HRLeavePeriods.Code));
                                    HRLeaveLedger.SetRange("Leave Type", 'ANNUAL');
                                    HRLeaveLedger.ModifyAll(Closed, true);
                                until HRLeavePeriods.Next() = 0;
                            end;
                        ...
                        if Answer = true then begin
                            ...
                            repeat     //*********start HRLeaveTypes loop
                                ...
                                begin
                                    ...
                                    HRLeaveLedger.Init;
                                    HRLeaveLedger."Entry No." := LineNo + 1;
                                    HRLeaveLedger."Leave Period" := Format(intNewYear);
                                    HRLeaveLedger."Staff No." := HREmp."No.";
                                    HRLeaveLedger."Staff Name" := HREmp.FullName;
                                    HRLeaveLedger."Posting Date" := Today;
                                    HRLeaveLedger."Leave Entry Type" := HRLeaveLedger."leave entry type"::Reimbursement;
                                    ...
                                    HRLeaveLedger."Leave Posting Description" := 'Reimbursed Leave Days';
                                    HRLeaveLedger."User ID" := UserId;
                                    HRLeaveLedger."Journal Batch Name" := 'LEAVE';
                                    HRLeaveLedger."Leave Type" := HRLeaveTypes.Code;
                            ...
                            HRLeavePeriods.Ends := CalcDate('1Y', PeriodStartDate);
                            HRLeavePeriods."Closed By" := UserId;
                            HRLeavePeriods.Description := Format(intNewYear);
                            HRLeavePeriods.Closed := false;
                            HRLeavePeriods.Insert;
