...
Table 57006 "PV Lines"
{
    ...
    fields
    {
                    ...
                    "account type"::"G/L Account":
                        begin
                            ...
                            if GLAccount.FindFirst() then begin
                                Rec."Shortcut Dimension 1 Code" := GLAccount."Global Dimension 1 Code";
                                Rec."Shortcut Dimension 2 Code" := GLAccount."Global Dimension 2 Code";
                            end;
        ...
        field(14; "Applies to Doc. No"; Code[100])
        {
                        ...
                        "account type"::Vendor:
                            begin
                                ...
                                if (("Document Type" = "document type"::"Payment Voucher") or ("Document Type" = "document type"::"Petty Cash")) then begin
                                    ...
                                    PVHeader.SetRange(PVHeader."No.", No);
                                    if PVHeader.Find('-') then begin
                                        "Document Description" := PVHeader."Payment Narration";
                                        Description := PVHeader."Payment Narration";
                                    end;
                                end;
                            end;
                    end;
            ...
            begin
                ...
                if VendLentry.FindFirst() then begin
                    InvoiceDate := VendLentry."Document Date";
                end;

                // DEBUG MESSAGE 1 — Check Amount and tax rates at entry point of OnValidate
                // This tells us: what Amount is at this point, and whether tax rates are loaded
                // If Amount=0 AND rates=0, tax codes were never set — the Type/Account was not validated first
                // If Amount>0 AND rates=0, the tax codes are missing — check W/Tax Code, VAT Code fields on the line
                //  Message('DEBUG 1 - OnValidate Entry\Amount: %1\VAT Rate: %2\W/Tax Rate: %3\PAYE Rate: %4\Retention Rate: %5\PCBL Charge: %6',
                //  Amount, "VAT Rate", "W/Tax Rate", "PAYE Rate", "Retention Rate", "PCBL Charge");

                if Amount = 0 then begin
                    VendLentry.CalcFields("Remaining Amount");
                    Amount := Abs(VendLentry."Remaining Amount");
                    Validate(Amount);
                end;
                ...
                if PurchInvHeader.FindSet then begin
                    ...
                    "Invoice No." := PurchInvHeader."Vendor Invoice No.";
                ...
                // After PurchInvHeader block, populate Invoice Vendor No from VLE
                InvoiceVLE.Reset();
                InvoiceVLE.SetRange("Document No.", "Applies to Doc. No");
                InvoiceVLE.SetRange("Vendor No.", "Account No");
                if InvoiceVLE.FindFirst() then
                    "Invoice Vendor No." := InvoiceVLE."Vendor No.";
                Modify(true);

                // DEBUG MESSAGE 2 — Check state just before CalculateTax is called
                // This tells us: what Amount is NOW after the PurchInvHeader block
                // and whether Modify(true) inside the block above reset or disrupted any fields
                // If Amount changed from DEBUG 1 value, Validate(Discounted) or Modify is resetting it
                // Message('DEBUG 2 - Before CalculateTax\Amount: %1\Net Amount: %2\VAT Rate: %3\W/Tax Rate: %4\Discounted: %5\PCBL Charge: %6',
                //     Amount, "Net Amount", "VAT Rate", "W/Tax Rate", Discounted, "PCBL Charge");

                Validate(Discounted);

                if RecPayTypes.Get(Type) then begin
                    if (RecPayTypes."PPRA Chargeble" = true) and (RecPayTypes."PPRA Code" <> '') then begin
                        if TarrifCode.Get(RecPayTypes."PPRA Code") then begin
                            "PPRA Code" := RecPayTypes."PPRA Code";
                            "PPRA Percentage Amount" := TarrifCode.Percentage;
                        end;
                    end else begin
                        "PPRA Code" := '';
                        "PPRA Percentage Amount" := 0;
                        "PCBL Charge" := 0;
                    end;
                end else begin
                    "PPRA Code" := '';
                ...
                // DEBUG MESSAGE 3 — Check result AFTER CalculateTax runs
                // This tells us: whether CalculateTax actually ran and produced results
                // If Net Amount = 0 here but Amount > 0, CalculateTax ran but all rates were 0
                // If Net Amount = Amount here, no deductions were applied — rates all zero
                // If Net Amount is correctly calculated, the issue is that results are not being saved
                // Message('DEBUG 3 - After CalculateTax\Amount: %1\Net Amount: %2\VAT Amount: %3\W/Tax Amount: %4\PAYE Amount: %5\PCBL Charge: %6',
                //  Amount, "Net Amount", "VAT Amount", "W/Tax Amount", "PAYE Amount", "PCBL Charge");
            end;
        }
        ...
        field(21; "Net Amount"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                "Net Amount" := Amount - "W/Tax Amount" - "VAT Withheld Amount" - "Retention  Amount" - "Advance Recovery" - "PAYE Amount" - "Liquidated Damages" - "Obligation Total Amount" - "PCBL Charge";
                "Remaining Amount" := "Net Amount";
                "Amount Paid" := 0.0;
            end;
        }
        ...
        field(24; Type; Code[55])
        {
            ...
            begin
                ...
                if RecPayTypes.Find('-') then begin
                    // --- SETUP TAX CODES FROM PAYMENT TYPE ---
                    ...
                    if (RecPayTypes."PPRA Chargeble" = true) and (RecPayTypes."PPRA Code" <> '') then begin
                        "PPRA Code" := RecPayTypes."PPRA Code";
                        if TarrifCode.Get("PPRA Code") then
                            "PPRA Percentage Amount" := TarrifCode.Percentage;
                    end else begin
                        "PPRA Code" := '';
                        "PPRA Percentage Amount" := 0;
        ...
        field(38; "Payment Type"; Option)
        {
            ...
            FieldClass = FlowField;
            OptionCaption = 'Payment Voucher,Imprest,Staff Claim,Imprest Surrender,Petty Cash,Bank Transfer,Petty Cash Surrender,Surrender,Retention,Standing Imprest Surrender';
            OptionMembers = "Payment Voucher",Imprest,"Staff Claim","Imprest Surrender","Petty Cash","Bank Transfer","Petty Cash Surrender",Surrender,Retention,"Standing Imprest Surrender";
            Editable = false;
        }
        ...
        field(50012; "Remaining Amount"; Decimal)
        {
            trigger OnValidate()
            begin
                "Remaining Amount" := Amount - "Actual Amount Spent" - "Cash Receipt Amount";
        ...
        field(50019; "Document Type"; Option)
        {
            ...
            FieldClass = FlowField;
            OptionCaption = 'Payment Voucher,Petty Cash,Imprest,Imprest Surrender,Bank Transfer,Surrender,Standing Imprest';
            OptionMembers = "Payment Voucher","Petty Cash",Imprest,"Imprest Surrender","Bank Transfer",Surrender,"Standing Imprest";
            Editable = false;
        }
        ...
        field(50047; "Obligation VAT"; Decimal)
        {
            ...
            begin
                ...
                "Net Amount" := Amount - "W/Tax Amount" - "VAT Withheld Amount" - "Retention  Amount" - "Advance Recovery" - "PAYE Amount" - "Supervision Cost" - "Obligation Total Amount" - "PCBL Charge";
        ...
        field(50064; "S/Advance No"; code[20])
        {
            DataClassification = ToBeClassified;
        }
        ...
        field(50066; "Actual Amount Spent"; Decimal)
        {
            ...
            begin
                ...
                PHead.Reset;
                PHead.SetRange(PHead."No.", Rec.No);
                // ImprestHeader.SETRANGE(ImprestHeader."Account Type",ImprestHeader."Account Type"::Employee);
                if PHead.FindSet then begin
                    "Employee No" := PHead."Account No.";
                    // Message("Employee No");
                end;

            end;
        }
        field(50067; "Cash Receipt Amount"; Decimal)
        {

            trigger OnValidate()
            begin
                Validate("Actual Amount Spent");
            end;
        }
        field(50068; "Receipt No."; Code[20])
        {
            ...
            begin
                ...
                "Cash Receipt Amount" := ReceiptHeader.Amount;
                //"Fully Allocated" := true;
                Validate("Cash Receipt Amount");
            end;
        }
        field(50069; "Employee No"; Code[20])
        {

            TableRelation = Employee."No.";
        }
        field(50070; "PPRA Code"; Code[50])
        {
            Editable = true;
            TableRelation = "Tariff Codes1".Code where(Type = filter(PPRA));
            // trigger OnValidate()
            // var
            //     TarriffCodes: Record "Tariff Codes1";
            // begin
            //     if TarriffCodes.Get("PPRA Code") then begin
            //         Rec.Validate("PPRA Percentage Amount", TarriffCodes.Percentage);
            //     end;
        ...
        field(50071; "PPRA Percentage Amount"; Decimal)
        {
            Editable = false;
        }
        field(50072; "PCBL Charge"; Decimal)
        {
            Editable = false;
            trigger OnValidate()
            var
            ...
            begin
                if "PPRA Code" = '' then begin
                    TarriffCodes.Reset();
                    TarriffCodes.SetRange(Type, TarriffCodes.Type::PPRA);
                    if TarriffCodes.FindFirst() then begin
                        // "PPRA Code" := TarriffCodes.Code;
                    end;
                end;
                if "PCBL Charge" <> 0 then begin
                    "Net Amount" := "Net Amount" - "PCBL Charge";
                end;
            end;
        }
        field(50073; InvoiceDate; Date)
        {
            // TableRelation = "Vendor Ledger Entry"."Document Date" where("Document No." = field("Applies to Doc. No"), "Vendor No." = field("Account No"));

        }
        field(50074; "Invoice No."; Code[20])
        {
            Editable = true;
        }
        field(50075; "Vendor name"; Text[100])
        {
            Editable = false;
        }
        field(50076; "Invoice Vendor No."; Code[20])
        {
            DataClassification = ToBeClassified;
    ...
    var
        CalculationType: Option VAT,"W/Tax",Retention,PAYE,PPRA;
        TotalTax: Decimal;
    ...
    begin
        ...
        if Amount <> 0 then begin
            ...
            IF "PPRA Percentage Amount" <> 0 then begin
                "PCBL Charge" := TaxCalc.CalculateTax(Rec, CalculationType::PPRA);
                TotalTax := TotalTax + "PCBL Charge";
            end;
        end;
        ...
        if "VAT Six % Rate" <> 0 then begin
            ...
            "VAT Withheld Amount" := TaxCalc.RoundUpToAmount("VAT Withheld Amount");
        ...
        "Net Amount" := Amount - "W/Tax Amount" - "VAT Withheld Amount" - "Retention  Amount" - "Advance Recovery" - "PAYE Amount" - "Liquidated Damages" - "PCBL Charge";
        Validate("Net Amount");
    end;
