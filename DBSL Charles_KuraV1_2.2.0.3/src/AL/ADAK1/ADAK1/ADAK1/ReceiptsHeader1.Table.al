...
Table 56007 "Receipts Header1"
{
    ...
    fields
    {
        ...
        field(10; "On Behalf Of"; Code[100])
        {
        ...
        field(20; Status; Option)
        {
            ...
            //     myInt: Integer;
            // begin
            //      if (Rec.Status <> xRec.Status) and (Rec.Status in [Status::Open]) then
            //         Error('It seems you are trying to updated the status to Open.');      
            // end;
        }
        ...
        field(30; "Receipt Status"; Option)
        {
            OptionCaption = 'Open,Pending Approval,Pending Prepayment,Released,,,Closed';
            OptionMembers = Open,"Pending Approval","Pending Prepayment",Released,,,Closed;
            ...
            var
                myInt: Integer;
            begin
                if (xRec."Receipt Status" = "Receipt Status"::Released) and
                   (Rec."Receipt Status" <> xRec."Receipt Status") then begin
                    Error('You cannot change the status once the document is Released.');
                end;
            end;
        }
        field(31; "Total Net Amount"; Decimal)
        {
            CalcFormula = sum("Receipt Lines1"."Net Amount" where("Receipt No." = field("No.")));
            FieldClass = FlowField;
            Editable = false;
        }
        field(32; "Total VAT Amount"; Decimal)
        {
            CalcFormula = sum("Receipt Lines1"."VAT Amount" where("Receipt No." = field("No.")));
            FieldClass = FlowField;
            Editable = false;
        }
        ...
        {
            DataClassification = ToBeClassified;
            OptionMembers = "Payment Voucher","Petty Cash",Imprest,"Imprest Surrender","Bank Transfer",Surrender,"Staff Claims","Direct Income Voucher";
        }
        ...
        field(499; "Bank Name"; Code[100])
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(500; "No. Printed"; Integer)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
    ...
    begin
        ...
        //     "Document No" := 'ADAK' + '/' + ResponsibilityCenter.Code + '/' + CopyStr("No.", 9) + '/' + "Current Budget";
    end;
            ...
            repeat
                ...
                if PurchLine."Dimension Set ID" <> NewDimSetID then begin
                    ...
                    DimMgt.UpdateGlobalDimFromDimSetID(
                      NewDimSetID, PurchLine."Shortcut Dimension 1 Code", PurchLine."Shortcut Dimension 2 Code");
                    PurchLine.Modify;
                end;
