...
Table 69205 "HR Leave Applications"
{
    ...
    fields
    {
        ...
        field(3; "Leave Type"; Code[30])
        {
            ...
            begin
                ...
                */
                ...
                // Check for existing TERMINAL leave applications
                CountLeaves := 0;
                if "Leave Type" = 'TERMINAL' then begin
                    objLeaveApps.Reset;
                    objLeaveApps.SetRange(objLeaveApps."Employee No", "Employee No");
                    objLeaveApps.SetRange(objLeaveApps."Leave Type", "Leave Type");
                    objLeaveApps.SetFilter(objLeaveApps."Application Code", '<>%1', "Application Code");
                    if objLeaveApps.FindSet then
                        repeat
                            if Date2dmy(objLeaveApps."Application Date", 3) = YearofApplication then
                                CountLeaves := CountLeaves + 1;
                    ...
                    if CountLeaves >= 1 then
                        Error('You have already applied for Terminal Leave this year. Terminal leave can only be applied once.');
                end;
        ...
        field(5; "Start Date"; Date)
        {
            ...
            begin
                ...
                */
                ...
                LeavePlanDetailedLines.SetRange("Employee No.", "Employee No");
                if LeavePlanDetailedLines.FindSet then begin
                    repeat
                        HRLeaveTypes.Reset();
                        HRLeaveTypes.SetRange(Code, "Leave Type");
                        if HRLeaveTypes.Find('-') then begin
                            GracePeriodWithPlanner := Format(HRLeaveTypes."Grace Period(in Days )");
                            // if "Application Date" < (CalcDate('-' + GracePeriodWithPlanner + 'D', LeavePlanDetailedLines."Start Date")) then
                            //     Error('You cannot apply for leave since the application date is outside of the range of the Leave Planner and the Grace Period');
                            // if "Application Date" > LeavePlanDetailedLines."Start Date" then
                            //     Error('You cannot apply for leave since the start date is outside of the range of the Leave Planner');
                            // if ("Days Applied" > LeavePlanDetailedLines."Days Applied") then
                            //     Error('The Days Applied is not within the Leave Day Planner.');
                            // // if LeavePlanDetailedLines."Start Date" <> "Start Date" then
                            // //     // SetRange("Start Date",LeavePlanDetailedLines."Start Date")
                            // //     Error('The Start Date is not within the Leave Day Planner.');
                            // // if (LeavePlanDetailedLines."Start Date" = "Start Date") and (LeavePlanDetailedLines."Days Applied" <> "Days Applied") then
                            // //     Error('The Days Applied is not within the Leave Day Planner.');
                            // // if (LeavePlanDetailedLines."Start Date" = "Start Date") and (LeavePlanDetailedLines."Days Applied" = "Days Applied") and (LeavePlanDetailedLines."End Date" = "End Date") then
                            // //     Error('The End Date is not within the Leave Day Planner.');
                        end;
                    until LeavePlanDetailedLines.Next = 0;
                end
        ...
        field(3961; "Employee No"; Code[20])
        {
            ...
            begin
                if Employees.Get("Employee No") then begin
                    ...
                    "Section Code" := Employees."Section Code";
        ...
        field(70026; "Document Type"; Option)
        {
            OptionMembers = "Leave Application","Additional Leave Days Memo";
            OptionCaption = 'Leave Application,Additional Leave Days Memo';
        }
        field(70027; "Additional Leave Days"; Integer)
        {
            DataClassification = ToBeClassified;
        }
        field(70028; "Section Code"; Code[50])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Responsibility Center" where("Operating Unit Type" = const(Section), "Direct Reports To" = field("Department Code"));
        }
        field(70029; "Section Name"; Code[100])
        {
            //CalcFormula = lookup("Dimension Value".Name where(Code = field("Shortcut Dimension 1 Code")));
        ...
        field(70030; Resumed; Boolean)
        {
Editable=true;
        }
        field(70031; Recalled; Boolean)
        {
            CalcFormula = exist("Leave Recall" where("Application No" = field("Application Code"), "Employee No" = field("Employee No"), Posted = const(true)));
            FieldClass = FlowField;
        }
    ...
    begin
        ...
        if "Application Code" = '' then begin
            ...
            rec."Application Code" := NoSeriesMgt.GetNextNo(HRSetup."Leave Application Nos.", WorkDate(), true);
        end;
        ...
        if UserSetup."Portal User" = false then begin
            ...
            if HREmp.Find('-') then begin
                ...
                "Section Code" := HREmp."Section Code";
    ...
    var
        ...
        TEXT004: Label 'You can not apply for more than two Terminal Leaves in a Year';
    ...
    begin
        ...
        TestField("Additional Leave Days");
        ...
        if HRSetup.Find('-') then begin
            ...
            LeaveGjline.Description := "Employee Name" + Reason;
            ...
            LeaveGjline."No. of Days" := "Additional Leave Days";
