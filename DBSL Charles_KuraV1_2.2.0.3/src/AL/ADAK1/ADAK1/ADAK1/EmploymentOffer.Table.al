...
Table 69728 "Employment Offer"
{
    ...
    fields
    {
        field(1; "Offer ID"; Code[30])
        {
            ...
            trigger OnValidate()
            begin
                if Rec."Offer ID" <> xRec."Offer ID" then begin
                    HRSetup.Get;
                    NoSeriesMgt.TestManual(HRSetup."Employment Offer Nos.");
                    "No. Series" := '';
                end;
            end;
        }
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = "Candidate Interview Line"."Application No."
        where("Vacancy ID" = field("Vacancy ID"),
              "Interview Panel Outcome" = const(Succesful));
            trigger OnValidate()
            begin
        ...
        //     //TableRelation = "Job Applications" where(Shortlist = const(true));
        ...
        field(30; "Vacancy ID"; Code[50])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Recruitment Requisition Header"."Document No."
                    where("Document Type" = const("Job Vacancy"));
    ...
    begin
        if "Offer ID" <> '' then
            exit;
        HRSetup.Get();
        HRSetup.TestField("Employment Offer Nos.");
        "Offer ID" := NoSeriesMgt.GetNextNo(HRSetup."Employment Offer Nos.", WorkDate(), true);
    end;
