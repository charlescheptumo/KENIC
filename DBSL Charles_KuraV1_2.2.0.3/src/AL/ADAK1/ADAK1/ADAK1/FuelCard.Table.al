...
Table 59013 "Fuel Card"
{
    ...
    fields
    {
        ...
        {
            CalcFormula = sum("Fuel Card Entries"."Amount in Shillings" where(RegNo = field("Registration No.")));
            FieldClass = FlowField;
            Editable = false;
        }
        ...
        {
            CalcFormula = sum("Vehicle Fuel Card Lines"."Amount in Shillings" where("No." = field(No)));
            FieldClass = FlowField;
            Editable = false;
        }
        ...
        field(32; check2; Decimal)
        {
            CalcFormula = sum("Vehicle Fuel Card Lines"."Amount in Litres" where("No." = field(No)));
            FieldClass = FlowField;
            Editable = false;
        }
        field(33; "Total Topups"; Decimal)
        {
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = Sum("Fuel Topup Request".Amount where("Card No." = field(No), Status = filter(Approved)));
        }
        field(34; "Total Balance1"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = Sum("Fuel TopUp Entries".Amount where("Card No" = field(No)));
        }
    }
    ...
    begin
        if No = '' then begin
            ...
            FltMgtSetup.TestField(FltMgtSetup."Fuel Card No");
             rec."No" := NoSeriesMgt.GetNextNo(FltMgtSetup."Fuel Card No", WorkDate(), true);
        end;
    end;
