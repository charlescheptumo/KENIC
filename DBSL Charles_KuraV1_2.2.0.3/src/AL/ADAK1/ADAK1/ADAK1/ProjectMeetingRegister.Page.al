...
Page 72254 "Project Meeting Register"
{
    PageType = Card;
    SourceTable = "Project Meeting Register";
    Caption = 'Project Meeting Minutes';
    PromotedActionCategories = 'New,Process,Report,Request Approval,Print,Minutes,Navigate,Generate Payment Certificate,Generate Invoice';
    ApplicationArea = All;
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Next Meeting Date"; Rec."Next Meeting Date")
                {
                    ApplicationArea = Basic;
                    Visible = false;
                    ToolTip = 'Specifies the value of the Next Meeting Date field.';
                }
            ...
            part(Control22; "Project Meeting Attendance")
            {
                SubPageLink = "Meeting ID" = field("Meeting ID");
            }
        }
        area(FactBoxes)
        {
            part(Control1906354007; "Doc. Attachment List Factbox")
            {
                ...
                SubPageLink = "Table ID" = const(72167),
                              "No." = field("Meeting ID");
    ...
    actions
    {
        area(navigation)
        {
            action("Project Meeting Agenda")
            {
                ApplicationArea = Basic;
                Caption = ' Project Meeting Agenda';
                Ellipsis = true;
                ...
                RunObject = Page "Project Meeting Agenda";
                RunPageLink = "Meeting ID" = field("Meeting ID");
                ToolTip = 'Executes the  Project Meeting Agenda action.';
            }
            ...
            action("Print Minutes")
            {
                ...
                begin
                    Rec.Reset;
                    Rec.SetRange("Meeting ID", Rec."Meeting ID");
                    Report.Run(Report::"Project Meeting Minutes", true, true, Rec);
                end;
            }
            action("Attach Docs")
            {
                ...
                trigger OnAction()
                begin
                    DMSManagement.UploadAIEAttchmentDocuments(Rec."Meeting ID", 'Project Meeting Register', Rec.RecordId, Rec."Region ID");
                    Rec.Modify();
                end;
            }
    ...
    var
        ...
        DocumentType: Option " ","Commenncement Order","Communication Plan","Staff Appointments","Internal Staff Disengement","Staff Disengagement","Mobilization Checklist","Mobilization Checklist Status Report","Professional Indemnity","Work Execution Plan","HSE Plan",RMP,QMP,"Contract Supervision Requirement","Site Agent Nomination","Contractor Key Personnel Appointment","Equipment Delivery To Site","Insurance of Works","Advance Payment Guarantee","Performance Guarantee","Equipment Tracking","Meeting Register";
