...
/// </summary>
TableExtension 50028 "Resource Ext" extends Resource
{
    fields
    {
        ...
        */
        ...
        field(65011; "Resource Type"; Option)
        {
            ...
            OptionCaption = ' ,Physician,Committee Member';
            OptionMembers = " ",Physician,"Committee Member";
        }
        ...
        field(72011; "Application Area"; Option)
        {
            DataClassification = ToBeClassified;
            OptionCaption = ' ,SREQ,Licensing,Collaboration';
            OptionMembers = " ",SREQ,Licensing,Collaboration;
        }
        field(72012; "Resource Capacity"; Integer)
        {
            DataClassification = ToBeClassified;
        }
        field(72013; "Capacity/Quantity"; Boolean)
        {
        ...
        field(72014; "Availability Status"; Option)
        {
            OptionMembers = ,Occupied,Free,Reserved;
            OptionCaption = ',Occupied,Free,Reserved';
        }
        field(72015; Locked; Boolean)
        {
            DataClassification = ToBeClassified;
        }

        field(72016; "License Number"; Code[2048])
        {
            DataClassification = ToBeClassified;
        }
        field(72017; "Licensing Body"; Code[50])
        {
            TableRelation = "Licensing Bodies"."Licensing Body";
        }
        field(72018; "Signature"; Media)
        // field(70119; "Signature"; Blob)
        {
            // ExtendedDatatype = ;
            // SubType = Bitmap;
        }
        field(72019; Title; Option)
        {
            DataClassification = ToBeClassified;
            OptionCaption = ' ,Prof,Doctor,Mr,Mrs,Miss,Ms';
            OptionMembers = " ",Prof,Doctor,Mr,Mrs,Miss,Ms;
        }
        field(72020; "Medical Specialty"; Code[250])
        {
            TableRelation = "Medical Specialties"."Specialty Code";
        }
        field(72021; "Practitioner Category"; Code[250])
        {
            TableRelation = "Medical Practioner Categories".Category;
        }
        field(72022; "First Name"; Text[50])
        {
            trigger OnValidate()
            begin
                IF Rec."Resource Type" = Rec."Resource Type"::Physician then begin
                    If (Rec."First name" <> '') and (Rec."Middle Name" <> '') and (Rec."Last Name" <> '') then
                        Rec.Validate(Name, Rec."First name" + ' ' + Rec."Middle Name" + ' ' + Rec."Last Name");
                    If (Rec."First name" <> ' ') and (Rec."Middle Name" = ' ') and (Rec."Last Name" = ' ') then
                        Rec.Validate(Name, Rec."First name");
                    If (Rec."First name" <> '') and (Rec."Middle Name" = '') and (Rec."Last Name" <> '') then
                        Rec.Validate(Name, Rec."First name" + ' ' + Rec."Last Name");
                    If (Rec."First name" <> '') and (Rec."Middle Name" <> '') and (Rec."Last Name" = '') then
                        Rec.Validate(Name, Rec."First name" + ' ' + Rec."Middle Name")
                    ;
                    if Rec.Title <> Rec.Title::" " then
                        Rec.Validate(Name, Format(Rec.Title) + ' ' + Rec.Name);
                end;
            end;
        }
        field(72023; "Middle Name"; Text[50])
        {
            trigger OnValidate()
            begin
                IF Rec."Resource Type" = Rec."Resource Type"::Physician then begin
        ...
        field(72025; Gender; Enum "Employee Gender")
        {
        }
        field(72028; "Phone No."; Code[20])
        {
    ...
    begin
        if "No." = '' then begin
            case "Resource Type" of
                Rec."Resource Type"::Physician:
                    begin
                        TUESetup.Get();
                        TUESetup.TestField("Physician Nos");
                        //TUESetup.TestField(MedicalPractitonerPostingGroup);
                             "No." := NoSeriesMgmt.GetNextNo(TUESetup."Physician Nos",WorkDate(),true);
                       // NoSeriesMgmt.InitSeries(TUESetup."Physician Nos", xRec."No. Series", 0D, "No.", "No. Series");
                        // "VAT Bus. Posting Group" := TUESetup."MedicalP VAT Posting Group";
                        // "Gen. Bus. Posting Group" := TUESetup.DefaultPractitionerGenBusPGrp;
                        // "Vendor Posting Group" := TUESetup.MedicalPractitonerPostingGroup;
                    end;
            end;
        end;
    end;
