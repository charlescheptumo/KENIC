...
Table 70101 "Procurement Plan Entry"
{
    ...
    fields
    {
        ...
        field(5702; "Budget Control Job Task No."; Code[20])
        {
            ...
            begin
                JobTask.Reset;
                JobTask.SetRange("Job Task No.", "Budget Control Job Task No.");
                JobTask.SetRange("Job No.", "Budget Control Job No");
                if JobTask.FindFirst() then begin
                    JobTask.CALCFIELDS("Schedule (Total Cost)");
                    //   "Total Budgeted Amount":=JobTask."Schedule (Total Cost)";
                    "Estimated Cost of Project" := JobTask."Schedule (Total Cost)";
                end;
            end;
        }
        ...
        field(70076; "Accumulated Total Budget"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = Sum("Procurement Plan Entry"."Line Budget Cost" where("Procurement Plan ID" = field("Procurement Plan ID"),"Planning Category"=field("Planning Category")));
