#pragma warning disable AA0005, AA0008, AA0018, AA0021, AA0072, AA0137, AA0201, AA0206, AA0218, AA0228, AL0254, AL0424,AL0432, AW0006 // ForNAV settings
Table 57000 "payments"
{
    ...
    fields
    {
        field(1; "No."; Code[20])
        {
            ...
            begin
                ...
                if "Payment Type" = "payment type"::"Salary Advance Memo" then begin
                    if "No." <> xRec."No." then
                        NoSeriesMgt.TestManual(CashMgt."Salary Advance Memo No");
                end;
            end;
        }
        field(2; Date; Date)
        ...
        field(4; "Pay Mode"; Code[20])
        {
            ...
            begin
                CashMgt.get;
                CashMgt.TeStfield("Cheque Nos");
                if "Pay Mode" = 'CHEQUE' then begin
                    if "Cheque No" = '' then begin
                        "Cheque No" := NoSeriesMgt.GetNextNo(CashMgt."Cheque Nos", WorkDate(), true);
                    end;
        ...
        field(14; "Shortcut Dimension 1 Code"; Code[50])
        {
            ...
            begin
                ...
                if PVLines.FindSet() then begin
                    repeat
                        PVLines.Validate("Shortcut Dimension 1 Code", "Shortcut Dimension 1 Code");
                        PVLines.Modify(true);
        ...
        field(18; "Paying Bank Account"; Code[20])
        {
            ...
            if ("Payment Type" = const("Batch Imprest Voucher")) "Bank Account" where(Type = filter(Bank | "Petty Cash"))
            else
            ...
            if ("Payment Type" = const("Staff Claim")) "Bank Account" where(Type = filter("Petty Cash" | Bank));
        ...
        {
            OptionCaption = 'Payment Voucher,Imprest,Staff Claim,Imprest Surrender,Petty Cash,Bank Transfer,Petty Cash Surrender,Surrender,Standing Imprest,Salary Advance,Payment Voucher Reversal,Imprest Reversal,Staff Claim Reversal,Imprest Surrender Reversal,Petty Cash Reversal,Bank Transfer Reversal,Petty Cash Surrender Reversal,Surrender Reversal,Standing Imprest Reversal,Salary Advance Reversal,Standing Imprest Surrender,AIE,Retention,Salary Advance Memo,Batch Imprest Voucher';
            OptionMembers = "Payment Voucher",Imprest,"Staff Claim","Imprest Surrender","Petty Cash","Bank Transfer","Petty Cash Surrender",Surrender,"Standing Imprest","Salary Advance","Payment Voucher Reversal","Imprest Reversal","Staff Claim Reversal","Imprest Surrender Reversal","Petty Cash Reversal","Bank Transfer Reversal","Petty Cash Surrender Reversal","Surrender Reversal","Standing Imprest Reversal","Salary Advance Reversal","Standing Imprest Surrender",AIE,Retention,"Salary Advance Memo","Batch Imprest Voucher";
        }
            ...
            begin
                case "Account Type" of
                    ...
                    "account type"::Employee:
                        begin
                            //Check if Employee exists in the standing Imprest Limit
                            if "Payment Type" = "Payment Type"::"Standing Imprest" then begin
                                if not CashMgt.Get then
                                    Error(Text000);
                                CashMgt.TestField("Current Financial Year");
                                CurrentFinYear := CashMgt."Current Financial Year";
                                StandImpLimit.Reset();
                                StandImpLimit.SetRange("Financial Year", CurrentFinYear);
                                StandImpLimit.SetRange("Employee No", "Account No.");
                                if not StandImpLimit.FindFirst() then
                                    Error('Employee %1 does not have a Standing Imprest Limit set up for Financial Year %2.', "Account No.", CurrentFinYear);
                                "Standing Imprest Limit" := StandImpLimit."Limit";
                            end;
        ...
        {
            OptionMembers = "Payment Voucher","Petty Cash",Imprest,"Imprest Surrender","Bank Transfer",Surrender,"Staff Claims","Salary Advance","Standing Imprest","Standing Imprest Surrender","Salary Advance Memo","Batch Imprest";
        }
        ...
        field(50021; "Standing Imprest Issue Doc. No"; Code[20])
        {
            FieldClass = Normal;
            TableRelation = payments."No." where("Payment Type" = filter("Standing Imprest"),
                                                  Posted = const(true),
                                                  Surrendered = const(false),
                                                  "Account No." = field("Account No."),
                                                   Reversed = FILTER(false)); // Allow users to view reversed and unapplied imprest surrenders
            ...
            trigger OnValidate()
            var
                StandImpSurrLines: Record "PV Lines";
                StandSurrLines: Record "PV Lines";
            begin

                StandImpSurrLines.Reset;
                StandImpSurrLines.SetRange(No, "No.");
                StandImpSurrLines.DeleteAll;
                ImpHeader.Reset;
                ImpHeader.SetRange(ImpHeader."No.", "Standing Imprest Issue Doc. No");
                if ImpHeader.FindFirst then
                    if ImpHeader."Payment Type" = ImpHeader."payment type"::"Standing Imprest Surrender" then
                        Error('Imprest has already been Surrendered');
                if ImpHeader.Get("Standing Imprest Issue Doc. No") then begin
                    ImpHeader.CalcFields("Imprest Amount");
                    ImpHeader.CalcFields("Total Amount");
                    // Message('Total Amount from Standing Imprest Issue: %1', ImpHeader."Total Amount");
                    "Account Type" := ImpHeader."Account Type";
                    "Account No." := ImpHeader."Account No.";
                    Validate("Account No.");
                    "Payment Narration" := ImpHeader."Payment Narration";
                    ...
                    "Reference No." := ImpHeader."Reference No.";
                    Validate(Project);
                    JobNo := ImpHeader.JobNo;
                    Validate(JobNo);
                    "Job Task No." := ImpHeader."Job Task No";
                    "Standing Imprest Limit" := ImpHeader."Total Amount";
                    //Validate("Job Task No.");
                    "Funding Source" := ImpHeader."Shortcut Dimension 2 Code";
                    "Shortcut Dimension 1 Code" := ImpHeader."Shortcut Dimension 1 Code";

                    //Commented not to insert Lines

                    // StandSurrLines.Reset;
                    // StandSurrLines.SetRange(StandSurrLines.No, ImpHeader."No.");
                    // if StandSurrLines.FindFirst then begin
                    //     repeat
                    //         StandImpSurrLines.Init;
                    //         StandImpSurrLines.No := "No.";
                    //         StandImpSurrLines."Line No" := StandSurrLines."Line No";
                    //         StandImpSurrLines."Document Type" := StandSurrLines."Document Type";
                    //         StandImpSurrLines."Account Type" := StandSurrLines."Account Type";
                    //         StandImpSurrLines."Account No" := StandSurrLines."Account No";
                    //         StandImpSurrLines."Account Name" := StandSurrLines."Account Name";
                    //         StandImpSurrLines.Amount := StandSurrLines.Amount;
                    //         StandImpSurrLines.Description := StandSurrLines.Description;
                    //         StandImpSurrLines."Reference No" := ImpHeader."Reference No.";

                    //         if not StandImpSurrLines.Get(StandImpSurrLines.No, StandImpSurrLines."Line No") then
                    //             StandImpSurrLines.Insert;

                    //     until StandSurrLines.Next = 0;
                    // end;
                end;
            end;
        }
        ...
        field(57010; "Vendor Type"; Code[55])
        {
        ...
        {
            TableRelation = Employee."Basic Pay" where("No." = field("Account No."));
        }
        ...
        field(69043; "Recovery Start Month"; Date)
        {
        ...
        field(70031; "Employee Mobile No"; Code[120])
        {
            DataClassification = ToBeClassified;
            //Removed for manual entry
        ...
        field(70071; "Payroll Deduction Code"; code[20])
        {
            DataClassification = ToBeClassified;
            TableRelation = DeductionsX;
        }
        field(70072; "Recovery From"; code[20])
        {
            DataClassification = ToBeClassified;
        }
        field(70073; "File No."; code[20])
        {
            DataClassification = ToBeClassified;
        }
        field(70074; "Salary Type"; Enum "Salary Type")
        {

        }
        field(70075; "Salary Advance Memo No"; Code[250])
        {
            // TableRelation = "Cash Management Setup"."Salary Advance Memo No";
            // TableRelation = "General Correspondence Header"."Document No" where(Status = const(Approved));
            TableRelation = "General Correspondence Header"."Document No" where("Sender No" = field("Account No."));
            ...
            // begin
            //     Payments1.Reset();
            //     Payments1.SetRange(Payments1."Salary Advance Memo No", "Salary Advance Memo No");
            //     if Payments1.Find('-') then begin
            //         "Salary Type" := Payments1."Salary Type";
            //         "Account Type" := Payments1."Account Type";
            //         "Account No." := Payments1."Account No.";
            //         "Account Name" := Payments1."Account Name";
            //         "Date of Joining" := Payments1."Date of Joining";
            //         "Shortcut Dimension 1 Code" := Payments1."Shortcut Dimension 1 Code";
            //         Validate("Shortcut Dimension 1 Code");
            //         "Department Name" := Payments1."Department Name";
            //         "Shortcut Dimension 2 Code" := Payments1."Shortcut Dimension 2 Code";
            //         Validate("Shortcut Dimension 2 Code");
            //         "Pay Mode" := Payments1."Pay Mode";
            //         "Project Name" := Payments1."Project Name";
            //         "Salary Advance" := Payments1."Salary Advance";
            //         "No of months deducted" := Payments1."No of months deducted";
            //         "Monthly Installment" := Payments1."Monthly Installment";
            //         Purpose := Payments1.Purpose;
            //         "Recovery Start Month" := Payments1."Recovery Start Month";
            //         "Payroll Deduction Code" := Payments1."Payroll Deduction Code";
            //         "Recovery From" := Payments1."Recovery From";
            //     end;
            // end;
        }
        field(70076; "Check Printed"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(70077; "Total Imprest Amount"; Decimal)
        {
            // FieldClass = FlowField;
            // CalcFormula = sum("Batch Imprest Voucher Lines"."Imprest Amount" where("No." = field("No.")));
            Editable = false;
        }
        field(70078; "Interim Standing Imprest"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(70079; "Standing Imprest Limit"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(70080; "Financial Year"; Code[20])
        {
            Caption = 'Financial Year';
            TableRelation = "Financial Year Code";
        }
        //Standing Imprest Fields
        field(70081; "Standing Imprest Amount"; Decimal)
        {
            CalcFormula = sum("PV Lines".Amount where(No = field("Reference No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(70082; "Amount Spent"; Decimal)
        {
            CalcFormula = sum("PV Lines"."Actual Amount Spent" where(No = field("No.")));
            FieldClass = FlowField;
            Editable = false;
        }
        field(70083; "Receipt Amount"; Decimal)
        {
            CalcFormula = sum("PV Lines"."Cash Receipt Amount" where(No = field("No.")));
            FieldClass = FlowField;
            Editable = false;
        }
        field(70084; "Imprest Remaining Amount"; Decimal)
        {
            CalcFormula = sum("PV Lines"."Remaining Amount" where(No = field("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(70085; "Reversed By"; Code[50])
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(70086; "Date Reversed"; Date)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(70087; "Vendor No"; Code[100])
        {
            DataClassification = ToBeClassified;

        }
        field(57064; "Standing Vendor Check No."; Code[20])
        {
            Caption = 'Vendor No.';
            FieldClass = FlowField;
            CalcFormula = lookup("Cash Management Setup"."Standing Vendor Check No");
            Editable = false;
        }
        field(57065; "Payee Name"; Text[150])
        {
            Caption = 'Payee';
            FieldClass = FlowField;
        ...
        field(57066; "Batch Total Amount"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("Batch Imprest Voucher Lines"."Imprest Amount" where("No." = field("No.")));
            Editable = false;
        }
        field(57067; Description; Text[300])
        {
            Editable = false;
            FieldClass = FlowField;
        ...
        field(57068; "PCBL Charge"; Decimal)
        {
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = sum("PV Lines"."PCBL Charge" where(No = field("No.")));
        }
        field(57069; "Destination Narration"; Text[250])
        {

        }
        field(57070; Subject; Code[250])
        {
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = lookup("Imprest Memo".Subject where("No." = field("Imprest Memo No")));
    ...
    begin
        ...
        if "Payment Type" = "payment type"::"Batch Imprest Voucher" then begin
            CashMgt.TestField(CashMgt."Batch Imprest Voucher Nos");
            if "No." = '' then
                "No." := NoSeriesMgt.GetNextNo(CashMgt."Batch Imprest Voucher Nos", WorkDate(), true);
        end;
        ...
        if "Payment Type" = "payment type"::"Salary Advance Memo" then begin
            CashMgt.TestField(CashMgt."Salary Advance Memo No");
            if "No." = '' then
                "No." := NoSeriesMgt.GetNextNo(CashMgt."Salary Advance Memo No", WorkDate(), true);
        end;
        ...
        if ("Payment Type" = "Payment Type"::"Payment Voucher") or ("Payment Type" = "Payment Type"::"Standing Imprest Surrender") then begin
            // Always take logged-in user
            "Created By" := UserId;
        end else begin
            if UserSetup."Portal User" = false then begin
                ...
                UserSetup.Reset();
                UserSetup.SetRange("Employee No.", "Account No.");
        ...
        // Always set Created By to the currently logged in user
        //"Created By" := UserId;
  ...
  DimMgt.EditDimensionSet(
    ...
    begin
        ...
        if GuiAllowed then begin
            if not Confirm(Text051, true) then
                exit;
        end;
        PurchLine.Reset;
