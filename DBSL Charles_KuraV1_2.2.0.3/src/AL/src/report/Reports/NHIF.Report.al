...
Report 69020 NHIF
{
    ...
    dataset
    {
        ...
        {
            // DataItemTableView = sorting("Employee No Sort Key", Type, Code, "Payroll Period", "Reference No") where(Type = const(Deduction), Code = filter('SHIF'));//D004
            DataItemTableView = sorting("Employee No Sort Key", Type, Code, "Payroll Period", "Reference No") where(Type = const(Deduction), Code = filter('D004'));//D004
    ...
    var
        ...
        MONTHLY_PAYROLL__BY_PRODUCT__RETURNS_TO_NHIFCaptionLbl: label 'MONTHLY PAYROLL (BY-PRODUCT) RETURNS TO SHIF';
