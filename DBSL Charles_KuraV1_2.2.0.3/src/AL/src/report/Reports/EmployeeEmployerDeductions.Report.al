...
#pragma warning disable AA0005, AA0008, AA0018, AA0021, AA0072, AA0137, AA0201, AA0206, AA0218, AA0228, AL0254, AL0424, AW0006 // ForNAV settings
{
    ...
    dataset
    {
        ...
        {
            //DataItemTableView = sorting("Employee No Sort Key") order(ascending) where(Type = const(Deduction));

            // DataItemTableView = sorting("Employee No Sort Key", Type, Code, "Payroll Period", "Reference No") order(ascending) where(Type = const(Deduction));//, Code = filter('D004'));
            ...
            RequestFilterFields = "Payroll Period", "Code", "Payroll Grouping", "Posting Group Filter";
            ...
            column(ABS__Employer_Amount____ABS_Amount_; ABS(PayrollRounding("Employer Amount")) + ABS(PayrollRounding(Amount)))
            {
            }
            column(EmployeeTotal; PayrollRounding(EmployeeTotal))
            {
            }
            column(EmployerTotal; PayrollRounding(EmployerTotal))
            {
            }
            ...
            column(Employee_No_Sort_Key; "Employee No Sort Key")
            {
            }
            ...
            begin
                ...
                EmployerTotal := EmployerTotal + Abs(PayrollRounding("Assignment Matrix-X"."Employer Amount"));
                EmployeeTotal := EmployeeTotal + Abs(PayrollRounding("Assignment Matrix-X".Amount));
                ...
                EmpTotal := ABS(PayrollRounding("Assignment Matrix-X"."Employer Amount")) + Abs(PayrollRounding("Assignment Matrix-X".Amount)) +
                    Voluntary;
                //SumTotal := SumTotal + Abs("Assignment Matrix-X"."Employer Amount") + Abs("Assignment Matrix-X".Amount) + Voluntary;
                SumTotal := SumTotal + Abs(PayrollRounding("Assignment Matrix-X"."Employer Amount")) + Abs(PayrollRounding("Assignment Matrix-X".Amount)) + Voluntary;
                Counter := Counter + 1;
    ...
    begin
        ...
        ExcelBuf.AddColumn(Abs(PayrollRounding("Assignment Matrix-X".Amount)) + Abs(PayrollRounding("Assignment Matrix-X"."Employer Amount")), false, '', false, false, false, '@', ExcelBuf."cell type"::Text);
    ...
    begin
        ...
        ExcelBuf.AddColumn(EmployeeTotal + EmployerTotal, false, '', false, false, false, '', ExcelBuf."cell type"::Number);
