...
Report 69031 "NSSF Reporting"
{
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            column(TierIEmployeeAmount; TierIEmployeeAmount)
            {
            }
            column(TierIEmployerAmount; TierIEmployerAmount)
            {
            }
            column(TierIIEmployeeAmount; TierIIEmployeeAmount)
            {
            }
            column(TierIIEmployerAmount; TierIIEmployerAmount)
            {
            }
            ...
            begin
                ...
                //     repeat
                //         GrossAmount += AssignMatrix.Amount;
                //     until AssignMatrix.Next() = 0;
                GrossAmount := 0;
                AssignMatrix.Reset();
                AssignMatrix.SetRange(Type, AssignMatrix.Type::Payment);
                AssignMatrix.SetRange(Code, 'BS');
                AssignMatrix.SetRange("Employee No", "No.");
                AssignMatrix.SetRange("Payroll Period", PaysPeriod);
                ...
                VoluntaryAmount := 0;
                TotalAmount := 0;
                TierIEmployeeAmount := 0;
                TierIEmployerAmount := 0;
                TierIIEmployeeAmount := 0;
                TierIIEmployerAmount := 0;
                HrSetup.Get();
                AssignMatrix.Reset();
                AssignMatrix.SetRange(Type, AssignMatrix.Type::Deduction);
                AssignMatrix.SetRange(Code, HrSetup."NSSF Tier I Code");
                AssignMatrix.SetRange("Employee No", "No.");
                AssignMatrix.SetRange("Payroll Period", PaysPeriod);
                if AssignMatrix.FindFirst() then begin
                    TierIEmployeeAmount := Abs(AssignMatrix.Amount);
                    TierIEmployerAmount := Abs(AssignMatrix."Employer Amount");
                end;
                AssignMatrix.Reset();
                AssignMatrix.SetRange(Type, AssignMatrix.Type::Deduction);
                AssignMatrix.SetRange(Code, HrSetup."NSSF Tier II Code");
                AssignMatrix.SetRange("Employee No", "No.");
                AssignMatrix.SetRange("Payroll Period", PaysPeriod);
                if AssignMatrix.FindFirst() then begin
                    TierIIEmployeeAmount := Abs(AssignMatrix.Amount);
                    TierIIEmployerAmount := Abs(AssignMatrix."Employer Amount");
                end;
                EmployeeAmount := TierIEmployeeAmount + TierIIEmployeeAmount;
                EmployerAmount := TierIEmployerAmount + TierIIEmployerAmount;
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Payroll Period"; PaysPeriod)
                    {
                        ...
                        ToolTip = 'Specifies the value of the PaysPeriod field.';
                    }
                }
            }
        }
    ...
    var
        ...
        PaysPeriod: Date;
        GrossAmount: Decimal;
        TierIEmployeeAmount: Decimal;
        TierIEmployerAmount: Decimal;
        TierIIEmployeeAmount: Decimal;
        TierIIEmployerAmount: Decimal;
