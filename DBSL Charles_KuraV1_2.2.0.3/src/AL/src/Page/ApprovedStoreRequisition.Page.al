                        ...
                        if Confirm('Are you sure you would like to issue this requisition?', false) = true then begin
                            ...
                            if Rec."Requisition Type" = Rec."requisition type"::Project then begin
                                ...
                                if RequisitionLine.Find('-') then begin
                                    ...
                                    Rec."Posted By" := CopyStr(UserId, 1, 50); // *** ADDED Posted By for Project path ***
                                    Rec.Modify;
                                ...
                                if Rec."Shortcut Dimension 1 Code" = '' then
                                    Error('Cost Center must be specified before posting.');
                                //if
                                ItemGnl.Reset;
                                ...
                                if RequisitionLine.Find('-') then begin
                                    repeat
                                        ...
                                        ItemGnl."Shortcut Dimension 1 Code" := Rec."Shortcut Dimension 1 Code";
                                        //MESSAGE('%1',RequisitionLine."Dimension Set ID");
                                    ...
                                    // *** DEBUG 1: right after journal posts, before any Modify ***
                                    // DebugUserID := UserId;
                                    // DebugPostedBy := Rec."Posted By";
                                    // Message('DEBUG1: UserID=[%1] RecNo=[%2] PostedByBefore=[%3]', DebugUserID, Rec."No.", DebugPostedBy);

                                    Rec."Fully Issued" := true;
                                    Rec."Posting Date" := Today;
                                    Rec."Posted By" := CopyStr(UserId, 1, 50);
                                    Rec.Modify;
                            ...
                            */
                            ...
                            Rec."Taken By" := '';
                            Rec."Posted By" := CopyStr(UserId, 1, 50); // *** KEEP Posted By when clearing Taken By ***
                            Rec.Modify;
                            ...
                            if RequisitionLine.FindFirst() then begin
                                ...
                                Rec."Posting Date" := Today;
                                Rec."Posted By" := CopyStr(UserId, 1, 50); // *** KEEP Posted By here too ***

                                // *** DEBUG 2: just before final Modify ***
                                // DebugPostedBy := Rec."Posted By";
                                // Message('DEBUG2: Final block UserID=[%1] PostedBy=[%2]', UserId, DebugPostedBy);
