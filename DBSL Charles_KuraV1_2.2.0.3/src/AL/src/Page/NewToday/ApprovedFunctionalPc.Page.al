...
Page 80502 "Approved Functional Pc"
{
    Caption = 'Approved Functional Pc';
    DeleteAllowed = false;
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                {
                    ApplicationArea = Basic;
                    ToolTip = 'Specifies the value of the Organiztional PC field.';
                }
            }
            part(Control3; "Board Activities")
            {
                SubPageLink = "AWP No" = field(No), "Primary Directorate" = field("Primary Directorate"),
    ...
    actions
    {
        area(creation)
        {
            action("Update Annual Workplan")
            {
                ...
                ToolTip = 'Executes the Consolidate to Organizational PC action.';

                trigger OnAction()
                begin
                    //Post to Annual Workplan.
            ...
            group("Request Approval")
            {
                ...
                action(SendApprovalRequest)
                {
                    ...
                    begin
                        ...
                        Rec.CalcFields("Total Assigned PC Weight(%)");
                        if Rec."Total Assigned PC Weight(%)" <> 100 then
