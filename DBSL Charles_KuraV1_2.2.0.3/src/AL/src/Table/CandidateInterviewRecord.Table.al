...
Table 69716 "Candidate Interview Record"
{
    ...
    fields
    {
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = "Candidate Interview Line"."Application No." where("Document No." = field("Interview Invitation No."),
                                                                                "Document Type" = filter("Interview Invitation"));
        ...
        field(18; "Panel Member No."; Code[30])
        {
            ...
            trigger OnValidate()
            var
                ComAppMember: Record "Commitee Appointed Member";
            begin
                ComAppMember.Reset();
                ComAppMember.SetRange("Member No.", "Panel Member No.");
                if ComAppMember.Find('-') then begin
                    Member := ComAppMember."Member Name";
                end
                // Resource.Reset;
        ...
        {
            CalcFormula = average("Candidate Interview Question"."Weighted Score %" where("Document No." = field("Document No.")));
            Editable = false;
    ...
    begin
        ...
        HumanResourcesSetup.TestField("Candidate Interview Nos");
            rec."Document No." := NoSeriesMgt.GetNextNo(HumanResourcesSetup."Candidate Interview Nos", WorkDate(), true);
        //"Created By":=USERID;
