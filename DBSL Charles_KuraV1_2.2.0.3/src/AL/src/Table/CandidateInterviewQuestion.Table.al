...
Table 69718 "Candidate Interview Question"
{
    ...
    fields
    {
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = "Hr Screening Template Line"."Question Id" where("Template ID" = field("Interview Scoresheet Template"), "Question Category" = field("Question Category"));

            trigger OnValidate()
            var
                HRScreen: Record "Hr Screening Template Line";
            begin
                HRScreen.Reset();
                HRScreen.SetRange("Question Id", Rec."Question Id");
                if HRScreen.Find('-') then begin
                    Description := HRScreen.Description;
                    Weight := HRScreen.Weight;
                end
            end;
        }
