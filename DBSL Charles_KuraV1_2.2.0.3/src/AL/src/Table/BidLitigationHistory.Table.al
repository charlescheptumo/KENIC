...
Table 70166 "Bid Litigation History"
{
    ...
    fieldgroups
    {
        // fieldgroup(DropDown;"Document Type","No.",Field5050,"Dispute Amount LCY",Field91,Year)
        // {
        // }
        // fieldgroup(Brick;"Document Type","No.",Field5050,"Dispute Amount LCY",Year,Field140)
        // {
        // }
    }
    trigger OnInsert()
    var
        BidLitigationHistory: Record "Bid Litigation History";
    begin
        BidLitigationHistory.Reset();
        BidLitigationHistory.SetRange("Document Type", Rec."Document Type");
        BidLitigationHistory.SetRange("No.", Rec."No.");
        if BidLitigationHistory.FindLast() then
            Rec."Entry No." := BidLitigationHistory."Entry No." + 1
        else
            Rec."Entry No." := 1;
    end;
