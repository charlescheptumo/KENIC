    ...
    begin
        ...
        if EmpRec.Find('-') then begin
            if EmpRec."Pays tax?" = true then begin
                ...
                TaxableAmount := EmpRec."Taxable Allowance" - Abs(EmpRec."Tax Deductible Amount");
                ...
                //KICD Non Cash Benefit
                Assignmatrix.Reset;
                Assignmatrix.SetRange(Assignmatrix.Code, 'NON CASH');
                Assignmatrix.SetRange(Assignmatrix.Type, Assignmatrix.Type::payment);
                Assignmatrix.SetRange(Assignmatrix."Employee No", EmployeeNo);
                Assignmatrix.SetRange(Assignmatrix."Payroll Period", DateSpecified);
                if Assignmatrix.findfirst then begin
                    TaxableAmount := TaxableAmount + (Assignmatrix.Amount - 3000);
                    //Message('%1::%2', TaxableAmount, Assignmatrix.Amount);
                end;
                //Non Cash Benefit end;
                ...
                if EmpRec.Get(EmployeeNo) then
                    if EmpRec.Disabled = true then begin
                ...
                //Message('%1 Before Relief',FinalTax);
                ...
                //Shif Relief--Added By Nehemia
                ShifRelef := 0;
                Assignmatrix.Reset;
                Assignmatrix.SetRange(Assignmatrix."Payroll Period", DateSpecified);
                Assignmatrix.SetRange(Type, Assignmatrix.Type::Payment);
                Assignmatrix.SetRange(Assignmatrix.Code, 'SHIFREL');
                Assignmatrix.SetRange(Assignmatrix."Employee No", EmployeeNo);
                if Assignmatrix.Find('-') then
                    ShifRelef := ShifRelef + Assignmatrix.Amount;
                //FinalTax := FinalTax - (PersonalRelief + InsuranceRelief);
                FinalTax := FinalTax - (InsuranceRelief) - ShifRelef;

                //Message('%1 Final Tax, %2 Personal Relief, %3 Insurance Relief', FinalTax,PersonalRelief,InsuranceRelief);
             ...
             EmpRec.Disabled = true then begin
                EmpRec.CalcFields(EmpRec."Taxable Allowance", "Tax Deductible Amount", "Relief Amount");
                TaxableAmount := EmpRec."Taxable Allowance";
                if (TaxableAmount > 150000) then
                    TaxableAmount := TaxableAmount - 150000
                else
                    TaxableAmount := 0;
    ...
    begin
        ...
        if EmpRec.Find('-') then begin
            if EmpRec."Pays tax?" = true then begin
                ...
                //FinalTax := FinalTax - (PersonalRelief + InsuranceRelief);
                FinalTax := FinalTax - (InsuranceRelief);
                //Kugun--add tax releif carry foward.
