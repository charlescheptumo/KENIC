...
Codeunit 72000 "RMS Management"
{
    ...
    var
        ...
        NSMgt: Codeunit "No. Series";
        ApproveOnlyOpenRequestsErr: Label 'You can only approve open approval requests.';

        ObjCompany: Record "Company Information";
        // SMTPSetup: Record 
        SenderEmail, SenderName, FileDirectory, Filename, SentMail, Header, Subject, Body, FilePath, URL2, EmpName, CurrUser : Text;
        GeneralCorrespondenceLine: Record "General Correspondence Line";
        CorrespondenceHeader: Record "General Correspondence Header";
        TempBlob, InputFile : Codeunit "Temp Blob";
        Ostream: OutStream;
        Istream, Istream1 : Instream;
        SMTPMessage: Codeunit "Email Message";
        RecordLink: Record "Record Link";
        FileManagement: Codeunit "File Management";
        InternalCorrespondApprovals: Record "General Correspond Approval";
        InternalCorrespondenceHeader: Record "General Correspondence Header";
        LastApprover, RequesterName, SenderMessage, CompanyDetails, LoginDetails, SupplierDetails, ProcNote, CustEmail, emailhdr, CCEmail : Text;
        SMTPMail: Codeunit Email;
        RecRef: RecordRef;
        FRef: FieldRef;
        FileDoesNotExistErr: label 'The file does not exist!';
        UserSetup: Record "User Setup";
        Emp, Employee : Record Employee;
        IncomingCorrespondenceLine: Record "Incoming Correspondence Line";
        CurrentSequence, NextSequence : Integer;
...
#pragma warning restore AL0296
        ...
        if MeasurementSheetBillCode.Find('-') then begin
            ...
            if ObjMPHeader.FindSet then begin
                repeat
                    ...
                    MeasurementPaymentLine."Job Task No." := ObjJobPlanningLines."Job Task No.";
                    MeasurementPaymentLine."Line No." := ObjJobPlanningLines."Line No.";
                    MeasurementPaymentLine.Type := MeasurementPaymentLine.Type::" ";
                    MeasurementPaymentLine."No." := ObjJobPlanningLines."No.";
                    Message(MeasurementPaymentLine."No.");
                    MeasurementPaymentLine.Description := ObjJobPlanningLines.Description;
                    MeasurementPaymentLine."Unit of Measure Code" := ObjJobPlanningLines."Unit of Measure";
                    MeasurementPaymentLine.Validate("Unit of Measure Code");
                    MeasurementPaymentLine."Contract Quantity" := ObjJobPlanningLines."Unit Cost";
                    MeasurementPaymentLine."Bill Item Category Code" := ObjJobPlanningLines."Road Activity Categories";
                    MeasurementPaymentLine."Direct Unit Cost (LCY)" := ObjJobPlanningLines."Unit Cost";
                    MeasurementPaymentLine."Gen. Prod. Posting Group" := ObjJobPlanningLines."Gen. Prod. Posting Group";
                    MeasurementPaymentLine."Technology Type" := ObjJobPlanningLines."Road Technology";
    ...
    begin
        ...
        if ObjMPLine.Find('-') then
            repeat
                ...
                ObjMPEntry.Reset;
                ObjMPEntry.SetRange("Document No", ObjMPHeader."Document No.");
                if ObjMPEntry.FindLast then
                    ObjMPEntry."Entry No" := ObjMPEntry."Entry No" + 100;
                ObjMPEntry."Posting Date" := ObjMPHeader."Document Date";
    ...
    begin
        ...
        MeasurementPaymentEntry.SetRange("Posting Date", MeasurementPaymentHeader."From Date", MeasurementPaymentHeader."To Date");
        if MeasurementPaymentEntry.FindSet then
            repeat
                MeasurementPaymentLine.Init;
                MeasurementPaymentLine."Document No" := MeasurementPaymentHeader."Document No.";
                MeasurementPaymentLine."Document Type" := MeasurementPaymentHeader."Document Type";
                ...
                MeasurementPaymentLine.SetRange(MeasurementPaymentLine."Document No", MeasurementPaymentHeader."Document No.");
    ...
    begin
        ...
        if DetailedMulti_YearLine.FindSet then begin
            repeat
                ...
                NewRoadWorkProgramPackage."Package No." := DetailedMulti_YearLine."Package No.";
                NewRoadWorkProgramPackage."KeRRA Budget Code" := DetailedMulti_YearLine."KeRRA Budget Code";
                NewRoadWorkProgramPackage."Package Name" := DetailedMulti_YearLine."Package Name";
                NewRoadWorkProgramPackage."Road Class" := DetailedMulti_YearLine."Road Class";
                NewRoadWorkProgramPackage."Road Section No." := DetailedMulti_YearLine."Road Section No.";
                NewRoadWorkProgramPackage."Section Name" := DetailedMulti_YearLine."Section Name";
                NewRoadWorkProgramPackage."Road Works Category" := DetailedMulti_YearLine."Road Works Category";
                NewRoadWorkProgramPackage."Pavement Surface Type" := DetailedMulti_YearLine."Pavement Surface Type";
                NewRoadWorkProgramPackage."Funding Source ID" := DetailedMulti_YearLine."Funding Source ID";
                NewRoadWorkProgramPackage."Start Chainage" := DetailedMulti_YearLine."Start Chainage";
                NewRoadWorkProgramPackage."End Chainage" := DetailedMulti_YearLine."End Chainage";
                NewRoadWorkProgramPackage."Road Project Category" := DetailedMulti_YearLine."Road Project Category";
                DetailedMulti_YearLine.CalcFields("Current Budget Amount");
                NewRoadWorkProgramPackage."Total Budget Cost" := DetailedMulti_YearLine."Current Budget Amount";
                NewRoadWorkProgramPackage."County ID" := DetailedMulti_YearLine."County ID";
                NewRoadWorkProgramPackage."Region ID" := DetailedMulti_YearLine."Region ID";
                NewRoadWorkProgramPackage."Constituency ID" := DetailedMulti_YearLine."Constituency ID";
    ...
    begin
        ...
        if not Consolidated.FindFirst() then begin
            ...
            Consolidatedi."Document No" := NSeriesMgt.GetNextNo(RoadManagementSetup."RAM Workplan Nos(Towns)", WorkDate(), true);
            if Consolidatedi.Insert() then
                ConsolidatedDocNo := Consolidatedi."Document No"
            else
                error('Could not Insert a consolidated workpan');
    ...
    begin
        ...
        if RAMWorkplanLines.FindSet() then begin
            repeat
                if InsertToBudgetEntry(RAMWorkplanLines, FinancialYear) then begin
                    RAMWorkplanLines.Scheduled := true;
                    RAMWorkplanLines.modify();
                end;
            until RAMWorkplanLines.Next() = 0;
        end;
    end;

    Procedure InsertToBudgetEntry(RAMWorkplanLine: Record "RAM Workplan Line"; FinancialYear: Code[30]): Boolean
    var
    ...
    begin
        ...
        if WorksCategory.Get(RAMWorkplanLine."Works Type") then begin
            ...
            exit(false);
        GLBudgetEntry."Global Dimension 1 Code" := RAMWorkplanLine."Region Code";
        GLBudgetEntry."Global Dimension 2 Code" := RAMWorkplanLine."Global Dimension 2 Code";
        // RAMWorkplanLine.CalcFields("Total Incl VAT");
        if RoadManagementSetup.Get() then
            GLBudgetEntry."Shortcut Dimension 3 Code" := RoadManagementSetup."Default Dimension 3";
        GLBudgetEntry.Amount := RAMWorkplanLine."Total Incl VAT";
    ...
    // Correspondence
    procedure FnSendInternalCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    var
        VarVariant: Variant;
        CustomApprovals: Codeunit "Custom Approvals Codeunit";
    begin
        InternalCorrespondApprovals.RESET;
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
        IF InternalCorrespondApprovals.FINDSET THEN BEGIN
            REPEAT
                IF InternalCorrespondApprovals.Sequence = 1 THEN BEGIN
                    InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Open;
                    InternalCorrespondApprovals."Date Sent Approval" := CURRENTDATETIME;
                    InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;

                    //Set Current Approver
                    InternalCorrespondenceHeader."Current Approver" := InternalCorrespondApprovals."User ID";
                    //Set Current Approver

                    // FnNotifyInternalCorrespondenceUsers(InternalCorrespondApprovals);
                    if InternalCorrespondApprovals.Type = InternalCorrespondApprovals.Type::Internal then begin
                        fnnotifyInternalCorrespondenceUsersonLines(InternalCorrespondApprovals);
                    end else begin
                        fnnotifyExternalCorrespondenceUsersonLines(InternalCorrespondApprovals);
                    end;

                END
                ELSE BEGIN
                    InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Pending;
                    InternalCorrespondApprovals."Date Sent Approval" := CURRENTDATETIME;
                    InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                END;
                InternalCorrespondenceHeader.Modify();
                InternalCorrespondApprovals.MODIFY(TRUE);
    ...
    procedure FnNotifyInternalCorrespondenceUsers(InternalCorrespondApprovals: Record "General Correspond Approval")
    begin
        ObjCompany.GET;
        // SMTPSetup.GET;
        ...
        SentMail := InternalCorrespondApprovals."Employee Email";

        InternalCorrespondenceHeader.RESET;
        InternalCorrespondenceHeader.SETRANGE(InternalCorrespondenceHeader."Document No", InternalCorrespondApprovals."Document No");
        IF InternalCorrespondenceHeader.FINDSET THEN BEGIN
            IF SentMail <> '' THEN BEGIN
                Header := ObjCompany.Name + ' ' + 'APPROVAL REQUEST FOR GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(InternalCorrespondApprovals.Type)) + ' ' + UPPERCASE(FORMAT(InternalCorrespondApprovals."Document Type"));
                // SMTPMail.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
                Body += '<br><br>';
                Body += 'Dear ' + InternalCorrespondApprovals."Employee Name";
                Body += '<br><br>';
                Body += 'This is to notify you that an' + ' ' + (FORMAT(InternalCorrespondApprovals.Type)) + ' ' + (FORMAT(InternalCorrespondApprovals."Document Type")) + ' ' + 'has been sent to you for review and approval.';
                Body += '<br><br>';
                Body += 'Correspondence Ref No: ' + InternalCorrespondenceHeader."External Reference No" + ' ' + 'From the ' + InternalCorrespondenceHeader."Sender Directorate Name";
                Body += '<br><br>';
                Body += 'Correspondence Subject: ' + InternalCorrespondenceHeader."Communication Subject";
                Body += '<br><br>';
                Body += '<br><br>';
                Body += 'Please login to the system and facilitate ';
                Body += '<br>';
                Body += 'For any assistance please contact ICT Department';
                Body += '<br>';
                Body += ' Thanks & Regards';
                Body += '<br><br>';
                Body += ObjCompany.Name;
                Body += '<br><br>';
                Body += 'This is a system generated mail.Please do not respond';
                Body += '<br><br>';
                SMTPMessage.Create(SentMail, Header, Body, TRUE);
                SMTPMail.Send(SMTPMessage, "Email Scenario"::Default);
                MESSAGE('Notifications to %1 sent successfully', InternalCorrespondApprovals."Employee Name");
            END;
        END;
    end;

    procedure fnnotifyInternalCorrespondenceUsersonLines(InternalCorrespondApprovals: Record "General Correspond Approval")
    var
        GeneralCorrespondenceLine: Record "General Correspondence Line";
    begin
        ObjCompany.GET;
        ...
        SentMail := GeneralCorrespondenceLine."Recipient Email";

        InternalCorrespondenceHeader.RESET;
        InternalCorrespondenceHeader.SETRANGE(InternalCorrespondenceHeader."Document No", GeneralCorrespondenceLine."Document No");
        IF InternalCorrespondenceHeader.FINDSET THEN BEGIN
            GeneralCorrespondenceLine.Reset();
            GeneralCorrespondenceLine.SetRange(GeneralCorrespondenceLine."Document No", InternalCorrespondenceHeader."Document No");
            GeneralCorrespondenceLine.SetRange(GeneralCorrespondenceLine.Type, GeneralCorrespondenceLine.Type::Internal);
            if GeneralCorrespondenceLine.FindSet then begin
                repeat
                    IF SentMail <> '' THEN BEGIN
                        Header := ObjCompany.Name + ' ' + 'APPROVAL REQUEST FOR GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(GeneralCorrespondenceLine.Type)) + ' ' + UPPERCASE(FORMAT(GeneralCorrespondenceLine."Document Type"));
                        // SMTPMail.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
                        Body += '<br><br>';
                        Body += 'Dear ' + GeneralCorrespondenceLine."Recipient Name";
                        Body += '<br><br>';
                        Body += 'This is to notify you that an' + ' ' + (FORMAT(GeneralCorrespondenceLine.Type)) + ' ' + (FORMAT(GeneralCorrespondenceLine."Document Type")) + ' ' + 'has been sent to you for review and approval.';
                        Body += '<br><br>';
                        Body += 'Correspondence Ref No: ' + InternalCorrespondenceHeader."External Reference No" + ' ' + 'From the ' + InternalCorrespondenceHeader."Sender Directorate Name";
                        Body += '<br><br>';
                        Body += 'Correspondence Subject: ' + InternalCorrespondenceHeader."Communication Subject";
                        ...
                        SMTPMessage.Create(SentMail, Header, Body, TRUE);
                        SMTPMail.Send(SMTPMessage, "Email Scenario"::Default);
                        MESSAGE('Notifications to %1 sent successfully', GeneralCorrespondenceLine."Recipient Name");
                    END;
    ...
    procedure fnnotifyExternalCorrespondenceUsersonLines(InternalCorrespondApprovals: Record "General Correspond Approval")
    var
        GeneralCorrespondenceLine: Record "General Correspondence Line";
    begin
        ObjCompany.GET;
        ...
        IF InternalCorrespondenceHeader.FINDSET THEN BEGIN
            GeneralCorrespondenceLine.Reset();
            GeneralCorrespondenceLine.SetRange(GeneralCorrespondenceLine."Document No", InternalCorrespondenceHeader."Document No");
            GeneralCorrespondenceLine.SetRange(GeneralCorrespondenceLine.Type, GeneralCorrespondenceLine.Type::External);
            if GeneralCorrespondenceLine.FindSet then begin
                repeat
                    IF SentMail <> '' THEN BEGIN
                        Header := ObjCompany.Name + ' ' + 'APPROVAL REQUEST FOR GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(GeneralCorrespondenceLine.Type)) + ' ' + UPPERCASE(FORMAT(GeneralCorrespondenceLine."Document Type"));
    ...
    procedure FnForwardCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    var
        VarVariant: Variant;
        CustomApprovals: Codeunit "Custom Approvals Codeunit";
    begin
        ...
        IF InternalCorrespondApprovals.FINDLAST THEN BEGIN
            ...
            LastApprover := InternalCorrespondApprovals."User ID";

        END;
        //MESSAGE('%1',LastApprover);

        InternalCorrespondApprovals.RESET;
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
        IF InternalCorrespondApprovals.FINDSET THEN BEGIN
            IF LastApprover = CurrUser THEN BEGIN
                ERROR('You are the Final Approver. Please Use the Approve Button');
            END
            ELSE BEGIN
                CurrentSequence := InternalCorrespondApprovals.Sequence;
                REPEAT
                    InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Approved;
                    InternalCorrespondApprovals.Forwarded := TRUE;
                    InternalCorrespondApprovals.MODIFY(TRUE);
                UNTIL InternalCorrespondApprovals.NEXT = 0;
        ...
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Pending);
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Sequence, NextSequence);
        IF InternalCorrespondApprovals.FINDSET THEN BEGIN
            REPEAT
                //MESSAGE('%1',InternalCorrespondApprovals."Employee Name");
                InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Open;
                InternalCorrespondApprovals."Date Sent Approval" := CURRENTDATETIME;
                InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                FnNotifyInternalCorrespondenceUsers(InternalCorrespondApprovals);
                InternalCorrespondApprovals.MODIFY(TRUE);
                MESSAGE('Document Successfully forwarded to %1', InternalCorrespondApprovals."Employee Name");
            UNTIL InternalCorrespondApprovals.NEXT = 0;
        END;
    end;


    procedure FnCancelCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    var
        VarVariant: Variant;
        CustomApprovals: Codeunit "Custom Approvals Codeunit";
    begin
        ...
        IF InternalCorrespondApprovals.FINDSET THEN BEGIN
            REPEAT
                InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Cancelled;
                // InternalCorrespondApprovals.TestField(Status, InternalCorrespondApprovals.Status::Pending);
                InternalCorrespondApprovals.Forwarded := FALSE;
                InternalCorrespondApprovals."Date Sent Approval" := 0DT;
                //InternalCorrespondApprovals."Date Sent Approval":=CURRENTDATETIME;
                InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                InternalCorrespondApprovals.MODIFY(TRUE);
            UNTIL InternalCorrespondApprovals.NEXT = 0;
        END;
        InternalCorrespondenceHeader."Current Approver" := '';
        InternalCorrespondenceHeader.MODIFY(TRUE);
    end;

    procedure FnRejectCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        InternalCorrespondApprovals.RESET;
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
        ...
        IF InternalCorrespondApprovals.FINDSET THEN BEGIN
            REPEAT
                InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Rejected;
                InternalCorrespondApprovals.Forwarded := FALSE;
                InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                InternalCorrespondApprovals.MODIFY(TRUE);
                FnNotifyInitiatorInternalCorrespondenceRejection(InternalCorrespondenceHeader);
                FnRevertOnRejectionOfCorrespondenceApprovalRequest(InternalCorrespondenceHeader);
            UNTIL InternalCorrespondApprovals.NEXT = 0;
        END;
    end;

    procedure FnApproveCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        //CurrUser:=USERID;
        // IF InternalCorrespondenceHeader."Requires Authorization" = FALSE THEN BEGIN
        InternalCorrespondApprovals.RESET;
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
        // IF InternalCorrespondApprovals.FINDLAST THEN BEGIN
        IF InternalCorrespondApprovals.Find('-') THEN BEGIN
            CurrUser := InternalCorrespondApprovals."User ID";
            IF CurrUser = USERID THEN BEGIN
                InternalCorrespondApprovals.TESTFIELD(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
                InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Approved;
                InternalCorrespondApprovals.Forwarded := TRUE;
                InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                InternalCorrespondApprovals.MODIFY(TRUE);
                FnNotifyInitiatorInternalCorrespondenceApproval(InternalCorrespondenceHeader);
                FnNotifyRecipientsGeneralCorrespondenceOnApproval(InternalCorrespondenceHeader);
            END
            ELSE BEGIN
                ERROR('You are not the Signatory.Please review and forward the document');
            END;
        END;
        // END;
        // end else begin
    ...
    procedure FnNotifyInitiatorInternalCorrespondenceApproval(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        ObjCompany.GET;
        // SMTPSetup.GET;
        // SenderEmail := SMTPSetup."Email Sender Address";
        ...
        // UserSetup.RESET;
        // UserSetup.SETRANGE(UserSetup."User ID",InternalCorrespondenceHeader."Created By");
        // IF UserSetup.FINDSET THEN BEGIN
        Emp.RESET;
        Emp.SETRANGE(Emp."No.", InternalCorrespondenceHeader."Creator No");
        IF Emp.FINDFIRST THEN begin
            EmpName := Emp."First Name" + ' ' + Emp."Middle Name" + ' ' + Emp."Last Name";
            SentMail := Emp."E-Mail";
            if SentMail = '' then
                Error('No email on creator no %1', Emp."First Name");
        end;
        // if SentMail = '' then
        //     Error('No email on creator no');
        //END;
        //SentMail:=InternalCorrespondenceHeader."Sender Email";

        IF SentMail <> '' THEN BEGIN
            Header := ObjCompany.Name + ' ' + 'APPROVAL STATUS OF GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(InternalCorrespondenceHeader.Type)) + ' ' + UPPERCASE(FORMAT(InternalCorrespondenceHeader."Document Type"));
            Body += '<br><br>';
            Body += 'Dear ' + EmpName;
            Body += '<br><br>';
            Body += 'This is to notify you that the' + ' ' + (FORMAT(InternalCorrespondenceHeader.Type)) + ' ' + (FORMAT(InternalCorrespondenceHeader."Document Type")) + ' ' + 'you sent for approval has been approved.';
            Body += '<br><br>';
            Body += 'Correspondence Ref No: ' + InternalCorrespondenceHeader."External Reference No" + ' ' + 'System Ref No ' + InternalCorrespondenceHeader."Document No";
            Body += '<br><br>';
            Body += 'Correspondence Subject: ' + InternalCorrespondenceHeader."Communication Subject";
            Body += '<br><br>';
            Body += '<br><br>';
            Body += 'Please navigate to the Approved Correspondences page for further action ';
            Body += '<br>';
            Body += 'For any assistance please contact ICT Department';
            Body += '<br>';
            Body += ' Thanks & Regards';
            ...
            Body += 'This is a system generated mail.Please do not respond';
            Body += '<br><br>';
            // SMTPMessage.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
            SMTPMessage.Create(SentMail, Header, Body, TRUE);
            SMTPMail.Send(SMTPMessage, "Email Scenario"::Default);
            MESSAGE('Notifications to %1 sent successfully', EmpName);
        END;
    end;


    procedure FnNotifyInitiatorInternalCorrespondenceRejection(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        ObjCompany.GET;
        // SMTPSetup.GET;
        // SenderEmail := SMTPSetup."Email Sender Address";
        ...
        IF SentMail <> '' THEN BEGIN
            Header := ObjCompany.Name + ' ' + 'APPROVAL STATUS FOR GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(InternalCorrespondenceHeader.Type)) + ' ' + UPPERCASE(FORMAT(InternalCorrespondenceHeader."Document Type"));
            // SMTPMail.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
            Body += '<br><br>';
            Body += 'Dear ' + EmpName;
            Body += '<br><br>';
            Body += 'This is to notify you that the' + ' ' + (FORMAT(InternalCorrespondenceHeader.Type)) + ' ' + (FORMAT(InternalCorrespondenceHeader."Document Type")) + ' you sent for approval has been rejected with comments';
            Body += '<br><br>';
            Body += 'Rejected By' + ' ' + InternalCorrespondenceHeader."Rejected By";
            Body += '<br><br>';
            Body += 'Rejection Comments: "' + InternalCorrespondenceHeader."Rejection Comments" + '"';
            Body += '<br><br>';
            Body += 'Correspondence Ref No: ' + InternalCorrespondenceHeader."External Reference No" + ' ' + 'System Ref No ' + InternalCorrespondenceHeader."Document No";
            Body += '<br><br>';
            Body += 'Correspondence Subject: ' + InternalCorrespondenceHeader."Communication Subject";
            Body += '<br><br>';
            Body += '<br><br>';
            Body += 'Please navigate to the General Correspondences page for further action ';
            Body += '<br>';
            Body += 'For any assistance please contact ICT Department';
            Body += '<br>';
            Body += ' Thanks & Regards';
            ...
            SMTPMessage.Create(SentMail, Header, Body, TRUE);
            SMTPMail.Send(SMTPMessage, "Email Scenario"::Default);
            MESSAGE('Notifications to %1 sent successfully', InternalCorrespondenceHeader."Creator Name");
            //InternalCorrespondApprovals.Notified:=TRUE;
            //InternalCorrespondApprovals.MODIFY();
        END;
    end;

    procedure FnNotifyInitiatorInternalCorrespondenceForwarding(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        ObjCompany.GET;
        // SMTPSetup.GET;
        // SenderEmail := SMTPSetup."Email Sender Address";
        ...
        SentMail := InternalCorrespondenceHeader."Recipient Email";


        IF SentMail <> '' THEN BEGIN
            Header := ObjCompany.Name + ' ' + 'APPROVAL REQUEST FOR INTERNAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(InternalCorrespondApprovals.Type)) + ' ' + UPPERCASE(FORMAT(InternalCorrespondApprovals."Document Type"));
            // SMTPMail.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
            Body += '<br><br>';
            Body += 'Dear ' + InternalCorrespondApprovals."Employee Name";
            Body += '<br><br>';
    ...
    procedure FnNotifyRecipientsGeneralCorrespondenceOnApproval(GeneralCorrespondenceHeader: Record "General Correspondence Header")
    var
    // ObjCompany: Record "Company Information";
    // // SMTPSetup: Record 
    // SenderEmail, SenderName, FileDirectory, Filename, SentMail, Header, Subject, Body, FilePath, URL2 : Text;
    // GeneralCorrespondenceLine: Record "General Correspondence Line";
    // CorrespondenceHeader: Record "General Correspondence Header";
    // TempBlob, InputFile : Codeunit "Temp Blob";
    // Ostream: OutStream;
    ...
    begin
        ...
        //Send to Recipients, Internal Memo
        IF GeneralCorrespondenceHeader."To all Staff" = FALSE THEN BEGIN
            IF GeneralCorrespondenceHeader.Type = GeneralCorrespondenceHeader.Type::Internal THEN
                IF GeneralCorrespondenceHeader."Document Type" = GeneralCorrespondenceHeader."Document Type"::Memo THEN
                    GeneralCorrespondenceLine.RESET;
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine."Document No", GeneralCorrespondenceHeader."Document No");
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine."Document Type", GeneralCorrespondenceHeader."Document Type");
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine.Type, GeneralCorrespondenceHeader.Type);
            IF GeneralCorrespondenceLine.FINDSET THEN BEGIN
                REPEAT
                    SentMail := GeneralCorrespondenceLine."Recipient Email";
                    // 
                    FileDirectory := 'C:\CORRESPONDENCES\';
                    Filename := 'Signed_Correspondence_' + FORMAT(GeneralCorrespondenceHeader."Document No") + '.pdf';

                    CorrespondenceHeader.RESET;
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader."Document No", GeneralCorrespondenceHeader."Document No");
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader."Document Type", GeneralCorrespondenceHeader."Document Type");
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader.Type, GeneralCorrespondenceHeader.Type);
                    IF CorrespondenceHeader.FINDFIRST THEN BEGIN
                        //  REPORT.SAVEASPDF(72107,FileDirectory+Filename,CorrespondenceHeader) ;       //  MESSAGE('SAVES');
                        //REPORT.SAVEAS(72107,'',ReportFormat::Pdf,
                        // TempBlob.Blob.CREATEOUTSTREAM(Ostream);
                        TempBlob.CREATEOUTSTREAM(Ostream);
                        RecRef.OPEN(DATABASE::"General Correspondence Header");
                        FRef := RecRef.FIELD(1);
                        FRef.SETRANGE(GeneralCorrespondenceHeader."Document No");
                        REPORT.SAVEAS(72107, '', REPORTFORMAT::Pdf, Ostream, RecRef);
                        // TempBlob.Blob.CREATEINSTREAM(Istream1);
                        TempBlob.CREATEINSTREAM(Istream1);
                        RecRef.CLOSE;
                    END;
                    if SentMail = '' then
                        Error('Error on 2 no recipient email');
                    IF SentMail <> '' THEN BEGIN
                        Header := ObjCompany.Name + ' ' + 'NOTIFICATION OF RECIEPT OF GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(GeneralCorrespondenceHeader.Type)) + ' ' + UPPERCASE(FORMAT(GeneralCorrespondenceHeader."Document Type"));
                        // SMTPMail.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
                        Body += '<br><br>';
                        Body += 'Dear ' + GeneralCorrespondenceLine."Recipient Name";
                        Body += '<br><br>';
                        Body += 'This is to notify you that a' + ' ' + (FORMAT(GeneralCorrespondenceHeader.Type)) + ' ' + (FORMAT(GeneralCorrespondenceHeader."Document Type")) + ' ' + 'has been sent to';
                        //Body+='<br><br>';
                        Body += GeneralCorrespondenceLine."Recipient Name" + ' ' + 'Signed by' + GeneralCorrespondenceHeader."Sender Name";
                        Body += '<br><br>';
                        Body += 'Correspondence Ref No: ' + GeneralCorrespondenceHeader."External Reference No" + ' ' + 'System Ref No ' + GeneralCorrespondenceHeader."Document No";
                        Body += '<br><br>';
                        Body += 'Correspondence Subject: ' + GeneralCorrespondenceHeader."Communication Subject";
                        Body += '<br><br>';
                        Body += 'Please find attached a copy of the communication';
                        Body += '<br><br>';
                        Body += '<br><br>';
                        Body += 'Login to the system on for more information ';
                        Body += '<br>';
                        Body += 'For any assistance please contact ICT Department';
                        Body += '<br>';
                        Body += ' Thanks & Regards';
                        ...
                        Body += ObjCompany.Name;
                        Body += '<br><br>';
                        // SMTPMail.AddAttachmentStream(Istream1, Filename);
                        // SMTPMail.AddAttachment(FileDirectory+Filename,Filename);
                        Body += 'This is a system generated mail.Please do not respond';
                        Body += '<br><br>';
                        RecordLink.RESET();
                        // RecordLink.SETRANGE("Document No", GeneralCorrespondenceHeader."Document No");
                        // IF RecordLink.FINDSET() THEN BEGIN
                        //     REPEAT
                        //         // FilePath :='C:\DOCS\ME_Project_AppointmentLetter_762.pdf';
                        //         FilePath := RecordLink.URL1;
                        //         // MESSAGE('%1',RecordLink.URL2);
                        ...
                        //                 SMTPMail.AddAttachment(FilePath, FileManagement.GetFileName(FilePath));
                        //                 // SMTPMail.AddAttachmentStream(Istream,FileManagement.GetFileName(FilePath));
                        //                 //InputFile.CLOSE;
                        //             END
                        //             ELSE
                        ...
                        //     UNTIL RecordLink.NEXT = 0;
                        // END;
                        SMTPMessage.Create(SenderEmail, Header, Body, TRUE);
                        SMTPMail.Send(SMTPMessage, "Email Scenario"::Default);
        ...
        IF GeneralCorrespondenceHeader."To all Staff" = FALSE THEN BEGIN
            IF GeneralCorrespondenceHeader.Type = GeneralCorrespondenceHeader.Type::Internal THEN
                IF GeneralCorrespondenceHeader."Document Type" = GeneralCorrespondenceHeader."Document Type"::Letter THEN
                    GeneralCorrespondenceLine.RESET;
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine."Document No", GeneralCorrespondenceHeader."Document No");
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine."Document Type", GeneralCorrespondenceHeader."Document Type");
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine.Type, GeneralCorrespondenceHeader.Type);
            IF GeneralCorrespondenceLine.FINDSET THEN BEGIN
                REPEAT
                    SentMail := GeneralCorrespondenceLine."Recipient Email";
                    FileDirectory := 'C:\DOCS\';
                    Filename := 'Signed_Correspondence ' + FORMAT(CorrespondenceHeader."Document No") + '.pdf';

                    CorrespondenceHeader.RESET;
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader."Document No", GeneralCorrespondenceHeader."Document No");
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader."Document Type", GeneralCorrespondenceHeader."Document Type");
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader.Type, GeneralCorrespondenceHeader.Type);
                    //REPORT.SAVEASPDF(72109,FileDirectory+Filename,CorrespondenceHeader);
                    TempBlob.CREATEOUTSTREAM(Ostream);
                    RecRef.OPEN(DATABASE::"General Correspondence Header");
                    FRef := RecRef.FIELD(1);
                    FRef.SETRANGE(GeneralCorrespondenceHeader."Document No");
                    // REPORT.SAVEAS(72109,'',REPORTFORMAT::Pdf,Ostream,RecRef);
                    TempBlob.CREATEINSTREAM(Istream1);
                    RecRef.CLOSE;
                    IF SentMail <> '' THEN BEGIN
                        Header := ObjCompany.Name + ' ' + 'NOTIFICATION OF RECEIPT OF GENERAL CORRESPONDENCE' + ' - ' + UPPERCASE(FORMAT(GeneralCorrespondenceHeader.Type)) + ' ' + UPPERCASE(FORMAT(GeneralCorrespondenceHeader."Document Type"));
                        // SMTPMail.Create(SenderName, SenderEmail, SentMail, Header, '', TRUE);
                        Body += '<br><br>';
                        Body += 'Dear ' + EmpName;
                        Body += '<br><br>';
        ...
        IF GeneralCorrespondenceHeader."To all Staff" = FALSE THEN BEGIN
            IF GeneralCorrespondenceHeader.Type = GeneralCorrespondenceHeader.Type::External THEN
                IF GeneralCorrespondenceHeader."Document Type" = GeneralCorrespondenceHeader."Document Type"::Letter THEN
                    GeneralCorrespondenceLine.RESET;
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine."Document No", GeneralCorrespondenceHeader."Document No");
            GeneralCorrespondenceLine.SETRANGE(GeneralCorrespondenceLine."Document Type", GeneralCorrespondenceHeader."Document Type");
            ...
            IF GeneralCorrespondenceLine.FINDSET() THEN BEGIN
                REPEAT
                    ...
                    CorrespondenceHeader.SETRANGE(CorrespondenceHeader.Type, GeneralCorrespondenceHeader.Type);
                    IF CorrespondenceHeader.FINDFIRST() THEN BEGIN
                        // REPORT.SAVEASPDF(72110,FileDirectory+Filename,CorrespondenceHeader);
                        TempBlob.CREATEOUTSTREAM(Ostream);
                        RecRef.OPEN(DATABASE::"General Correspondence Header");
                        FRef := RecRef.FIELD(1);
                        FRef.SETRANGE(CorrespondenceHeader."Document No");
                        // REPORT.SAVEAS(72110,'',REPORTFORMAT::Pdf,Ostream,RecRef);
                        TempBlob.CREATEINSTREAM(Istream1);
                        Filename := 'Signed_Correspondence ' + FORMAT(CorrespondenceHeader."Document No") + '.pdf';
                    END;
    ...
    procedure FnNotifyApproversInternalCorrespondenceRejection(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        ObjCompany.GET;
        // SMTPSetup.GET;
        // SenderEmail := SMTPSetup."Email Sender Address";
        ...
        IF SentMail <> '' THEN BEGIN
            ...
            SMTPMessage.Create(SentMail, Header, Body, TRUE);
            SMTPMail.Send(SMTPMessage, "Email Scenario"::Default);
            MESSAGE('Notifications to %1 sent successfully', InternalCorrespondenceHeader."Sender Name");
            //InternalCorrespondApprovals.Notified:=TRUE;
            //InternalCorrespondApprovals.MODIFY();
        END;
    end;

    procedure FnNotifyRecipientsOnIncomingCorrespondences(IncomingCorrespondenceHeader: Record "Incoming Correspondence Header")
    begin
        ObjCompany.GET;
        // SMTPSetup.GET;
        // SenderEmail := SMTPSetup."Email Sender Address";
        ...
        IncomingCorrespondenceLine.SETRANGE(IncomingCorrespondenceLine."Document No", IncomingCorrespondenceHeader."Document No");
        IF IncomingCorrespondenceLine.FINDSET THEN BEGIN
            REPEAT
                SentMail := IncomingCorrespondenceLine."Recipient Email";

                IF SentMail <> '' THEN BEGIN
                    Header := ObjCompany.Name + ' ' + 'NOTIFICATION OF RECIEPT OF INCOMING CORRESPONDENCE';
                    // SMTPMail.CreateMessage(SenderName, SenderEmail, SentMail, Header, '', TRUE);
                    Body += '<br><br>';
                    Body += 'Dear ' + IncomingCorrespondenceLine."Recipient Name";
                    Body += '<br><br>';
                    Body += 'This is to notify you that an External Correspondence has been sent received from' + IncomingCorrespondenceHeader."Sender Name" + 'and forwarded to you';
                    //Body +='<br><br>';
                    Body += 'Correspondence Ref No: ' + IncomingCorrespondenceHeader."External Reference No" + ' ' + 'System Ref No ' + IncomingCorrespondenceHeader."Document No";
                    Body += '<br><br>';
                    Body += 'Correspondence Subject: ' + IncomingCorrespondenceHeader."Communication Summary";
                    Body += '<br><br>';
                    Body += 'Please find attached a copy of the scanned communication';
                    Body += '<br><br>';
                    Body += '<br><br>';
                    Body += 'Login to the system on for more information ';
                    Body += '<br>';
                    Body += 'For any assistance please contact Registry Department';
                    Body += '<br>';
                    Body += ' Thanks & Regards';
                    Body += '<br><br>';
                    Body += ObjCompany.Name;
                    ...
                    Body += '<br><br>';
                    RecordLink.RESET();
                    // RecordLink.SETRANGE("Document No", IncomingCorrespondenceHeader."Document No");
                    // IF RecordLink.FINDSET() THEN BEGIN
                    //     //REPEAT
                    //     FilePath := RecordLink.URL1;
                    //     IF FileManagement.ServerFileExists(FilePath) THEN BEGIN
    ...
    procedure FnAuthorizeCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        //CurrUser:=USERID;
        IF InternalCorrespondenceHeader."Requires Authorization" = TRUE THEN BEGIN
            InternalCorrespondApprovals.RESET;
            InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
            InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
            InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
            IF InternalCorrespondApprovals.FINDLAST THEN BEGIN
                ...
                IF CurrUser = USERID THEN BEGIN
                    ...
                    InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                    InternalCorrespondApprovals.MODIFY(TRUE);
                    MESSAGE('Document No %1 Approved Successfully', InternalCorrespondenceHeader."Document No");
                END
                ELSE BEGIN
                    ERROR('You are not the Authorizer.Please Review and Forward the document');
                END;
            END;
        END;
    end;

    procedure FnApproveCorrespondenceApprovalRequestWithAuthorization(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        //IF InternalCorrespondenceHeader."Requires Authorization"= TRUE THEN BEGIN
        InternalCorrespondApprovals.RESET;
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
        ...
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
        IF InternalCorrespondApprovals.FINDFIRST THEN BEGIN
            // MESSAGE('%1', InternalCorrespondApprovals."User ID");
            // IF InternalCorrespondenceHeader."Signatory User ID" = InternalCorrespondApprovals."User ID" THEN BEGIN
            IF InternalCorrespondApprovals."User ID" = UserId THEN BEGIN
                InternalCorrespondApprovals.Testfield(Comments);
                CurrentSequence := InternalCorrespondApprovals.Sequence;
                // Message('Status Bef: %1',InternalCorrespondApprovals.Status);
                InternalCorrespondApprovals.Status := InternalCorrespondApprovals.Status::Approved;
                InternalCorrespondApprovals.Modify();
                // Message('State: %1', InternalCorrespondApprovals.Status);
                InternalCorrespondApprovals.Forwarded := TRUE;
                InternalCorrespondApprovals."DateTime Modified" := CURRENTDATETIME;
                FnNotifyInitiatorInternalCorrespondenceApproval(InternalCorrespondenceHeader);
                // FnNotifyRecipientsGeneralCorrespondenceOnApproval(InternalCorrespondenceHeader);
                //Set Current Approver
                if CurrentSequence = 1 then begin
                    InternalCorrespondApprovals.Reset();
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
                    // InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Pending);
                    InternalCorrespondApprovals.SetRange(InternalCorrespondApprovals.Sequence, 2);
                    if InternalCorrespondApprovals.find('-') then begin
                        InternalCorrespondenceHeader."Current Approver" := InternalCorrespondApprovals."User ID";
                        InternalCorrespondenceHeader.Modify();
                    end
                end;
                if CurrentSequence = 2 then begin
                    InternalCorrespondApprovals.Reset();
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
                    // InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
                    InternalCorrespondApprovals.SetRange(InternalCorrespondApprovals.Sequence, 3);
                    if InternalCorrespondApprovals.find('-') then begin
                        InternalCorrespondenceHeader."Current Approver" := InternalCorrespondApprovals."User ID";
                        InternalCorrespondenceHeader.Modify();
                    end
                end;
                if CurrentSequence = 3 then begin
                    InternalCorrespondApprovals.Reset();
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
                    InternalCorrespondApprovals.SetRange(InternalCorrespondApprovals.Sequence, 4);
                    if InternalCorrespondApprovals.find('-') then begin
                        InternalCorrespondenceHeader."Current Approver" := InternalCorrespondApprovals."User ID";
                        InternalCorrespondenceHeader.Modify();
                    end
                end;
                if CurrentSequence = 4 then begin
                    InternalCorrespondApprovals.Reset();
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
                    // InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
                    InternalCorrespondApprovals.SetRange(InternalCorrespondApprovals.Sequence, 5);
                    if InternalCorrespondApprovals.find('-') then begin
                        InternalCorrespondenceHeader."Current Approver" := InternalCorrespondApprovals."User ID";
                        InternalCorrespondenceHeader.Modify();
                    end
                end;
                if CurrentSequence = 5 then begin
                    InternalCorrespondApprovals.Reset();
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
                    InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
                    // InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Open);
                    InternalCorrespondApprovals.SetRange(InternalCorrespondApprovals.Sequence, 6);
                    if InternalCorrespondApprovals.find('-') then begin
                        InternalCorrespondenceHeader."Current Approver" := InternalCorrespondApprovals."User ID";
                        InternalCorrespondenceHeader.Modify();
                    end
                ...
                //Set Current Approver
                // Message('Approved before modify');
                // Message('Curr Bef App: %1',InternalCorrespondenceHeader."Current Approver");

                InternalCorrespondApprovals.MODIFY(TRUE);
                // Message('Approved after modify');
                // Message('Curr After App: %1',InternalCorrespondenceHeader."Current Approver");
                // Message('Aftr Status: %1',InternalCorrespondApprovals.Status);
            END ELSE BEGIN
    ...
    procedure FnSendAssignToNotificationOnincomingCorrespondence(IncomingCorrespondenceHeader: Record "Incoming Correspondence Header")
    begin
        UserSetup.RESET;
        UserSetup.SETRANGE(UserSetup."User ID", IncomingCorrespondenceHeader."Assign To");
        IF UserSetup.FINDSET THEN BEGIN
            UserSetup.TESTFIELD("Employee No.");
            Employee.RESET;
            Employee.SETRANGE(Employee."No.", UserSetup."Employee No.");
        ...
        CompanyDetails := 'Dear' + ' ' + RequesterName + ',<br>';
        SenderMessage := '<BR>This is to notify you that you have been assigned to review an external Communication ' + IncomingCorrespondenceHeader."Document No" + ' ' + 'Submitted by' + ' ' + IncomingCorrespondenceHeader."Sender Name" + ' '
        + ' Communication subject ' + ' ' + IncomingCorrespondenceHeader."Communication Summary";
        SenderMessage := SenderMessage + ' Assignment Remarks ' + IncomingCorrespondenceHeader."Assignment Remarks" + '<br/><br/>';
        SenderMessage := SenderMessage + ' in the system by ' + USERID + '<br/><br/>';
        SenderMessage := SenderMessage + 'Once completed, you can send the confirmation and notes by using the Send Assignment Completion Notification Action' + '<br/><br/>';
        SupplierDetails := '';
        LoginDetails := 'Please login to the system to get the document on the Assigned Incoming Correspondences List.<br/>';
        ProcNote := '<BR> Kind Regards,';
        ...
        emailhdr := UPPERCASE(CompInfo.Name) + ' ASSIGNMENT TO REVIEW INCOMING CORRESPONDENCE';
        // cu400.CreateMessage(CompInfo.Name, SenderAddress, CCEmail, emailhdr,
        // CompanyDetails + '<BR></BR>' + SenderMessage + SupplierDetails + LoginDetails, TRUE);
        // cu400.AddCC(CustEmail);
        // cu400.AppendBody(ProcNote);
        //cu400.AddAttachment(FileDirectory+FileName,FileName);
        SMTPMessage.Create(SentMail, Header, Body, TRUE);
        // SMTPEmail.Send(EmailMessage, "Email Scenario"::Default);
        SendingDate := TODAY;
        SendingTime := TIME;
        SLEEP(1000);
        Window.CLOSE;

        MESSAGE('%1 Notified Successfully', IncomingCorrespondenceHeader."Assign To");
    end;

    procedure FnSendAssignmentCompletionNotificationnOnIncomingCorrespondences(IncomingCorrespondenceHeader: Record "Incoming Correspondence Header")
    begin
        UserSetup.RESET;
        UserSetup.SETRANGE(UserSetup."User ID", IncomingCorrespondenceHeader."Assigned By");
        IF UserSetup.FINDSET THEN BEGIN
            UserSetup.TESTFIELD("Employee No.");
            Employee.RESET;
            Employee.SETRANGE(Employee."No.", UserSetup."Employee No.");
        ...
        CompanyDetails := 'Dear' + ' ' + RequesterName + ',<br>';
        SenderMessage := '<BR>This is to notify you that review of the Incoming Correspondence ' + ' Submitted by:' + ' ' + IncomingCorrespondenceHeader."Sender Name" + ' '
        + ' Communication Subect' + ' ' + IncomingCorrespondenceHeader."Communication Summary";
        SenderMessage := SenderMessage + ' assigned to ' + IncomingCorrespondenceHeader."Assign To" + ' has been completed and feedback provided' + '<br/><br/>';
        //SenderMessage:=SenderMessage+'Once completed, you can send the confirmation and notes by using the Send Assignment Completion Notification Action'+'<br/><br/>';      
        SupplierDetails := '';
        LoginDetails := 'Please login to the system to get the document on the Incoming Correspondence and any notes/questions raised.<br/>';
        ProcNote := '<BR> Kind Regards,';

        IF CustEmail = '' THEN
            EXIT;

        emailhdr := UPPERCASE(CompInfo.Name) + ' COMPLETION OF ASSIGNMENT TO REVIEW INCOMING CORRESPONDENCE ' + IncomingCorrespondenceHeader."Document No";
        // cu400.CreateMessage(CompInfo.Name, SenderAddress, CCEmail, emailhdr,
        // CompanyDetails + '<BR></BR>' + SenderMessage + SupplierDetails + LoginDetails, TRUE);
        // cu400.AddCC(CustEmail);
        // cu400.AppendBody(ProcNote);
    ...
    //Revert to Previous Approver on Rejection
    procedure FnRevertOnRejectionOfCorrespondenceApprovalRequest(InternalCorrespondenceHeader: Record "General Correspondence Header")
    begin
        InternalCorrespondApprovals.RESET;
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document No", InternalCorrespondenceHeader."Document No");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals."Document Type", InternalCorrespondenceHeader."Document Type");
        InternalCorrespondApprovals.SETRANGE(InternalCorrespondApprovals.Type, InternalCorrespondenceHeader.Type);
        InternalCorrespondApprovals.SetRange(InternalCorrespondApprovals.Status, InternalCorrespondApprovals.Status::Approved);
        IF InternalCorrespondApprovals.FINDSET THEN BEGIN
            REPEAT
                if InternalCorrespondApprovals."Employee Email" = '' then
                    Error('There is no email for %1', InternalCorrespondApprovals."Employee Name");
                SentMail := InternalCorrespondApprovals."Employee Email";
                ...
                IF SentMail <> '' THEN BEGIN
                    ...
                    Body += 'Dear ' + EmpName;
                    Body += '<br><br>';
                    Body += 'This is to notify you that the' + ' ' + (FORMAT(InternalCorrespondenceHeader.Type)) + ' ' + (FORMAT(InternalCorrespondenceHeader."Document Type")) + ' you had approved has been rejected with comments';
                    Body += '<br><br>';
                    Body += 'Rejected By' + ' ' + InternalCorrespondenceHeader."Rejected By";
                    Body += '<br><br>';
                    Body += 'Rejection Comments: "' + InternalCorrespondenceHeader."Rejection Comments" + '"';
                    ...
                    Body += '<br><br>';
                    Body += '<br><br>';
                    Body += 'This email is for notification purposes only.';
                    Body += '<br>';
                    Body += 'For any assistance please contact ICT Department';
                    Body += '<br>';
                    Body += ' Thanks & Regards';
