...
codeunit 70111 procurementQueries
{
    trigger OnRun()
    begin
        fngetprojectsforvendor('VEND000195');
        fngetlpoforvendor('VEND000001');
    ...
    procedure fngetprojectsforvendor(vendorNo: Text) data: Text
    var
        projects: Record "Purchase Header";
    begin
        projects.Reset();
        projects.SetRange("Buy-from Vendor No.", vendorNo);
        projects.SetRange("Document Type", projects."Document Type"::"Blanket Order");
        projects.SetRange("Contract Type", projects."Contract Type"::"Original Contract");
        if projects.Find('-') then begin
            repeat
                data += Format(projects."No.") + '*' + Format(projects.Description) + '::::';
            until projects.Next = 0;
        end;
        Exit(data);
    end;
    //     procedure fngetlpoforvendor(vendorNo: Text): Text
    // var
    //     projects: Record "Purchase Header";
    //     data: Text;
    //     recordCount: Integer;
    // begin
    //     data := '';
    //     data += 'Checking for Vendor No.: ' + vendorNo + '::::';
    //     projects.Reset();
    //     projects.SetRange("Buy-from Vendor No.", vendorNo);
    //     if projects.Count = 0 then begin
    //         data += 'No purchase headers found for vendor number filter.::::';
    //     end;

    //     projects.SetRange("Document Type", projects."Document Type"::"Order");
    //     if projects.Count = 0 then begin
    //         data += 'No purchase orders found for vendor with Document Type = Order.::::';
    //     end;

    //     if projects.Find('-') then begin
    //         repeat
    //             data += Format(projects."No.") + '*' + Format(projects."Buy-from Vendor Name") + '::::';
    //             recordCount += 1;
    //         until projects.Next() = 0;

    //         data += 'Total Purchase Orders Found: ' + Format(recordCount) + '::::';
    //     end else begin
    //         data += 'No matching purchase orders found after filtering.::::';
    //     end;

    //     exit(data);
    // end;

    procedure fngetlpoforvendor(vendorNo: Text) data: Text
    var
        projects: Record "Purchase Header";
    begin
        projects.Reset();
    ...
    begin
        ...
        if tbl_bidContractSecurityRegister.FindSet(true) then begin
            repeat
                data += Format(tbl_bidContractSecurityRegister."Document Type") + '*' + tbl_bidContractSecurityRegister."IFS No" + '*' + Format(tbl_bidContractSecurityRegister."Security Type") 
                + '*' + Format(tbl_bidContractSecurityRegister."Issuer Institution Type") + 
                '*' + Format(tbl_bidContractSecurityRegister."Issuer Registered Offices") + '*' + 
                tbl_bidContractSecurityRegister.Description + '*' + Format(tbl_bidContractSecurityRegister."Security Amount (LCY)") + '*' +
                 Format(tbl_bidContractSecurityRegister."Bid Security Effective Date") + '*' + 
                 Format(tbl_bidContractSecurityRegister."Bid Security Validity Expiry") + '*' + Format(tbl_bidContractSecurityRegister."Security ID") + '*' + Format(tbl_bidContractSecurityRegister."Security Closure Date") + '*' + Format(tbl_bidContractSecurityRegister."Security Closure Voucher No.") + '*' + Format(tbl_bidContractSecurityRegister."Security Closure Type") + '*' + Format(tbl_bidContractSecurityRegister."Form of Security") + '*' + Format(tbl_bidContractSecurityRegister."Issuer/Guarantor Name") + ':';
    ...
    begin
        ...
        if tbl_bidPastExperience.FindSet(true) then begin
            repeat
                data += Format(tbl_bidPastExperience.Address) + '*' + tbl_bidPastExperience."Client Name" + '*' + Format(tbl_bidPastExperience.City) + '*' + tbl_bidPastExperience."Address 2"
                 + '*' + tbl_bidPastExperience."Phone No." + '*' + tbl_bidPastExperience."Nationality ID" + '*' +
                  Format(tbl_bidPastExperience."Citizenship Type") + '*' + Format(tbl_bidPastExperience."Entity Ownership %") + '*' 
                  + Format(tbl_bidPastExperience."Share Types") + '*' + Format(tbl_bidPastExperience."No. of Shares") + '*' 
                  + Format(tbl_bidPastExperience."Nominal Value/Share") + '*' + Format(tbl_bidPastExperience."Total Nominal Value") + '*' 
                  + Format(tbl_bidPastExperience."Assignment/Project Name") + '*' + Format(tbl_bidPastExperience."Project Completion % (Work)") + '*' 
                  + Format(tbl_bidPastExperience."Project Completion % (Value)") + '*' + Format(tbl_bidPastExperience."Assignment Status") + '*'
                   + Format(tbl_bidPastExperience."Assignment Value LCY") + '*' + Format(tbl_bidPastExperience."Assignment End Date") + '*' 
                   + Format(tbl_bidPastExperience."Assignment Start Date") + '*' + Format(tbl_bidPastExperience."Contract Ref No") + '*' 
                   + Format(tbl_bidPastExperience."Delivery Location") + '*' + tbl_bidPastExperience."Project Scope Summary" + '*' 
                   + tbl_bidPastExperience."Primary Contact Email" + '*' + Format(tbl_bidPastExperience."Primary Contact Designation") + '*' 
                   + Format(tbl_bidPastExperience."Primary Contact Tel") + '*' + Format(tbl_bidPastExperience."Primary Contact Person") + '*' + Format(tbl_bidPastExperience.Blocked) + '*' + Format(tbl_bidPastExperience."No. Series") + '*' + tbl_bidPastExperience."E-Mail" + '*' + tbl_bidPastExperience.County + '*' + tbl_bidPastExperience."Post Code" + '*' + Format(tbl_bidPastExperience."Country/Region Code") + '*' + Format(tbl_bidPastExperience."Ownership Effective Date") + '*' + Format(tbl_bidPastExperience."Entry No.")+':';
    ...
    var
        banking: Record "Employee Bank AccountX";
    begin
        banking.Reset();
        repeat
            data += banking.Code + '*' + banking."Bank Name" + ':';
        until banking.Next = 0;
        Exit(data);
    end;
