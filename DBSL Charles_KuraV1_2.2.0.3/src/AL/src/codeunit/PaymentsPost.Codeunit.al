...
Codeunit 57000 "Payments-Post"
{
    ...
    var
        ...
        Text029: label 'Are you sure you want to post Batch Imprest No %1 ?';
        ...
        Text030: label 'Are you sure you want to post Standing Imprest No %1 ?';
    ...
    begin
        IF CONFIRM('Are you sure you want to post the Payment Voucher No. ' + PV."No." + ' ?') = TRUE THEN BEGIN
            ...
            GenJnLine."Document Type" := PV."Document Type"::"Standing Imprest";
            ...
            GenJnLine."Bank Name" := PV."Bank Name";
            GenJnLine."Remaining Amount" := PVLines."Remaining Amount";
            ...
            IF PVLines.FindSet() THEN BEGIN
                REPEAT
                    ...
                    GenJnLine."External Document No." := PV."Cheque No";
                    GenJnLine."Payment Method Code" := PV."Pay Mode";
                    GenJnLine."Payment Reference" := PV."Paying Bank Account";
                    //GenJnLine.Description := CopyStr(PV.Payee + '-VAT Withheld', 1, 100);
                    GenJnLine.Description := CopyStr(PV.Payee + '-VAT Withheld @ ' + Format(Tariffs.Percentage) + '%', 1, 100);
                    ...
                    GenJnLine."Paying Bank Code" := PV."Paying Bank Account";
                    GenJnLine."Bank Name" := PV."Bank Name";
                    GenJnLine."VAT Withheld Code PV" := PVLines."VAT Withheld Code";
                    GenJnLine."Remaining Amount" := PVLines."Remaining Amount";
                    ...
                    GenJnLine.Description := CopyStr('VAT Withheld on:' + PVLines.Description + ' @ ' + Format(Tariffs.Percentage) + '%', 1, 100);
                    //GenJnLine."Description 2":=PV."On behalf of";
                    ...
                    GenJnLine."Paying Bank Code" := PV."Paying Bank Account";
                    GenJnLine."Bank Name" := PV."Bank Name";
                    GenJnLine."WHT Code" := PVLines."W/Tax Code";
                    GenJnLine."Remaining Amount" := PVLines."Remaining Amount";
                    //GenJnLine."Applies-to Doc. Type":=GenJnLine."Applies-to Doc. Type"::Invoice;
                    //GenJnLine."Applies-to Doc. No.":=PVLines."Applies to Doc. No";
                    //GenJnLine.VALIDATE(GenJnLine."Applies-to Doc. No.");
                    ...
                    //Post PCBL Charge
                    IF PVLines."PCBL Charge" > 0 THEN
                        Tariffs.GET(PVLines."PPRA Code");
                    LineNo := LineNo + 10000;
                    GenJnLine.INIT;
                    IF CMSetup.GET THEN
                        GenJnLine."Journal Template Name" := CMSetup."PV Journal Template";
                    ...
                    GenJnLine."External Document No." := PV."Cheque No";
                    GenJnLine."Payment Method Code" := PV."Pay Mode";
                    GenJnLine.Description := CopyStr(PV.Payee + ' ' + 'PCBL Charge', 1, 100);
                    GenJnLine.Amount := -PVLines."PCBL Charge";
                    IF PV."Pay Mode" = 'CHEQUE' THEN
                        // GenJnLine."Currency Code" := PV.Currency;
                        GenJnLine."Project No" := PVLines."Project No";
                    GenJnLine."Contractor No" := PVLines."Contractor No";
                    ...
                    GenJnLine."Paying Bank Code" := PV."Paying Bank Account";
                    GenJnLine."Bank Name" := PV."Bank Name";
                    GenJnLine."PPRA Code PV" := PVLines."PPRA Code";
                    GenJnLine."Remaining Amount" := PVLines."Remaining Amount";
                    IF GenJnLine.Amount <> 0 THEN
                        GenJnLine.INSERT;

                    //Post PCBL Charge Balancing Entry
                    LineNo := LineNo + 10000;
                    GenJnLine.INIT;
                    IF CMSetup.GET THEN
                        GenJnLine."Journal Template Name" := CMSetup."PV Journal Template";
                    ...
                    GenJnLine."Paying Bank Code" := PV."Paying Bank Account";
                    GenJnLine."Bank Name" := PV."Bank Name";
                    GenJnLine."PAYE Code PV" := PVLines."PAYE Code";
                    GenJnLine."Remaining Amount" := PVLines."Remaining Amount";
                    ...
                    GenJnLine."Paying Bank Code" := PV."Paying Bank Account";
                    GenJnLine."Bank Name" := PV."Bank Name";
                    GenJnLine."Retention Code PV" := PVLines."Retention Code";
                    GenJnLine."Remaining Amount" := PVLines."Remaining Amount";
                    ...
                    //    GenJnLine."Employee No." := PV."Account No.";
                    //   GenJnLine."Employee Name" := CopyStr(PV."Account Name", 1, 100);
                    // GenJnLine."Destination Narration" := CopyStr(PV."Payment Narration", 1, 250);
    ...
    begin
        IF CONFIRM('Are you sure you want to post the Payment Voucher No. ' + PV."No." + ' ?') = TRUE THEN BEGIN
            ...
            IF PV."Pay Mode" = 'CHEQUE' THEN BEGIN
                PV.TESTFIELD(PV."Cheque No");
                PV.TESTFIELD(PV."Cheque Date");
                PV.TestField("Check Printed", true);
            END;
            ...
            IF PVLines.FindSet() THEN BEGIN
                REPEAT
                    ...
                    // DEBUG MESSAGE 1 - Check all tariff codes on each line before any GET
                    // MESSAGE('LINE %1 | Acct: %2 | VATWHCode: [%3] VATWHAmt: %4 | WTaxCode: [%5] WTaxAmt: %6 | RetCode: [%7] RetAmt: %8 | PAYECode: [%9] PAYEAmt: %10',
                    //     PVLines."Line No",
                    //     PVLines."Account No",
                    //     PVLines."VAT Withheld Code",
                    //     PVLines."VAT Withheld Amount",
                    //     PVLines."W/Tax Code",
                    //     PVLines."W/Tax Amount",
                    //     PVLines."Retention Code",
                    //     PVLines."Retention  Amount",
                    //     PVLines."PAYE Code",
                    //     PVLines."PAYE Amount");
                    ...
                    IF PVLines."VAT Withheld Amount" > 0 THEN begin
                        IF PVLines."VAT Withheld Code" = '' THEN
                            ERROR('VAT Withheld Code is blank on PV Line %1 for Account %2. Please populate the code before posting.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."VAT Withheld Code");
                    ...
                    IF PVLines."W/Tax Amount" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Please populate the code before posting.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                    ...
                    IF PVLines."PCBL Charge" > 0 THEN begin
                        IF PVLines."PPRA Code" = '' THEN
                            ERROR('PPRA/PCBL Code is blank on PV Line %1 for Account %2. Please populate the code before posting.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."PPRA Code");
                    ...
                    IF PVLines."PAYE Amount" > 0 THEN begin
                        IF PVLines."PAYE Code" = '' THEN
                            ERROR('PAYE Code is blank on PV Line %1 for Account %2. Please populate the code before posting.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."PAYE Code");
                    ...
                    IF PVLines."Retention  Amount" > 0 THEN begin
                        IF PVLines."Retention Code" = '' THEN
                            ERROR('Retention Code is blank on PV Line %1 for Account %2. Please populate the code before posting.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."Retention Code");
                    ...
                    // // DEBUG MESSAGE 2 - Check obligation amounts and the W/Tax Code they will use
                    // MESSAGE('OBLIGATIONS on Line %1 | ObligWHT: %2 | ObligIncTax: %3 | ObligVAT: %4 | W/Tax Code used for all three: [%5]',
                    //     PVLines."Line No",
                    //     PVLines."Obligation WHT",
                    //     PVLines."Obligation Income Tax",
                    //     PVLines."Obligation VAT",
                    //     PVLines."W/Tax Code");

                    IF PVLines."Obligation WHT" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Obligation WHT cannot be posted without it.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                    ...
                    IF PVLines."Obligation WHT" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Obligation WHT balancing entry cannot be posted without it.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                    ...
                    IF PVLines."Obligation Income Tax" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Obligation Income Tax cannot be posted without it.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                    ...
                    IF PVLines."Obligation Income Tax" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Obligation Income Tax balancing entry cannot be posted without it.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                    ...
                    IF PVLines."Obligation VAT" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Obligation VAT cannot be posted without it.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                        ...
                        GenJnLine."External Document No." := PV."Cheque No";
                        GenJnLine."Payment Method Code" := PV."Pay Mode";
                        GenJnLine.Description := CopyStr(PV.Payee + ' ' + 'Obligation VAT', 1, 100);
                        GenJnLine.Amount := PVLines."Obligation VAT";
                        IF PV."Pay Mode" = 'CHEQUE' THEN
                            GenJnLine."Currency Code" := PV.Currency;
                        GenJnLine."Project No" := PVLines."Project No";
                    ...
                    IF PVLines."Obligation VAT" > 0 THEN begin
                        IF PVLines."W/Tax Code" = '' THEN
                            ERROR('W/Tax Code is blank on PV Line %1 for Account %2. Obligation VAT balancing entry cannot be posted without it.', PVLines."Line No", PVLines."Account No");
                        Tariffs.GET(PVLines."W/Tax Code");
                ...
                // DEBUG MESSAGE 3 - Confirm all journal lines inserted for this PV line before moving to next
                // MESSAGE('COMPLETED processing PV Line %1. Total GenJnLine count in batch now: %2. Moving to next line.',
                //     PVLines."Line No",
                //     LineNo / 10000);
            ...
            // // Stamp Paying Bank Account, Narration and Employee details on all GL Entries for this PV
            // GLEntry.RESET;
            // GLEntry.SETRANGE(GLEntry."Document No.", PV."No.");
            // GLEntry.SETRANGE(GLEntry.Reversed, FALSE);
            // IF GLEntry.FindSet(true) THEN BEGIN
            //     REPEAT
            //         GLEntry."Paying Bank Account" := PV."Paying Bank Account";
            //         GLEntry."Destination Narration" := CopyStr(PV."Payment Narration", 1, 250);
            //         GLEntry."Employee No." := PV."Account No.";
            //         GLEntry."Employee Name" := PV."Account Name";
            //         GLEntry.Modify;
    ...
    begin
        if Confirm(Text002, false, Imprest."No.") = true then begin
            ...
            Imprest.CalcFields(ConvertedAmount);
            ...
            // GenJnLine.Description := CopyStr(Imprest.Payee + ' Imprest - ' + Imprest.Description, 1, 100);
            GenJnLine.Description := CopyStr(Imprest.Description, 1, 100);
            ...
            GenJnLine.Validate(GenJnLine."Dimension Set ID");
            GenJnLine."Employee No." := Imprest."Account No.";
            GenJnLine."Employee Name" := CopyStr(Imprest."Account Name", 1, 100);
            GenJnLine."Paying Bank Account" := Imprest."Paying Bank Account";
            GenJnLine."Bank Name." := Imprest."Bank Name";
            GenJnLine."Destination Narration" := CopyStr(Imprest."Destination Narration", 1, 250);
    ...
    procedure BatchPostImprest(var Imprest: Record payments)
    var
        ImprestLines: Record payments;
        GenJnLine: Record "Gen. Journal Line";
        LineNo: Integer;
    ...
    begin
        ...
        if Confirm(Text029, false, Imprest."No.") = true then begin
            // if Imprest.Status <> Imprest.Status::Released then
            //     Error(Text003, Imprest."No.");
            ...
            GenJnlTemplate.Get(CMSetup."Imprest Journal Template");
            ...
            BatchImprestVoucherLines.SetRange("No.", Imprest."No.");
            if BatchImprestVoucherLines.FindSet() then begin
                repeat
                    ...
                    GenJnLine."Account No." := BatchImprestVoucherLines."Account No.";
                    GenJnLine."Posting Date" := Imprest."Posting Date";
                    GenJnLine."Document No." := BatchImprestVoucherLines."Imprest Req. No.";
                    GenJnLine."External Document No." := BatchImprestVoucherLines."Imprest Req. No.";
                    GenJnLine.Description := CopyStr(BatchImprestVoucherLines."Account Name", 1, 100);
                    GenJnLine."Currency Code" := '';
                    GenJnLine.VALIDATE("Currency Code");
                    GenJnLine.Amount := BatchImprestVoucherLines."Imprest Amount";
                    GenJnLine."Shortcut Dimension 1 Code" := BatchImprestVoucherLines."Shortcut Dimension 1 Code";
                    GenJnLine.Validate("Shortcut Dimension 1 Code");
                    GenJnLine."Shortcut Dimension 2 Code" := BatchImprestVoucherLines."Shortcut Dimension 2 Code";
                    GenJnLine.Validate("Shortcut Dimension 2 Code");
                    GenJnLine."Dimension Set ID" := Imprest."Dimension Set ID";
                    ...
                    if GenJnLine.Amount <> 0 then
                        GenJnLine.Insert();
                    TotalAmount += BatchImprestVoucherLines."Imprest Amount";
                // GenJnLine.Init;
                // GenJnLine."Journal Template Name" := CMSetup."Imprest Journal Template";
                // GenJnLine."Journal Batch Name" := CMSetup."IMPREST Journal Batch Name";
                // GenJnLine."Line No." := LineNo;
                // GenJnLine."Account Type" := GenJnLine."account type"::"Bank Account";
                // GenJnLine."Account No." := BatchImprestVoucherLines."Paying Bank Account";
                // GenJnLine."Posting Date" := Today;
                // GenJnLine."Document No." := BatchImprestVoucherLines."No.";
                // GenJnLine."External Document No." := Imprest."Cheque No";
                ...
                // GenJnLine."Bal. Account Type" := GenJnLine."bal. account type"::Employee;
                // GenJnLine."Bal. Account No." := BatchImprestVoucherLines."Account No.";
                // GenJnLine.Validate("Bal. Account No.");
                // GenJnLine."Dimension Set ID" := Imprest."Dimension Set ID";
                // GenJnLine.Validate(GenJnLine."Dimension Set ID");
                // if GenJnLine.Amount <> 0 then
                ...
                // BatchImprestHeader.SetRange("No.", BatchImprestVoucherLines."Imprest Req. No.");
                // if BatchImprestHeader.FindFirst() then begin
                //     BatchImprestHeader.Posted := true;
                //     BatchImprestHeader."Posted By" := UserId;
                //     BatchImprestHeader."Posted Date" := Today;
                //     BatchImprestHeader."Time Posted" := Time;
                //     BatchImprestHeader.Modify;
                // end;
                // end;
                until BatchImprestVoucherLines.Next() = 0;
            end;
            // One Bank Line for Total
            if TotalAmount <> 0 then begin
                ...
                GenJnLine."Document No." := Imprest."No.";
                GenJnLine."External Document No." := Imprest."Cheque No";
                GenJnLine.Description := CopyStr(Imprest."Payment Narration", 1, 100);
                GenJnLine."Currency Code" := '';
            ...
            if BatchImprestVoucherLines.FindSet() then begin
                repeat
                    if BatchImprestHeader.Get(BatchImprestVoucherLines."Imprest Req. No.") then
                        if not BatchImprestHeader.Posted then begin
                            BatchImprestHeader.Posted := true;
                            BatchImprestHeader."Posted By" := UserId;
                            BatchImprestHeader."Posted Date" := Today;
                            BatchImprestHeader."Time Posted" := Time;
    ...
    begin
        ...
        AddToNoText(NoText, NoTextIndex, PrintExponent, WText028);
        //AddToNoText(NoText, NoTextIndex, PrintExponent, Format(No * 100) + '/100');
        // Convert cents to words instead of fraction
        CentsAmount := Round(No * 100, 1);
        if CentsAmount = 0 then
            CentsText := 'Zero Cents'
        else
            CentsText := ConvertCentsToWords(CentsAmount) + ' CENTS';
    ...
    local procedure ConvertCentsToWords(CentsAmount: Integer): Text[50]
    var
        Tens: Integer;
        Ones: Integer;
        Result: Text[50];
    begin
        ...
        if CentsAmount < 0 then
            CentsAmount := Abs(CentsAmount);

        if CentsAmount <= 20 then
            exit(OnesText[CentsAmount])
        else begin
            Tens := CentsAmount DIV 10;
            Ones := CentsAmount MOD 10;
            Result := TensText[Tens];
            if Ones > 0 then
                Result := Result + ' ' + OnesText[Ones];
            exit(Result);
        end;
    end;
    ...
    begin
        ...
        if Confirm(Text002, false, Imprest."No.") = true then begin
          ...
          *///
            ...
            GenJnLine."Employee Name" := CopyStr(Imprest."Account Name", 1, 100);
            GenJnLine."Paying Bank Account" := Imprest."Paying Bank Account";
            GenJnLine."Destination Narration" := CopyStr(Imprest."Payment Narration", 1, 250);
    ...
    procedure PrintPVCheck(PV: Record payments)
    var
        PVLines: Record "PV Lines";
        GenJnLine: Record "Gen. Journal Line";
        LineNo: Integer;
        ...
        HasValue: boolean;
    begin
        IF CONFIRM('Are you sure you want to print check  Payment Voucher No. ' + PV."No." + ' ?') = TRUE THEN BEGIN
            IF PV.Status <> PV.Status::Released THEN
                ERROR('The Payment Voucher No %1 has not been fully approved', PV."No.");
            IF PV.Posted THEN
                ERROR('Payment Voucher %1 has been posted', PV."No.");
            ...
            if PVLines.FindSet then begin
                ...
                GenJnLine.Validate("Currency Code");
                GenJnLine.Amount := PV."Total Net Amount";
                GenJnLine.Description := CopyStr((PV.Payee), 99);
            ...
            if GenJnLine.FindSet then begin
                repeat
                    CMSetup.Get;
                    PrintCheck(GenJnLine, CMSetup."PV Journal Template", CMSetup."PV Journal Template");
    ...
    //
    procedure PrintBatchCheck(PV: Record "Batch Check Voucher")
    var
    ...
    begin
        if Confirm('Are you sure you want to print check Batch No. ' + PV."No." + ' ?') = false then
            exit;

        if PV.Posted then
            Error('The Voucher %1 has been posted', PV."No.");
        ...
        if PVLines.FindFirst() then begin
            ...
            GenJnLine."Payment Method Code" := PV."Pay Mode";
            GenJnLine.Validate("Currency Code");
            GenJnLine.Amount := PV.Amount;
            GenJnLine.Description := CopyStr(PV.Payee, 1, 50);
            GenJnLine."Bal. Account Type" := GenJnLine."Account Type"::"Bank Account";
            GenJnLine."Bal. Account No." := PV."Paying Bank Account";
            GenJnLine."Bank Payment Type" := GenJnLine."Bank Payment Type"::"Computer Check";
            GenJnLine.Validate("Currency Code");
        ...
        if BankAcc.Get(PV."Paying Bank Account") then begin
            CheckReport.InitializeRequest(
                PV."Paying Bank Account",
                BankAcc."Last Check No.",
                false,
                false,
                false,
                false
        ...
        CheckReport.SetTableView(GenJnLine);
    ...
    begin
        ...
        if PV."Pay Mode" = 'CHEQUE' then
            if not PV."Check Printed" then
                Error('Check must be printed before posting Batch Check Voucher %1.', PV."No.");
        ...
        if PVLines.FindSet() then begin
            repeat
                ...
                if VendLedgEntry.FindFirst() then begin
                    VendLedgEntry."Vendor Name" := CopyStr(PVLines."Vedor Name", 1, 100);
    ...
    procedure PrintCheck(PCASH: Record payments)
    var
    ...
    begin
        ...
        // GenJournalLine.SetRange("Journal Template Name",CMS."Bank TR Journal Template");
        // GenJournalLine.SetRange("Journal Batch Name", CMS."Bank TR Journal Batch Name");
        // DocumentPrint.PrintCheck(GenJournalLine);
        // CODEUNIT.Run(CODEUNIT::"Adjust Gen. Journal Balance", Rec);

        if Confirm('Are you sure you want to print Check for' + PCASH."No." + ' ?') = true then begin
            if PCASH.Status <> PCASH.Status::Released then
                Error('The Standing Imprest No %1 has not been fully approved', PCASH."No.");
            if PCASH.Posted then
                Error('Check has already  %1 been printed', PCASH."No.");
            PCASH.TestField(Date);
            PCASH.TestField("Paying Bank Account");
            //PCASH.TESTFIELD(PCASH.Payee);
            PCASH.TestField(PCASH."Pay Mode");
            ...
            PCASHLines.SetRange(PCASHLines.No, PCASH."No.");
            if not PCASHLines.FindLast then
                Error('Standing Imprest Lines cannot be empty');
            ...
            if PCASHLines.FindFirst then begin
                repeat
                    ...
                    //
                    CMSetup.Get();
                    CMSetup.TestField("Standing Vendor Check No");
                    If Vendor.Get(CMSetup."Standing Vendor Check No") then begin
                        VendorName := Vendor.Name;
                    end;
                    //
                    GenJnLine.Description := VendorName;
                ...
                if GenJnLine.FindSet then begin
                    repeat
                        CMSetup.Get;
                        GenJnLine.SetRange("Bal. Account No.", PCASH."Paying Bank Account");
    ...
    //Standing Imprest
    procedure "Post Standing Imprest"(var Imprest: Record payments)
    var
        ImprestLines: Record payments;
        GenJnLine: Record "Gen. Journal Line";
        LineNo: Integer;
    ...
    begin
        ...
        if Confirm(Text030, false, Imprest."No.") = true then begin
            if Imprest.Status <> Imprest.Status::Released then
                Error(Text003, Imprest."No.");

            CMSetup.Get;
            CMSetup.TestField("StandingImprest Posting Group");
            ...
            GenJnLine."Posting Date" := Imprest."Posting Date";
            GenJnLine."Document No." := Imprest."No.";
            //GenJnLine."Document Type":=GenJnLine."Document Type"::"Standing Imprest Requisition";
            GenJnLine."External Document No." := Imprest."Cheque No";
            //GenJnLine.Description:='Imprest issued to '+Imprest.Payee;
            GenJnLine.Description := CopyStr(Imprest."Payment Narration", 1, 100);
            GenJnLine."Currency Code" := '';
            ...
            GenJnLine."Bal. Account No." := Imprest."Account No.";
            GenJnLine.Validate("Bal. Account No.");
            GenJnLine."Posting Group" := CMSetup."StandingImprest Posting Group";



            //add jobs Morris
            // GenJnLine."Job No." := ImprestLines.JobNo;
            // GenJnLine."Job Task No." := ImprestLines."Job Task No.";
            // GenJnLine."Job Quantity" := 1;
            // GenJnLine.Validate(GenJnLine."Job No.");
            // GenJnLine.Validate(GenJnLine."Job Task No.");
            ...
            if GLEntry.FindFirst then begin
                ...
                Imprest.Modify;
                //SendPostingNotification(Imprest);

                // From: Current Standing Imprest Requisition
                // To:   The same Imprest record (now posted)

                RecordLinks.Reset();
                RecordLinks.SETRANGE(RecordLinks."Record ID", payments.RecordId);
                if RecordLinks.FindSet() then
                    repeat
                        RecordLinksTransfer.Init();
                        RecordLinksTransfer."Link ID" := 0;
                        RecordLinksTransfer."Record ID" := Imprest.RecordId;
    ...
    var
        ...
        CMSetup: Record "Cash Management Setup";
    begin
        if Confirm('Are you sure you want to post this InterBank Transfer' + PCASH."No." + ' ?') = true then begin
            if PCASH.Status <> PCASH.Status::Released then
            ...
            if PCASH."Pay Mode" = 'CHEQUE' then begin
                PCASH.TestField(PCASH."Cheque No");
                PCASH.TestField(PCASH."Cheque Date");
                PCASH.TestField(PCASH."Check Printed", true);
            end;
            ...
            */
            ...
            if PCASH."Pay Mode" = 'CHEQUE' then
                GenJnLine."Bank Payment Type" := GenJnLine."bank payment type"::"Computer Check";
            GenJnLine."Check Printed" := PCASH."Check Printed";
            GenJnLine."Posting Date" := PCASH."Posting Date";
            GenJnLine."Document No." := PCASH."No.";
            GenJnLine."External Document No." := PCASH."Cheque No";
            GenJnLine."Payment Method Code" := PCASH."Pay Mode";
    ...
    //
    procedure PostBankTransfer1(PCASH: Record Payments)
    var
        PCASHLines: Record "PV Lines";
        GenJnLine: Record "Gen. Journal Line";
        LineNo: Integer;
    ...
    begin
        if Confirm('Are u sure u want to post this InterBank Transfer' + PCASH."No." + ' ?') = true then begin
            ...
            GenJnLine.SetRange("Journal Batch Name", CMSetup."Bank TR Journal Batch Name");
            if GenJnLine.FindSet then begin
                Codeunit.Run(Codeunit::"Gen. Jnl.-Post", GenJnLine);
            end;
    ...
    begin
        if Confirm(text025, false, Sclaims."No.") = true then begin
            ...
            //Add Deductions
            // PayPeriod.SetFilter(PayPeriod."Starting Date", Format(Sclaims."Payroll Period"));
            PayPeriod.SetFilter(PayPeriod."Starting Date", Format(Sclaims."Recovery Start Month"));
            // REPORT.RUN(69059,TRUE,TRUE,PayPeriod);
            Report.Run(Report::"Salary Advance Deductions", true, true, PayPeriod);
    ...
    //
    procedure "Post StandingImprestSurrender"(var Imprest: Record payments)
    var
        StandingImprestlines: Record "PV Lines";
        GenJnLine: Record "Gen. Journal Line";
        LineNo: Integer;
    ...
    begin
        ...
        if Confirm(text028, false, Imprest."No.") = true then begin
            if Imprest.Status <> Imprest.Status::Released then
                Error(Text003, Imprest."No.");
            Imprest.TestField(Imprest.Date);
            Imprest.CalcFields(Imprest."Total Amount");
            if Imprest."Total Amount" = 0 then
                Error(text022);
            Imprest.TestField("Account No.");
            //Imprest.TESTFIELD("Paying Bank Account");
            ...
            */
            ...
            Imprest.CalcFields("Standing Imprest Amount", "Amount Spent");
            if Imprest."Amount Spent" = 0 then
                Error(Text005);
            ...
            CMSetup.Get();
            CMSetup.TestField(CMSetup."StandingImprest Posting Group");
            CMSetup.TestField(CMSetup."Standing Imprest Account No");
            CMSetup.TestField(CMSetup."Standing Imp Journal Template");
            CMSetup.TestField(CMSetup."StandingImp Journal Batch Name");
            GenJnLine.Reset;
            GenJnLine.SetRange("Journal Template Name", CMSetup."Standing Imp Journal Template");
            ...
            GenJnLine."Account No." := Imprest."Account No.";
            GenJnLine.Validate("Account No.");
            //Change Employee Posting Group
            GenJnLine."Posting Group" := CMSetup."StandingImprest Posting Group";
            GenJnLine."Posting Date" := Imprest."Posting Date";
            GenJnLine."Document No." := Imprest."No.";
            GenJnLine."External Document No." := Imprest."Standing Imprest Issue Doc. No";
            GenJnLine."Gen. Posting Type" := GenJnLine."gen. posting type"::" ";
            //GenJnLine.Description:='Surrendered by: '+Imprest.Payee;
            if ImprestHeader.Get(Imprest."Standing Imprest Issue Doc. No") then
                GenJnLine.Description := copystr(ImprestHeader."Payment Narration", 1, 99);
            Imprest.CalcFields("Imprest Remaining Amount", "Standing Imprest Amount", "Amount Spent");
            if Imprest."Imprest Remaining Amount" < 0 then begin
                GenJnLine.Amount := -Imprest."Standing Imprest Amount";
            end else
                if Imprest."Imprest Remaining Amount" > 0 then begin
                    GenJnLine.Amount := -Imprest."Amount Spent";
                end
                else
                    GenJnLine.Amount := -Imprest."Amount Spent";
            GenJnLine.Validate(Amount);
          ...
          *///
            ...
            GenJnLine."Applies-to Doc. No." := Imprest."Standing Imprest Issue Doc. No";
            GenJnLine.Validate(GenJnLine."Applies-to Doc. No.");
            ...
            //IMP surrender Lines Entries
            StandingImprestlines.Reset;
            StandingImprestlines.SetRange(StandingImprestlines.No, Imprest."No.");
            if StandingImprestlines.FindSet() then begin
                repeat
                    //ImprestLines.TESTFIELD(ImprestLines."Job Task No.");
                    StandingImprestlines.Validate(StandingImprestlines.Amount);
                    LineNo := LineNo + 10000;
                    ...
                    GenJnLine."External Document No." := Imprest."Standing Imprest Issue Doc. No";
                    GenJnLine.Description := copystr(ImprestHeader."Payment Narration", 1, 99);
                    GenJnLine.Amount := StandingImprestlines."Actual Amount Spent";
                    // if StandingImprestlines."Remaining Amount" < 0 then begin
                    //     GenJnLine.Amount := StandingImprestlines.Amount;
                    // end else
                    //     if StandingImprestlines."Remaining Amount" > 0 then begin
                    //         GenJnLine.Amount := StandingImprestlines."Actual Amount Spent";
                    ...
                    //add jobs Morris
                    // GenJnLine."Job No." := StandingImprestlines."Job No.";
                    // GenJnLine."Job Task No." := StandingImprestlines."Job Task No.";
                    // GenJnLine."Job Quantity" := 1;
                    // GenJnLine.Validate(GenJnLine."Job No.");
                    // GenJnLine.Validate(GenJnLine."Job Task No.");
                    // GenJnLine.Validate(GenJnLine."Job Quantity"); //Added by Morris
            ...
            if ImprestHeader.Get(Imprest."Standing Imprest Issue Doc. No") then begin
                ImprestHeader.Surrendered := true;
                ImprestHeader."Surrender Date" := Today;
                ImprestHeader.Modify;
            end;
    ...
    begin
        ...
        BankAcc := LineValue."Bal. Account No.";
        //Message(BankAcc);
