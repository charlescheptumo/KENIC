...
Table 69009 "Assignment Matrix-X"
{
    ...
    fields
    {
            ...
            begin
                ...
                if Type = Type::Payment then begin
                    if Payments.Get(Code) then begin
                        ...
                        if Payments."Special Duty Allowance" = true then begin
                            "Special Duty Allowance" := true;
                        end;
                        ...
                        if Payments."Calculation Method" = Payments."calculation method"::"% of Basic pay" then begin
                            if Empl.Get("Employee No") then begin
                                ...
                                Amount := Abs((Payments.Percentage / 100 * (Empl.Basic)) - Payments."Deduct From Pension");
                                //         MESSAGE('Amount to be paid %1',FORMAT(Amount));
                                "Employer Amount" := (Payments."Percentage Employer" / 100 * (Empl.Basic)) - Payments."Deduct From Pension";
                        ...
                        if Payments."Calculation Method" = Payments."calculation method"::"% of Basic pay" then begin
                            if payments."Special Duty Allowance" = true then begin
                                HRSetup.Get;
                                if HRSetup."Special Duty Days" = 0 then
                                    Error('You must specify Special Duty Allowance Days under HR setup');

                                if Empl.Get("Employee No") then begin
                                    if Empl."Special Duty Days" = 0 then
                                        Error('You must Specify Special Duty Days for Employee %1 for the Current Pay Period in the Employee Card', Empl."No.");
                                    Empl.SetRange(Empl."Pay Period Filter", PayStartDate);
                                    Empl.CalcFields(Empl.Basic);
                                    Amount := (Payments.Percentage / 100 * Empl.Basic) * (Empl."Special Duty Days" / HRSetup."Special Duty Days");
                                    Amount := PayrollRounding(Amount);//round
                        ...
                        if Payments."Calculation Method" = Payments."calculation method"::"% of SHIF" then begin
                            if Empl.Get("Employee No") then begin
                                ...
                                AssignmentMatrix.SetRange("Payroll Period", PayStartDate);
                                AssignmentMatrix.SetRange(Code, 'SHIF');
                                if AssignmentMatrix.Find('-') then begin
                                    Amount := Payments.Percentage / 100 * Abs(AssignmentMatrix.Amount);
                                end;
                ...
                if Type = Type::Deduction then begin
                    if Deductions.Get(Code) then begin
                        ...
                        if Deductions."Calculation Method" = Deductions."calculation method"::"% of Basic Pay" then begin
                            if Empl.Get("Employee No") then begin
                                Empl.SetRange(Empl."Pay Period Filter", PayStartDate);
                                Empl.CalcFields(Empl.Basic, "Retirement Contribution", "SSF Employer to Date", Empl."Basic Arrears");
                                TotalBasic := Empl.Basic + (Deductions."Calculate Excess Pension" ? Empl."Basic Arrears" : 0);
                                Amount := Deductions.Percentage / 100 * TotalBasic;
                                Amount := PayrollRounding(Amount);//round
                                                                  // "Employer Amount" := (Deductions."Percentage Employer" / 100 * TotalBasic) * -1;
                                "Employer Amount" := -(Amount * Deductions."Percentage Employer" / Deductions.Percentage);
                        ...
                        //     "Employer Amount":=Deductions."Maximum Amount";
                        if Deductions."Calculation Method" = Deductions."Calculation Method"::"Flat Amount" then begin
                            Amount := LoansRounding(Amount);//round
                            "Employer Amount" := LoansRounding("Employer Amount") //round
                        end
                        else begin
                            Amount := PayrollRounding(Amount);//round
                            "Employer Amount" := PayrollRounding("Employer Amount");//round
                    ...
                    if Deductions."Calculation Method" = Deductions."calculation method"::"Based on Table" then begin
                        ...
                        if Deductions."Pension Scheme" then
                            ...
                            if (HRSetup."NSSF Tier II Code" = Deductions.Code) or (HRSetup."NSSF Tier I Code" = Deductions.Code) then begin
        ...
        field(65010; "Special Duty Allowance"; Boolean)
        {
        }
        field(65011; "Employee No Sort Key"; Text[50])
        {
            Caption = 'Employee No Sort Key';
            Editable = false;
        }
        field(65012; "Employee Status"; enum "Employee Status")
        {
    ...
    begin
        ...
        if Employee.Get("Employee No") then
            "Employee No Sort Key" := Employee."Employee No Sort Key";
    ...
    procedure LoansRounding(var Amount: Decimal) PayrollRounding: Decimal
    var
        HRsetup: Record "Human Resources Setup";
    begin

        HRsetup.Get;
        if HRsetup."Loan Rounding Precision" = 0 then
            Error('You must specify the rounding precision under HR setup');

        if HRsetup."Loan Rounding Type" = HRsetup."Loan Rounding Type"::Nearest then
            PayrollRounding := ROUND(Amount, HRsetup."Loan Rounding Precision", '=');

        if HRsetup."Loan Rounding Type" = HRsetup."Loan Rounding Type"::Up then
            PayrollRounding := ROUND(Amount, HRsetup."Loan Rounding Precision", '>');

        if HRsetup."Loan Rounding Type" = HRsetup."Loan Rounding Type"::Down then
            PayrollRounding := ROUND(Amount, HRsetup."Loan Rounding Precision", '<');
    end;
    ...
    //             if (BasicPay >= BracketTable."Lower Limit") and (BasicPay <= BracketTable."Upper Limit") then
    //                if BracketTable.Percentage <> 0 then
    //                 TableAmount := BasicPay * BracketTable.Percentage / 100
    //             else
    //                 TableAmount :=  BracketTable."Taxable Amount";
    ...
    begin
        ...
        IF BracketTable.FIND('-') THEN BEGIN
            REPEAT
                // Update of new NSSF Rates
                // if ((BasicPay-BracketTable."Lower Limit")>BracketTable."Lower Limit") AND ((BasicPay-BracketTable."Lower Limit")<=BracketTable."Upper Limit") then begin
                IF ((BasicPay >= BracketTable."Lower Limit") AND (BasicPay < BracketTable."Upper Limit")) THEN BEGIN
                    TableAmount := (BasicPay - BracketTable."Lower Limit") * (BracketTable.Percentage / 100);
                end else
                    if BasicPay > BracketTable."Upper Limit" then begin
                        TableAmount := (BracketTable."Upper Limit" - BracketTable."Lower Limit" + 1) * BracketTable.Percentage / 100;
                    end;
