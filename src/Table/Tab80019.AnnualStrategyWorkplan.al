...
Table 80019 "Annual Strategy Workplan"
{
    ...
    LookupPageID = "Approved Functional Workplans";
    ...
    fields
    {
        ...
        {
            CalcFormula = sum("Strategy Workplan Lines"."Q1 Budget" where(No = field(No)));
    ...
    //             rec."No" := NoSeriesMgt.GetNextNo(SPMSetup."Organizational PC Nos", WorkDate(), true);
    //         end;
    //     IF Rec."Annual Strategy Type" = Rec."annual strategy type"::"Functional PC" THEN
    //         if No = '' then begin
    //             SPMSetup.Get;
    //             SPMSetup.TestField("Functional Nos");
    //             rec."No" := NoSeriesMgt.GetNextNo(SPMSetup."Functional Nos", WorkDate(), true);
    //         end;
    ...
    //         rec."No" := NoSeriesMgt.GetNextNo(SPMSetup."Corp Strategic Plan Nos", WorkDate(), true);
    //     end;
    // end;
    trigger OnInsert()
    begin
        ...
        case Rec."Annual Strategy Type" of
            Rec."annual strategy type"::"Organizational PC":
                begin
                    SPMSetup.TestField("Organizational PC Nos");
                    Rec.No := NoSeriesMgt.GetNextNo(SPMSetup."Organizational PC Nos", WorkDate(), true);
                end;
            Rec."annual strategy type"::"Functional PC":
                begin
                    SPMSetup.TestField("Functional Nos");
                    Rec.No := NoSeriesMgt.GetNextNo(SPMSetup."Functional Nos", WorkDate(), true);
                end;
            Rec."annual strategy type"::Functional:
                begin
                    SPMSetup.TestField("Functional AWP Nos");
                    Rec.No := NoSeriesMgt.GetNextNo(SPMSetup."Functional AWP Nos", WorkDate(), true);
                end;
            else begin
                SPMSetup.TestField("Corp Strategic Plan Nos");
                Rec.No := NoSeriesMgt.GetNextNo(SPMSetup."Corp Strategic Plan Nos", WorkDate(), true);
