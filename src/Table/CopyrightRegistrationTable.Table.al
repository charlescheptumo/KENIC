#pragma warning disable AA0005, AA0008,AL0603, AA0018, AA0021, AL0432,AA0072, AA0137, AA0201, AA0204, AA0206, AA0218, AA0228, AL0254, AL0424, AS0011, AW0006 // ForNAV settings
Table 56022 "Copyright Registration Table"
{
    ...
    fields
    {
        field(1; "No."; Code[20])
        {
            ...
            begin
                if "No." <> xRec."No." then begin
                    SalesSetup.Get;
                    if "Registration Type" = "registration type"::new then
                        NoSeriesMgt.TestManual(SalesSetup."Copyright Registration Nos.");
                    if "Registration Type" = "registration type"::transfer then
                        NoSeriesMgt.TestManual(SalesSetup."Copyright Transfer Nos.");
                    "No. Series" := '';
                end;
        ...
        field(58002; "Customer Type"; Option)
        {
            ...
            OptionMembers = "Copyright Applicant",Customer;
        }
        ...
        {
            OptionCaption = 'Open,Pending,Approved,Rejected,Amending';
            OptionMembers = Open,Pending,Approved,Rejected,Amending;
        }
        ...
        field(58010; "Application Date"; Date)
        {
        }
        ...
        field(58049; "Applicant Account"; Code[50])
        {
            ...
            begin
                ...
                if Customer.Get("Applicant Account") then begin
                    ...
                    //   Customer.CalcFields("Login Type");
                    //   "County Code":=Customer."Campus Code";
        ...
        field(69041; AuthorPhysicalAddress; Text[100])
        {
        }
        field(69042; Submitted; Boolean)
        {
        }
        field(69043; "County Code"; Code[10])
        {
            TableRelation = Counties.Code;
        }
        field(69044; Decription; Text[30])
        {
        ...
        field(69045; draft; Boolean)
        {
        }
        field(69046; "Payment Method"; Text[30])
        {
        }
        field(69047; "Registration Type"; Option)
        {
            OptionMembers = new,transfer,ammendment;
        }
        field(69048; "Holder Customer Id"; Code[20])
        {
            TableRelation = Customer."No." where("Customer Type" = const(Institution));
        }
        field(69049; "Original certificate Surrender"; Boolean)
        {
        }
        field(69050; "Submit Agreement"; Boolean)
        {
        }
        field(69051; "Date of Change"; Date)
        {
        }
        field(69052; "Certificate Number"; Code[20])
        {
            TableRelation = "Service Item"."No." where("Applicant Account" = field("Holder Customer Id"));
        }
        field(69053; "Registration Date"; Date)
        {
        }
    }
    ...
    begin
        SalesSetup.Get();
        if "No." = '' then begin
            if "Registration Type" = "registration type"::new then begin
                rec."No." := NoSeriesMgt.GetNextNo(SalesSetup."Copyright Registration Nos.", WorkDate(), true);

            end else if "Registration Type" = "registration type"::transfer then begin
                rec."No." := NoSeriesMgt.GetNextNo(SalesSetup."Copyright Transfer Nos.", WorkDate(), true);
            end;

        end;

        "Created By" := UserId;
        //Ruth added on 17 July,2018 to avoid to many open documents.
        /*
        Limit:= 10;
        NCount:=0;
        objCopyright.RESET;
        objCopyright.SETRANGE(objCopyright.Status,objCopyright.Status::Open);
        objCopyright.SETRANGE(objCopyright."Created By",USERID);
        objCopyright.SETFILTER("No.",'<>%1',"No.");
         IF objCopyright.FINDSET THEN
                BEGIN
                repeat
                NCount:= NCount +1;
                //ERROR(Text016,objCopyright."No.");
                until objCopyright.NEXT = 0;
                END;
    ...
    var
        ...
        Text016: label 'The document %1 is still open,kindly use it to avoid too many open documents.';
    ...
    begin
        ...
        Cust."No." := NoSeriesMgt.GetNextNo(SalesSetup."Copyright Registration Nos.", WorkDate(), true);
        Rec := Cust;
        exit(true);
    end;
