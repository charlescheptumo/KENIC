    ...
    actions
    {
        area(navigation)
        {
            action("Post Disbursement(F9)")
            {
                ...
                PromotedOnly = true;
                Visible = false;
                ToolTip = 'Executes the Post Disbursement(F9) action.';
            ...
            action(AttachDocuments)
            {
                ...
                trigger OnAction()
                begin
                    DMSManagement.UploadAIEAttchmentDocuments(Rec."Document No", 'AIE Vouchers', Rec.RecordId, 'R48');
                end;
            }
            action(Reopen)
            {
            ...
            action("Post Receipt(F9)")
            {
                ...
                PromotedOnly = true;
                Visible = false;
                ToolTip = 'Executes the Post Receipt(F9) action.';
