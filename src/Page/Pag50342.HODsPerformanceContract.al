#pragma warning disable AA0005, AA0008,AL0603, AA0018, AA0021, AA0072, AA0137, AA0201, AA0206, AA0218, AA0228, AL0254, AL0424, AW0006
#pragma implicitwith disable
Page 50342 "HODs Performance Contract"
{
    Caption = 'HODs Annual Workplan';
    DeleteAllowed = false;
    PageType = Card;
        ...
        area(content)
        {
            group(General)
            {
                ...
                field("Responsible Employee No."; Rec."Responsible Employee No.")
                {
                    ...
                    trigger OnValidate()
                    var
                        ExistingInitiatives: Record "Perfomance Contract Header";
                        ExistingObjlines: Record "PC Objective";
                        CurrentPCoBJ: Record "PC Objective";
                        PrevPCObj: Record "PC Objective";
                        NewObjlines: Record "PC Objective";
                        LastLineNo: Integer;
                        ExistingSecondaryPCObjlines: Record "Secondary PC Objective";
                        CurrentSecondaryoBJ: Record "Secondary PC Objective";
                        PrevSecondaryObj: Record "Secondary PC Objective";
                        NewSecondarylines: Record "Secondary PC Objective";
                    begin
                        ExistingInitiatives.Reset();
                        ExistingInitiatives.SetRange("Annual Reporting Code", Rec."Annual Reporting Code");
                        ExistingInitiatives.SetRange("Responsible Employee No.", Rec."Responsible Employee No.");
                        ExistingInitiatives.SetRange("Approval Status", ExistingInitiatives."Approval Status"::Released);
                        if ExistingInitiatives.FindLast() then begin
                            IF CONFIRM('You have an existing scorecard for this same financial year.\Do you wish to proceed and Amend?') then begin

                                PrevPCObj.Reset();
                                PrevPCObj.SetRange("Workplan No.", Rec.No);
                                if PrevPCObj.findSet then
                                    PrevPCObj.DeleteAll();


                                ExistingObjlines.reset();
                                ExistingObjlines.SetRange("Workplan No.", ExistingInitiatives.No);
                                if ExistingObjlines.FindSet() then begin
                                    repeat
                                        CurrentPCoBJ.Reset();
                                        CurrentPCoBJ.SetRange("Workplan No.", Rec.No);
                                        if CurrentPCoBJ.FindLast() then begin
                                            LastLineNo := CurrentPCoBJ.EntryNo + 1;
                                        end else begin
                                            LastLineNo := 1;
                                        end;
                                        NewObjlines.Init();
                                        NewObjlines.TransferFields(ExistingObjlines);
                                        NewObjlines."Workplan No." := Rec.No;
                                        NewObjlines.EntryNo := LastLineNo;
                                        NewObjlines.Insert();
                                    until ExistingObjlines.Next() = 0;
                                    PrevSecondaryObj.Reset();
                                    PrevSecondaryObj.SetRange("Workplan No.", Rec.No);
                                    if PrevSecondaryObj.findSet then
                                        PrevSecondaryObj.DeleteAll();
                                    ExistingSecondaryPCObjlines.reset();
                                    ExistingSecondaryPCObjlines.SetRange("Workplan No.", ExistingInitiatives.No);
                                    if ExistingSecondaryPCObjlines.FindSet() then begin
                                        repeat
                                            CurrentSecondaryoBJ.Reset();
                                            CurrentSecondaryoBJ.SetRange("Workplan No.", Rec.No);
                                            if CurrentSecondaryoBJ.FindLast() then begin
                                                LastLineNo := CurrentSecondaryoBJ.EntryNo + 1;
                                            end else begin
                                                LastLineNo := 1;
                                            end;
                                            NewSecondarylines.Init();
                                            NewSecondarylines.TransferFields(ExistingSecondaryPCObjlines);
                                            NewSecondarylines."Workplan No." := Rec.No;
                                            NewSecondarylines.EntryNo := LastLineNo;
                                            NewSecondarylines.Insert();
                                        until ExistingSecondaryPCObjlines.Next() = 0;
                                    end;
                                    Rec.Amended := true;
                                    Rec."Amendment of Document No" := ExistingInitiatives.No;
                                    Rec."Amendment Date" := Today;
                                    HasBeenAmended := true;
                                    ExistingInitiatives.Amended := true;
                                    ExistingInitiatives."Amended to Document No" := Rec.No;
                                    Rec.Modify();
                                    ExistingInitiatives.Modify();
                                end;
                            end;
                        end;
                    end;
                ...
                field("Amended to Document No"; Rec."Amended to Document No")
                {
                    ApplicationArea = All;
                    Visible = HasBeenAmended;
                    ToolTip = 'Specifies the value of the Amended to Document No field.', Comment = '%';
                }
                field("Amendment of Document No"; Rec."Amendment of Document No")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Amendment of Document No field.', Comment = '%';
                    Visible = HasBeenAmended;
                }
                field("Amendment Date"; Rec."Amendment Date")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Amendment Date field.', Comment = '%';
                    Visible = HasBeenAmended;
                }
    ...
    begin
        if Rec.Amended = true then begin
            IsAmended := true;
        end else begin
            IsAmended := false;
        end;
