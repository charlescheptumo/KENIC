page 80544 "Approved Departments Pc"
{
    Caption = 'Departments\Centers Annual Workplan';
    DeleteAllowed = false;
    PageType = Card;
    SourceTable = 80055;
    ApplicationArea = All;
    ...
    actions
    {
        ...
        area(creation)
        {
            ...
            {
                Caption = ' Individual HOD Workplan';
                Image = Print;
                Promoted = true;
                PromotedCategory = Category6;
                ToolTip = 'Executes the  Individual HOD Workplan action.';

                trigger OnAction()
                begin
                    Rec.SETRANGE(No, Rec.No);
                    //REPORT.RUN(80018,TRUE,TRUE,Rec)
                end;
            }
            action("Print Sub Indicators")
            {
                Caption = 'Detailed HOD Workplan';
                Image = Print;
                Promoted = true;
                PromotedCategory = Category6;
                Visible = true;
                ToolTip = 'Executes the Detailed HOD Workplan action.';

                trigger OnAction()
                begin
                    Rec.SETRANGE(No, Rec.No);
                    //REPORT.RUN(80060,TRUE,TRUE,Rec)
                end;
            }
            action("Performance Contract Report")
            {
                Caption = 'Annual Work Plan Report';
                Image = Print;
                Promoted = true;
                PromotedCategory = Category6;
                ToolTip = 'Executes the Annual Work Plan Report action.';
