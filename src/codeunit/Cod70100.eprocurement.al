...
Codeunit 70100 eprocurement
{
    ...
    trigger OnRun()
    begin
        addFileLinks('Order', 'LPO001', 'CAPEX LIFE PENSIONS NOMINATION FORM (2025).pdf', 'https://kicd2020.sharepoint.com/sites/kicd/ERP Documents/KICD/LPO/VEND000001/CAPEX LIFE PENSIONS NOMINATION FORM (2025).pdf', 'pdf');
        addLpoSharepointLinks('LPO001', 'CAPEX LIFE PENSIONS NOMINATION FORM (2025).pdf', 'https://kicd2020.sharepoint.com/sites/kicd/ERP Documents/KICD/LPO/VEND000001/CAPEX LIFE PENSIONS NOMINATION FORM (2025).pdf');
    end;
            ...
            if StrLen(password) < 4 then begin
                ...
                if Contact.FindSet then begin
                    ...
                    if Vendor.Find('-') then begin
                        ...
                        Vendor."Country/Region Code" := country;
                        Vendor."E-Mail" := emailAddress;
                        Vendor."Post Code" := postCode;
                        Vendor.Validate("Post Code");
                        Vendor."Phone No." := phoneNumber;
                        Vendor.Modify();
                    end else begin
                        Vendor.Init();
                        Vendor."No." := NoSeriesManagement.GetNextNo(PurchasePayables."Vendor Nos.", WorkDate(), true);
                        Vendor.Name := vendorName;
                        Vendor."Country/Region Code" := country;
                        Vendor."E-Mail" := emailAddress;
                        Vendor."Post Code" := postCode;
    ...
    begin
        if VendorTemplMgt.SelectVendorTemplateFromContact(VendorTempl, Contact) then begin
            Message('Start of Creation as Vendor');
            exit(CreateVendorFromTemplate(VendorTempl.Code));
            Message('End of Creation as Vendor');
        end
        // exit(CreateVendorFromTemplate(VendorTempl.Code));
    end;
    ...
    begin
        ...
        if VendorTemplateCode <> '' then begin
            ...
            IsHandled := false;
            OnCreateVendorFromTemplateOnBeforeInitVendorNo(Vend, Contact, VendorTempl, IsHandled);
            if not IsHandled then
                VendorTemplMgt.InitVendorNo(Vend, VendorTempl);
        end;
        OnBeforeVendorInsert(Vend, Contact, VendorTemplateCode);
        Vend.Insert(true);
        Vend.SetInsertFromContact(false);
        VendorNo := Vend."No.";

        if ContComp.Type = ContComp.Type::Company then
            ContComp.Get(ContComp."No.")
        else
            ContComp.Get(ContComp."Company No.");
        ...
        OnAfterVendorInsert(Vend, Contact);

        UpdateCustVendBank.UpdateVendor(ContComp, ContBusRel);
        IsHandled := false;
        OnCreateVendorFromTemplateOnBeforeCommit(Contact, Vend, IsHandled);
        if not IsHandled then
            Commit();
        Vend.Get(Vend."No.");
        ...
        OnCreateVendorOnAfterUpdateVendor(Vend, Contact, ContBusRel);

        ShowResultForVendor(Vend);

        OnAfterCreateVendor(Contact, Vend);
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeCheckCompanyNo(Contact, IsHandled);
        if not IsHandled then
            ContactRec.TestField(ContactRec."Company No.");
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeCreateCompanyContactVendor(Contact, VendorNo, HideValidationDialog, VendorCreated, IsHandled);
        if IsHandled then
            exit(VendorCreated);

        if (ContactRec.Type = ContactRec.Type::Person) and (ContactRec."Company No." <> '') and (ContactRec."No." <> ContactRec."Company No.") then
            if Contact.Get(ContactRec."Company No.") then begin
    ...
    begin
        if ContactRec."Privacy Blocked" then
            Error(PrivacyBlockedGenericErr, ContactRec."No.", ContactRec.Name);
    end;

    local procedure ValidateSalesPerson()
    begin
        if ContactRec."Salesperson Code" <> '' then
            if Salesperson.Get(ContactRec."Salesperson Code") then
    ...
    begin
        IsHandled := false;
        OnBeforeCheckForExistingRelationships(Contact, LinkToTable.AsInteger(), IsHandled);
        if IsHandled then
            exit;
        ...
        if ContactRec."No." <> '' then begin
            CheckForCompanyContactExistingRelationships(Contact, ContBusRel);

            if ContBusRel.FindByContact(LinkToTable, Contact."No.") then
                Error(
                  AlreadyExistErr,
                  Contact.TableCaption(), ContactRec."No.", ContBusRel.TableCaption(), ContBusRel."Link to Table", ContBusRel."No.");
        end;
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeShowResultForVendor(Vendor, Contact, HideValidationDialog, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        if (Contact.Type = Contact.Type::Person) and (Contact."Company No." <> '') then
            if ContBusRel.FindByContact(ContBusRel."Link to Table", Contact."Company No.") then
                Error(
                  AlreadyExistErr,
                  Contact.TableCaption(), ContactRec."Company No.", ContBusRel.TableCaption(), ContBusRel."Link to Table", ContBusRel."No.");
    end;
    ...
    procedure CreateVendor(VendorNo: Code[20])// VendorNo: Code[20]
    var
        VendorTempl: Record "Vendor Templ.";
        VendorTemplMgt: Codeunit "Vendor Templ. Mgt.";
    begin
        if VendorTemplMgt.SelectVendorTemplateFromContact(VendorTempl, Contact) then begin
            Message('Start of Creation as Vendor');
            //exit(CreateVendorFromTemplate(VendorTempl.Code));
            CreateVendorFromTemplate(VendorTempl.Code);
            Message('End of Creation as Vendor');
        end
        // exit(CreateVendorFromTemplate(VendorTempl.Code));
    end;
    ...
    begin
        ...
        if Contact.FindFirst() then begin
            ...
            if DynasoftPortalUser.FindFirst then begin
                ...
                Body := 'Dear ' + DynasoftPortalUser."Full Name" + ',' + '<br><br>';
                Body := Body + 'You have successfully signed up as a vendor to <b>' + Company.Name + '</b>.' + '<br><br>';
                Body := Body + 'Your email address is your username.' + '<br>';
                Body := Body + 'Your one-time password to the supplier portal is <b>' + DynasoftPortalUser."Password Value" + '</b>.' + '<br><br>';
                // Body := Body + 'Please use the link below to change your password:<br>';
                // Body := Body + '<a href="https://eprocurement.adak.or.ke/Home/ChangePassword">Chnage Your Password</a><br><br>';
                Body := Body + 'Once you change the password , you will need to login to our portal and complete the following:<br>';
                Body := Body + '<ul>';
                Body := Body + '<li>Confidential Business Questionnaire</li>';
                Body := Body + '<li>Business Profile Summary</li>';
                Body := Body + '<li>Bank Account Details</li>';
                Body := Body + '</ul><br>';
                Body := Body + 'In case of any assistance, please contact our Procurement team via email.<br>' + Proc."Default Proc Email Contact" + '<br>';
                Body := Body + 'This is a system-generated email. Kindly do not respond.<br><br>';
    ...
    //         Body := Body + 'Your email address is your username.' + '<br>';
    //         Body := Body + 'Your one-time password to the supplier portal is <b>' + DynasoftPortalUser."Password Value" + '</b>.' + '<br><br>';
    //         Body := Body + ' ' + 'Kind Regards, Kenya Institute of Curriculum Development' + '<br><br>';
    ...
    procedure fnSupplierRegistration(businessType: Text; vendorType: Option; ownershipType: Option; dateofIncorporation: DateTime; dateofOps: DateTime; languageCode: Code[20];
     vendorNo: Code[20]; certofIncopNo: Code[20]; agpocertno: Code[10]; registeredspecialgroup: Code[10]; productservicecategory: Text;
     certeffectivedate: DateTime; certexpirydate: DateTime; haveagpo: Boolean) insertstatus: Text
    begin
        objVendor.Reset;
        ...
        if objVendor.FindSet then begin
            ...
            ObjVendor."Certificate of Registration" := certofIncopNo;
            objVendor."Ownership Type" := ownershipType;
            objVendor."Reg/Incorporation Date" := DT2DATE(dateofIncorporation);
            objVendor."Operations Start Date" := DT2DATE(dateofOps);
            objVendor."Gen. Bus. Posting Group" := 'LOCAL';
            objVendor."Vendor Posting Group" := 'SUPPLIERS';
            ...
            if objVendor.Modify(true) then begin
                ...
                if (objVendor."HaveAgpo?" = true) then begin
                    ...
                    if objVendorSpecialGroup.FindSet then begin
                        ...
                        objVendorSpecialGroup."Registered Special Group" := registeredspecialgroup;
                        objVendorSpecialGroup."Products/Service Category" := productservicecategory;
                        objVendorSpecialGroup."Certificate Expiry Date" := DT2DATE(certexpirydate);
                        objVendorSpecialGroup."Effective Date" := DT2DATE(certeffectivedate);
                        if objVendorSpecialGroup.Modify(true) then begin
    ...
    begin
        objDirectors.Reset;
        objDirectors.SetRange("Vendor No.", vendorNo);
        objDirectors.SetRange("ID/Passport No.", idnumber);
        if objDirectors.FindSet then begin
            //modify
            ...
            if type = 0 then begin
                ...
                objDirectors."ID/Passport No." := idnumber;
    ...
    procedure fnInsertBidResponseSecurityDetails(vendorNo: Code[50]; bidnumber: Code[50]; ifsnumber: Code[50]; formofbidesecurity: Code[50]; issuertype: Integer; securitytype: Integer; gaurantorname: Code[50]; issueroffices: Text; description: Text; securityamount: Decimal; bidsecurityeffectivedate: Datetime; bidsecurityexpirydate: Datetime) dirinserted: Text
    begin
        ...
        objTenderSecurity."Bid Security Effective Date" := DT2Date(bidsecurityeffectivedate);
        objTenderSecurity."Bid Security Validity Expiry" := DT2Date(bidsecurityexpirydate);
    ...
    procedure fnDeleteLitigationHistoryDetailsReq(vendorNo: Code[20]; litigationCode: Code[20]) status: Text
    var
        tbl_bidLitigationHistory: Record "Bid Litigation History";
    begin
        tbl_bidLitigationHistory.Reset;
        tbl_bidLitigationHistory.SetRange("Vendor No.", vendorNo);
        tbl_bidLitigationHistory.SetRange("No.", litigationCode);
        if tbl_bidLitigationHistory.FindFirst then begin
            if tbl_bidLitigationHistory.Delete(true) then begin
                status := 'success*Your Litigation History details have been successfully deleted!'
            end else begin
                status := 'danger*Your Litigation History details could not be deleted!'
            end
    ...
    procedure fnInsertPastExperienceDetails(vendorNo: Code[10]; clientName: Code[50]; address: Code[30]; assignmentProject: Text; projectScope: Text; projectStartDate: Datetime; projectEndDate: DateTime; projectValue: Decimal; contractRef: Code[50]; filename: Text) insertstatus: Text
    begin
        objPastXp.Reset;
        ...
        if objPastXp.Find('-') then begin
            ...
            objPastXp."Assignment Name" := assignmentProject;
            objPastXp."Project Scope Summary" := projectScope;
            objPastXp."Assignment Start Date" := DT2Date(projectStartDate);
            objPastXp."Assignment End Date" := DT2Date(projectEndDate);
            objPastXp."Assignment Value LCY" := projectValue;
            objPastXp."Contract Ref No" := contractRef;
            if objPastXp.Modify then begin
                //fnInsertLinks(vendorNo, sharepointlink, filename);
                insertstatus := 'success*Your past experience entry saved successfully!' + '*' + Format(objPastXp."Entry No.");
            end else begin
                insertstatus := 'danger*Unknown error occured!';
            end
    ...
    [NonDebuggable]
    procedure fnDeleteBidPastExperienceDetails(vendorNo: Code[20]; experienceCode: Integer) status: Text
    var
    ...
    procedure addEotReq(vendorNo: Code[50]; projectId: Code[50]; Cost: Decimal; Summary: Text; startDate: Datetime; endDate: Datetime) status: Text
    var
        EOTReq: Record "Project Variation Header";
    begin

        EOTReq.Reset();
        EOTReq.SetRange("Contractor No.", vendorNo);
        if EOTReq.Find('-') then begin
            EOTReq."Project ID" := projectId;
            EOTReq.Validate("Project ID");
            EOTReq."Cost Claim Amount" := Cost;
            EOTReq."Executive Summary" := Summary;
            EOTReq."Original Project Starting Date" := dt2date(startDate);
            EOTReq.Validate("Original Project Starting Date");
            EOTReq."Original Project Ending Date" := dt2date(endDate);
            EOTReq.Validate("Original Project Ending Date");
            if EOTReq.Modify(true) then begin
                status := 'success*You have submitted this entry successfully!' + '*' + EOTReq."Document No.";
            end;
        end else begin
            EOTReq.Init();
            EOTReq."Contractor No." := vendorNo;
            EOTReq."Project ID" := projectId;
            EOTReq.Validate("Project ID");
            EOTReq."Cost Claim Amount" := Cost;
            EOTReq."Executive Summary" := Summary;
            EOTReq."Document Type" := EOTReq."Document Type"::"Extension of Time Request";
            EOTReq."Original Project Starting Date" := dt2date(startDate);
            EOTReq.Validate("Original Project Starting Date");
            EOTReq."Original Project Ending Date" := dt2date(endDate);
            EOTReq.Validate("Original Project Ending Date");
            if EOTReq.Insert(true) then begin
                status := 'success*You have submitted this entry successfully!' + '*' + EOTReq."Document No.";
            end else begin
                status := 'danger*Unknown Error Occured!';
            end
        end;
    end;

    procedure sendEotForApproval(vendorNo: Text; requisitionNo: Code[50]) status: Text
    var
        EOTReq: Record "Project Variation Header";
        CustomApprovals: Codeunit "Custom Approvals Codeunit";
        VarVariant: Variant;
    begin
        //status := 'danger*The response could not be captured';
        EOTReq.Reset;
        EOTReq.SetRange("Document No.", requisitionNo);
        EOTReq.SetRange(Status, EOTReq.Status::Open);
        if EOTReq.FindSet then begin
            VarVariant := EOTReq;
            IF CustomApprovals.CheckApprovalsWorkflowEnabled(VarVariant) THEN
                CustomApprovals.OnSendDocForApproval(VarVariant);
            IF EOTReq.GET(EOTReq."Document No.") THEN BEGIN
                IF EOTReq.Status = EOTReq.Status::Open THEN BEGIN
                    status := 'warning*Your application  could not be submitted . Kindly try again or contact the administrator if this error persists';
                END ELSE BEGIN
                    status := 'success*Your Response was successfully Captured';
                END;
            end else begin
            ...
            status := 'info*Your have already submitted your response, kindly wait for feedback';
        end;
        exit(status);
    end;

    procedure addGroundForEot(docNo: Code[20]; vendorNo: Code[50]; projectId: Code[50]; projectDate: Datetime; Summary: Text) status: Text
    var
        EOTReq: Record "Project Variation Header";
        EOTLines: Record "Extension of Time Reason";

    begin
        EOTReq.Reset();
        EOTReq.SetRange("Contractor No.", vendorNo);
        EOTReq.SetRange("Document No.", docNo);
        if EOTReq.FindFirst() then begin
            EOTLines.Init;
            EOTLines."Line No." := getLineNo(docNo);
            EOTLines."Document No." := docNo;
            EOTLines."Cause of Delay" := Summary;
            EOTLines."Revised Project End Date" := Dt2date(projectDate);
            if EOTLines.Insert(true) then begin
                status := 'success*You have submitted this entry successfully!';
            end else begin
                status := 'danger*Unknown Error Occured!';
            end
        end;
    ...
    var
        OtherCosts: Record "Extension of Time Reason";
        lineNo: Integer;
    begin
        lineNo := 0;
        OtherCosts.Reset;
        OtherCosts.SetRange("Document No.", imprestNo);
    ...
    begin
        ...
        tbl_documentAttachments.URL1 := fileLink;
    ...
    procedure AddLpoSharepointLinks(storerequisitionnumber: Code[50]; filename: Text; sharepointlink: Text): Text
    var
        RFIResponse: Record "Purchase Header";
        RecordLink: Record "Record Link";
        RecordIDNumber: RecordID;
    begin
        RFIResponse.Reset();
        RFIResponse.SetRange("No.", storerequisitionnumber);
        if RFIResponse.FindFirst() then begin
            ...
            exit('success*Link added successfully.');
        end else
            exit('danger*Purchase Header not found.');
    end;

    procedure contractorvariationrequest(projectid: Code[50]; vendorNo: Code[30]; amount: Decimal) status: Text
    var
        PurchaseHeader: Record "Project Variation Header";
    begin

        PurchaseHeader.Init();
        PurchaseHeader."Contractor No." := vendorNo;
        PurchaseHeader.Validate("Contractor No.");
        PurchaseHeader."Project ID" := projectid;
        PurchaseHeader.Validate("Project ID");
        PurchaseHeader."Cost Claim Amount" := amount;
        PurchaseHeader."Document Type" := PurchaseHeader."Document Type"::"Contractor Variation Request";
        if PurchaseHeader.Insert(true) then begin
            status := 'success*Successfully inserted contract' + PurchaseHeader."Document No.";
        end
        else begin
            status := 'danger* Unable to insert the contract';
        end;
    end;
    ...
    begin
        ...
        objPastExperience."Assignment Start Date" := DT2Date(assignmentstartdate);
        objPastExperience."Assignment End Date" := DT2Date(assignmentenddate);
    ...
    begin
        ...
        if objVendor.FindSet then begin
            ...
            objVendor."Profile Complete" := true;
            if objVendor.Modify() then begin
                insertstatus := 'success*You have successfully submitted your registration, kindly wait for KICD to review and communicate to you!';
            end
            else begin
                insertstatus := 'danger*Unknown error Occured!';
            end
        ...
        if objVendor.FindSet then begin
            ...
            if objRFIResponse.FindSet then begin
                ...
                if objRFIResponse."Document Type" = objRFIResponse."document type"::"IFP Response" then begin
                    ...
                    objRFIResponse."Document No." := NoMgt.GetNextNo(ProcurementSetup."IFP Response Nos", WorkDate(), true);
    ...
    procedure fnInsertRFIResponseLines(documentNo: Code[30]; procurementCategory: Code[30]; rfiDocumentNo: Code[30]; vendorNo: Code[30]; evalutionDecision: Option) insertstatus: Text
    begin
        objIFPResponseLine.Reset;
        objIFPResponseLine.SetRange("Procurement Category", procurementCategory);
        ...
        if objIFPResponseLine.FindSet then begin
            objIFPResponseLine."Document No." := documentNo;
            objIFPResponseLine."Vendor No." := vendorNo;
            objIFPResponseLine.Validate("Vendor No.");
            objIFPResponseLine."RFI Document No." := rfiDocumentNo;
            objIFPResponseLine."Evaluation Decision" := evalutionDecision;
            if objIFPResponseLine.Modify(true) then begin
                insertstatus := 'success*Item successfully modified!';
    ...
    procedure fnPrequalificationResponseDetails(vendorNo: Code[20]; rfiDocumentNo: Code[12]; repFullname: Text; repDesignation: Text; rfiAppliNo: Code[50]; registrationPeriod: Code[50]) insertstatus: Text
    begin
        objRFIResponse.Reset;
        objRFIResponse.SetRange("Vendor No.", vendorNo);
        objRFIResponse.SetRange("RFI Document No.", rfiDocumentNo);
    ...
    procedure fnInsertPrequalificatinDocuments(vendorNo: Code[30]; procDoctype: Code[250]; docdescription: Text; certNo: Code[200]; issuedDate: DateTime; expiryDate: Datetime; fileName: Text; documentNo: Code[200]; SharepointLink: Text[500]) insertstatus: Text
    var
    ...
    begin
        ...
        if RFIResponseFiledDocument.FindFirst then begin
            ...
            RFIResponseFiledDocument."Issue Date" := DT2DATE(issuedDate);
            RFIResponseFiledDocument."Expiry Date" := DT2DATE(expiryDate);
    ...
    procedure fnInsertBidReponseDocuments(vendorNo: Code[30]; procDoctype: Code[250]; docdescription: Text; certNo: Code[200]; issuedDate: DateTime; expiryDate: Datetime; fileName: Text; documentNo: Code[200]; SharepointLink: Text[500]) insertstatus: Text
    var
    ...
    begin
        ...
        if BidFiledDocument.FindFirst then begin
            ...
            BidFiledDocument."Issue Date" := DT2DATE(issuedDate);
            BidFiledDocument."Expiry Date" := DT2DATE(expiryDate);
        ...
        if objVendor.FindSet then begin
            ...
            if not Purch.FindSet then begin
                ...
                if IFS.FindSet then begin
                    ...
                    Purch."Vendor Posting Group" := 'SUPPLIERS';
                    Purch.Insert(true);
                end;
                IFSLines.Reset;
                ...
                if IFSLines.FindSet then begin
                    ...
                    if Purch.FindSet then begin
                        repeat
                            ...
                            PurchLines."Gen. Bus. Posting Group" := 'LOCAL';
                            PurchLines."VAT Bus. Posting Group" := 'LOCAL';
                ...
                if Vendor.FindSet then begin
                    ...
                    Purch."Vendor Posting Group" := Vendor."Vendor Posting Group";
                    Purch."Nominal Capital LCY" := Vendor."Nominal Capital LCY";
                    Purch."VAT Bus. Posting Group" := Vendor."VAT Bus. Posting Group";
    ...
    begin
        ...
        if Purch.FindSet then begin
            ...
            if Purch.Modify then
                ...
                insertstatus := 'danger*Error saving your info, contact KICD Office!'
        end
        else begin
    ...
    begin
        ...
        if DynasoftPortalUser.FindSet then begin
            ...
            Body := 'Dear, ' + DynasoftPortalUser."Full Name" + ' your password for the account: ' + ' <strong>' + DynasoftPortalUser."Authentication Email" + '</strong> has been reset successfully. Kindly use the password below to access your account<br> <strong>Password: ' + '</strong> <br>';
            Body := Body + ' ' + 'Kind Regards, Anti Doping Agency of Kenya' + '<br><br>';
            Body := Body + '[This email is automated. Kindly do not reply to it]<br><br>';
            SMTPMail.Create(Contact."E-Mail", Subject, Body, true);
            Smail.Send(SMTPMail, Enum::"Email Scenario"::Default);
        end;
        ...
        if objVendor.FindSet then begin
            ...
            if not Purch.FindSet then begin
                ...
                if IFS.FindSet then begin
                    ...
                    purch."PP Procurement Method" := purch."PP Procurement Method"::RFQ;
                    Purch.Insert(true);
                ...
                if IFSLines.FindSet then begin
                    ...
                    if Purch.FindSet then begin
                        ...
                        if PurchLines.IsEmpty then begin
                            repeat
                                PurchLines.Init;
    ...
    procedure FnGenerateRFQPreviewReport1(VendorNumber: Code[20]; bidrsponseNumber: Code[20]) BaseImage: Text
    var
    ...
    begin
        ...
        if RfqResponse.FindSet then begin
            RecRef.GetTable(RfqResponse);
            Report.SaveAs(Report::"RFQ", '', ReportFormat::Pdf, OutStr, RecRef);
            FileManagement_lCdu.BLOBExport(TempBlob_lRec, STRSUBSTNO('RFQ_%1.Pdf', RfqResponse.Code), TRUE);
    ...
    procedure FnGeneratePrequalificationPreviewReport2(VendorNumber: Code[20]; PrequalificationNumber: Code[20]) BaseImage: Text
    var
        RecRef: RecordRef;
        RFIResponse: Record "RFI Response";
    begin
        ...
        RFIResponse.SetRange("Vendor No.", VendorNumber);
        if RFIResponse.FindSet then begin
            RecRef.GetTable(RFIResponse);
            Report.SaveAs(Report::"Prequalification Response", '', ReportFormat::Pdf, OutStr, RecRef);
            FileManagement_lCdu.BLOBExport(TempBlob_lRec, STRSUBSTNO('prequalification_%1.Pdf', RFIResponse."Document No."), TRUE);
            TempBlob_lRec.CreateInstream(InStr, TEXTENCODING::UTF8);
            BaseImage := Base64Convert.ToBase64(InStr);
        end;
    end;
    ...
    begin
        ...
        // '<BR>Anti Doping Agency of Kenya'+
        ...
        '<BR><BR>Regards,' +
        '<BR>Supply chain management' +
        '<BR>Kenya Institute of Curriculum Development' +
        '<BR><BR>Note: This is a system generated mail. Please DO NOT reply to it.';
        if CustEmail = '' then
            exit;
        ...
        if Contact.FindSet then begin
            ...
            if not ApplicantProfile.FindSet then begin
                ...
                if ApplicantProfile.Insert(true) then begin
                    ...
                    PortalUSer.SetRange("record type", PortalUSer."record type"::Vendor);
                    if not PortalUSer.findFirst() then begin
                        ...
                        if PortalUSer.Modify(true) then begin
                            status := 'success* Vendor modified successfully';
                        end;
                    end;
                end;
            end else begin
    ...
    begin
        ...
        if ContractMilestones.Find('-') then begin
            repeat
                ...
                Format(ContractMilestones."Start Date") + '* ' + Format(ContractMilestones."End Date") + '* ' + ContractMilestones."Milestone Deliverables" + '* ' + ContractMilestones."Payment Instalment Code");
    ...
    */
    ...
    begin
        ...
        if objVendor.FindSet then begin
            ...
            if objVendor.FindSet then begin
                ...
                //  objVendor."Language Code":=languageCode;
                //objVendor."Industry Group" := industryGroup;
                objVendor."Supplier Category" := industryGroup;
                objVendor."Vision Statement" := Vision;
                ...
                //  objVendor."Fax No.":=faxNo;
                //objVendor."Company Size" := companysize;
                objVendor."Company Size Code" := companysize;
                objVendor."Nominal Capital LCY" := nominalCap;
                objVendor."Max Value of Business" := maxbizvalue;
                objVendor."Phone No." := mobileNo;
                objVendor."Nature of Business" := natureofBiz;
    ...
    begin
        ...
        if DocumentoNo = '' then begin
            ...
            InsuranceGuaranteeHeader.Init;
            DocumentoNo := NoMgt.GetNextNo(RoadManagementSetup."Performance Guarantee Nos", WorkDate(), true);
