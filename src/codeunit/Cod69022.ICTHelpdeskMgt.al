...
Codeunit 69022 "ICT Helpdesk Mgt"
{

    trigger OnRun()
    begin
        FnCLoseICTIssue(ICTHelpdesk);
    end;
    ...
    procedure FnCLoseICTIssue(ICTHelpdesk: Record "ICT Helpdesk")
    begin
        if ICTHelpdesk."Attended Date" <> 0D then begin
            if Today >= CalcDate('1D', ICTHelpdesk."Attended Date") then begin
                ICTHelpdesk.Status := ICTHelpdesk.Status::Closed;
                ICTHelpdesk.Modify;
            end;
        end;
    end;
