...
Report 89028 "1 Page Payslip"
{
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            DataItemTableView = sorting("Employee No Sort Key");
            ...
            column(PayPeriodDisplay; UpperCase(Format(DateSpecified, 0, '<month text> <year4>')))
            {
            }
            ...
            begin
                ...
                if ResponsibilityCenter.FindFirst() then
                    DeptArr[1, 1] := ResponsibilityCenter.Name;

                //get Bank
                EmpBank.Reset();
                EmpBank.SetRange("Bank Branch No.", Employee."Bank Branch");
                if EmpBank.FindFirst then
                    BankArr[1, 1] := EmpBank."Bank Name";
                ...
                if Earn.Find('-') then begin
                    ...
                    repeat
                        ...
                        if AssignMatrix.Find('-') then begin
                            repeat
                                if i < 95 then begin  // safety guard
                ...
                if i < 50 then
                    i := i + 1;
                //cc
                // PAYE Deduction - run directly, no outer loop
                AssignMatrix.Reset;
                AssignMatrix.SetRange(AssignMatrix."Payroll Period", DateSpecified);
                ...
                if AssignMatrix.Find('-') then begin
                    repeat
                        ...
                        if LoanBalances.Find('-') then begin
                            ...
                            Evaluate(ArrEarningsAmt[1, i], Format(Abs(AssignMatrix.Amount)));
                            ArrEarningsAmt[1, i] := ChckRound(ArrEarningsAmt[1, i]);
                            if (AssignMatrix.Description = 'NSSF') or (AssignMatrix.Description = 'PENSION') then
                                TotalingDefineContAmount += Abs(AssignMatrix.Amount);
                        end;
                        TotalDeduction := TotalDeduction + Abs(AssignMatrix.Amount);
                ...
                if i < 50 then
                    i := i + 1;
                // taxation
                ArrEarnings[1, i] := 'Important Information';
                j := j + 1;
                ArrHeadings[i] := j;
                ...
                if Earn.Find('-') then begin
                    ...
                    AssignMatrix.SetRange(Code, 'PAYERELIEF');
                    if AssignMatrix.Find('-') then begin
                        TotalingDefineContAmount += Abs(AssignMatrix.Amount);
                    end;
                    // until Earn.Next = 0;
                ...
                if AssignMatrix.Find('-') then begin
                    ...
                    Evaluate(ArrEarningsAmt[1, i], Format(Abs(TotalingDefineContAmount)));
                    ArrEarningsAmt[1, i] := ChckRound(ArrEarningsAmt[1, i]);
                ...
                ArrEarningsAmt[1, i] := ChckRound(ArrEarningsAmt[1, i]);
                //     //Removed since they are not used at ADAK
                    ...
                    repeat
                        ...
                        if AssignMatrix.Find('-') then begin
                            repeat
                                ...
                                Evaluate(ArrEarningsAmt[1, i], Format(Abs(AssignMatrix.Amount)));
                                ArrEarningsAmt[1, i] := ChckRound(ArrEarningsAmt[1, i]);
                                if (AssignMatrix.Description = 'PAYE RELIEF') then
                                    TotalingDefineContAmount += Abs(AssignMatrix.Amount);
                ...
                if Ded.FindSet() then
                    repeat
                        ...
                        if Ded."Total Amount Employer" <> 0 then begin
                            ArrEarnings[1, i] := Ded.Description + '(Employer Contribution)';
    ...
    var
        ...
        PAY_PERIODCaptionLbl: label 'PAY PERIOD';
        ...
        PAYE: Decimal;
        ArrEarnings: array[3, 150] of Text[250];
        ArrDeductions: array[3, 100] of Text[250];
        Index: Integer;
        Index1: Integer;
        j: Integer;
        ArrEarningsAmt: array[3, 150] of Text[250];
        ...
        Totalprovid: Decimal;
        BalanceArray: array[3, 150] of Decimal;
        PensionContribution: Decimal;
        ArrHeadings: array[150] of Integer;
        tRelief: Date;
        ...
        TotalingDefineContAmount: Decimal;
