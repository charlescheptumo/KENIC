...
Report 69007 "Import payroll details"
{
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            begin
                ...
                for i := 1 to NoOfEarnings do begin
                    ...
                    //Check for 2/3 of pay
                    if NetPay <= ((BasicPay / 3) * 1) then
                        Error('The Salary for the Employee %1 is less than 2/3 of the basic pay.', Employee.FullName());
                end;
                ...
                if PayrollHeader.Find('-') then begin
                    ...
                    //Check for 2/3 of pay
                    if NetPay <= ((BasicPay / 3) * 1) then
                        Error('The Salary for the Employee %1 is less than 2/3 of the basic pay.', PayrollLines."Employee Name");
                    PayrollLines.Insert;
                end;
            end;
        ...
        {
            // DataItemTableView = where(Code = filter(<> 'D068'));
            ...
            trigger OnAfterGetRecord()
            begin
                // DeductionsX.SetRange(DeductionsX.Code, '<>%1', 'D068');
    ...
    begin
        ...
        //CC
        // DedRec.SetRange(DedRec.CODE, '<>%1', 'D068');
        if DedRec.Find('-') then
            repeat
