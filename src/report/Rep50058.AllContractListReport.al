report 50058 "All Contract List Report"
{
    ApplicationArea = All;
    Caption = 'All Contract List Report';
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/All Contract List Report.rdlc';
    ...
    dataset
    {
        dataitem("Purchase Header"; 38)
        {
            ...
            column(RemaningDays; RemaningDays)
            {
            }
            ...
            column(Duration; "Purchase Header"."Contract Period")
            {
            }
            column(ContractStatuse; ContractStatuse)
            {
            }
            ...
            column(Id_PurchaseHeader; "Purchase Header"."No.")
            {
            }
            ...
            column(Consitituency_PurchaseHeader; "Purchase Header".Consitituency)
            {
            }
            column(PriorityLevel_PurchaseHeader; "Purchase Header"."Priority Level")
            {
            }
            column(PlannedCommencementDate_PurchaseHeader; "Purchase Header"."Planned Commencement Date")
            {
            }
            column(RoadCode_PurchaseHeader; "Purchase Header"."Road Code")
            {
            }
            column(LinkName_PurchaseHeader; "Purchase Header"."Link Name")
            {
            }
            ...
            column(WorkPlanNo_PurchaseHeader; "Purchase Header"."WorkPlan No")
            {
            }
            ...
            begin
                ...
                IF "Purchase Header"."Contract End Date" > 0D THEN BEGIN

                    IF "Purchase Header"."Contract End Date" < TODAY THEN BEGIN
                        ContractStatuse := 'Expired';
                        RemaningDays := 0;
                    ...
                    IF "Purchase Header"."Contract End Date" > TODAY THEN BEGIN
                        ContractStatuse := 'Ongoing';
                        ContracttToday := CREATEDATETIME(TODAY, TIME);
                        ConttractEndDate := CREATEDATETIME("Purchase Header"."Contract End Date", TIME);
                        RemaningDays := ConttractEndDate - ContracttToday;

                    END;

                    // IF ("Purchase Header"."Contract End Date"<>0D) AND ("Purchase Header"."Contract Start Date"<>0D) THEN BEGIN
                    // Duration:="Purchase Header"."Contract End Date"-"Purchase Header"."Contract Start Date";
                    //
                    //  END;
                END ELSE BEGIN
                    ContractStatuse := '-'
    ...
    var
        ...
        ContractStatuse: Text[240];
        Duration: Duration;
        RemaningDays: Duration;
        ConttractEndDate: DateTime;
        ContracttToday: DateTime;
}
