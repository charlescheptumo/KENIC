...
report 50065 "Standard Purchase - Order 2"
{
    Caption = 'Purchase - Order';
    // DefaultRenderingLayout = "StandardPurchaseOrder.docx";
    DefaultLayout = Word;
    WordLayout = './Layouts/StandardPurchaseOrder2.docx';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem("Purchase Line"; "Purchase Line")
            {
                ...
                column(NoOfDays_PurchLine; "Purchase Line"."No. of Days")
                {
                }
                column(NoOfPeople_PurchLine; "Purchase Line"."No. of People")
                {
                }
            ...
            begin
                ...
                if ApprovalEntries.Find('-') then begin
                    ...
                    repeat
                        i := i + 1;
                        if i = 1 then begin
                            Approvers[1] := ApprovalEntries."Sender ID";
                            ApproverDate[1] := ApprovalEntries."Date-Time Sent for Approval";
                            if UserSetup.Get(Approvers[1]) then
                                UserSetup.CalcFields(Picture);

                            Approvers[2] := ApprovalEntries."Approver ID";
                            ApproverDate[2] := ApprovalEntries."Last Date-Time Modified";
                            if UserSetup1.Get(Approvers[2]) then
                                UserSetup1.CalcFields(Picture);
                        end;
                        if i = 2 then
                            if ApprovalEntries."Sequence No." = i then begin
                                Approvers[3] := ApprovalEntries."Approver ID";
                                ApproverDate[3] := ApprovalEntries."Last Date-Time Modified";
                                if UserSetup2.Get(Approvers[3]) then
                                    UserSetup2.CalcFields(Picture);
                            end;
                        if i = 3 then begin
                            Approvers[4] := ApprovalEntries."Approver ID";
                            ApproverDate[4] := ApprovalEntries."Last Date-Time Modified";
                            if UserSetup3.Get(Approvers[4]) then
    ...
    //     {
    //         Type = Word;
    //         LayoutFile = './Layouts/StandardPurchaseOrder2.docx';
