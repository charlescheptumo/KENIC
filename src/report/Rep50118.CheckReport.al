...
report 50118 CheckReport
{
    ApplicationArea = All;
    Caption = 'Check';
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Check2.rdlc';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            column(AmountInWords_GenJnlLine; AmountInWords)
            {
            }
            ...
            dataitem(CheckPages; "Integer")
            {
                ...
                column(BankAccountNo; BankAcc2."No.")
                {
                }

                column(BankBranchNo; BankAcc2."Bank Branch No.")
                {
                }
                column(BankBranchName; BankAcc2."Bank Branch Name")
                {
                }
                column(BankAccountNumber; BankAcc2."Bank Account No.")
                {
                }
                column(ChequeDate; payment."Cheque Date")
                {

                }
                column(payee; payment.Payee)
                { }
                column(amount; payment."Total Amount")
                { }
                column(BankName; BankAcc2."Bank Account  Name")
                {
                }
                column(AccountName; BankAcc2.Name)
                {
                }
                ...
                //     {
                //     }
                //     column(DateCheck; GenJnlLine."Posting Date")
                //     {
                //     }
                dataitem(PrintCheck; "Integer")
                {
                    ...
                    column(BankAccountNo_Print; BankAcc2."No.")
                    {
                    }
                    column(BankBranchNo_Print; BankAcc2."Bank Branch No.")
                    {
                    }
                    column(BankAccountNumber_Print; BankAcc2."Bank Account No.")
                    {
                    }
                    column(BankName_Print; BankAcc2."Bank Account  Name")
                    {
                    }
                    column(AccountName_Print; BankAcc2.Name)
                    {
                    }
                    column(CheckNoText_Print; CheckNoText)
                    {
                    }
                ...
                begin
                    ...
                    if not TestPrint then begin
                        Payments.Reset();
                        Payments.SetRange("No.", GenJnlLine."Document No.");
                        // if Payments.FindFirst() then begin
                        //     Payments."Cheque No" := UseCheckNo;
                            Payments."Check Printed" := true;
                        //     Payments.Modify();
                        // end;

                        BatchCheckLines.Reset();
                        BatchCheckLines.SetRange("Document No", GenJnlLine."Document No.");
                        if BatchCheckLines.FindSet() then begin
                            repeat
                                if Payments.Get(BatchCheckLines."PV No") then begin
                                    Payments."Cheque No" := UseCheckNo;
                                    Payments.Modify();
                                end;
                            until BatchCheckLines.Next() = 0;
                        end;

                        if BatchCheckVoucher.Get(GenJnlLine."Document No.") then begin
                            BatchCheckVoucher."Check No" := UseCheckNo;
                            BatchCheckVoucher."Check Printed" := true;
                            BatchCheckVoucher.Modify();
                        end;

                        CashMgt.Get();
                        CashMgt.TestField("Standing Vendor Check No");
                        if Vendor.Get(CashMgt."Standing Vendor Check No") then
            ...
            begin
                FormatNoText(NoText, "Amount (LCY)", "Currency Code");
                ...
                if payment.FindFirst() then begin
                    payment.CalcFields("Total Amount");
                    payment."Cheque Date" := Payment."Cheque Date";
                end else begin
                    Clear(payment);
                end;
                ...
                if not TestPrint then begin
                    ...
                    if BatchCheckVoucher.Get(GenJnlLine."Document No.") then begin
                        if BatchCheckVoucher.Payee <> '' then begin
                            Clear(CheckToAddr);
                            CheckToAddr[1] := BatchCheckVoucher.Payee;
                        end;
                    end;
    ...
    requestpage
    {
        ...
        begin
            ...
            if GenJnLine.FindFirst() then begin
                BankAcc2."No." := GenJnLine."Bal. Account No.";
                //Message(BankAcc2."No.");
            end;
