codeunit 50025 "Sharepoint Management"
{
    ...
    begin
        ...
        if SharePointClient.AddFileToFolder(FileDirectory, FileName, IS, SharePointFile) then begin//Use AddFileToFolder to create a file from a stream
            IsSuccess := true;
            GOdataID := SharePointFile.OdataId;
        end else begin
    ...
    Procedure CreateFolder(ServerRelativeUrl: Text): Boolean
    var
        SharePointFolder: Record "SharePoint Folder" temporary;
        Ok: Boolean;
    begin
        InitializeConnection(); //Initialize a connection if not connected
        Ok := SharePointClient.CreateFolder(ServerRelativeUrl, SharePointFolder);
        exit(Ok);
    end;
    ...
    procedure OpenFile(OdataID: Text; Filename: Text): Boolean
    var
        Success: Boolean;
    begin
        InitializeConnection(); //Initialize a connection if not connected
        Success := SharePointClient.DownloadFileContent(OdataID, FileName);
    end;
    ...
    begin
        ...
        // Secret := SecretStrSubstNo('%1', SharepointSetup."Client Secret");
        // exit(SharePointAuth.CreateAuthorizationCode(AadTenantId, SharepointSetup."Client ID", Secret, Scopes));
